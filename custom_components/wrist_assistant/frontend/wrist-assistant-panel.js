      ...
      <label class="field num part-size">${Oe("Font size",j?{atDefault:!1,title:`Back to the layer size (${m} pt)`,reset:()=>y(A=>{delete A.fontSize})}:void 0,eo(c.fontSize??m,U,{step:1,min:dl,max:cl}))}
        ...
        ${T?f:h`
          ...
          <div class="hint">A short tick on the scale at that value, so the fill reads
            ...
            nothing. Leave it empty for no mark.</div>`}`;break}case"chart":{let x=n.payload,$=(E,O)=>s(ge=>E(ge.payload),O),T=u.historyMinutes,F=u.historyPoints,D=x.historyMinutes>0,q=D&&x.source==="statistics",te=D&&!q,ke=D?q?"statistics":"history":"value",$t=q?Vi:An,Vt=In(x)??Ln(x),ze=x.value.kind.kind==="entityState",fe=Vt===void 0?void 0:e.historySeries(Vt),Ve=D&&ze?fe??"":e.resolve(x.value)??"",Je=x.historyPoints<1,pm=te&&ze&&Vt!==void 0?e.historyReadings?.(Vt):void 0,Nl=tx(Je,pm,Jn(x.historyMinutes)),Dl=ml(a,x.historyMinutes,$t),co=D&&ze?ws(Ve):{values:je(Ve),holes:[]},ti=co.values,Ol=E=>x.limit>0&&E.length>x.limit?x.takeFromEnd?E.slice(E.length-x.limit):E.slice(0,x.limit):E,hm=Ol(ti),Bt=ks(hm,rt(x.smoothing),co.holes.length>0?Ol(co.holes):[]),mm=!D&&ze&&ti.length===1,uo=e.config.elements.filter(E=>E.kind==="chart"&&E.payload.id!==a),Vl=de(e),ka=x.scaleFrom!==void 0&&uo.some(E=>E.payload.id===x.scaleFrom);C=h`
...
`;break}case"shape":C=h`<div class="grid2">
    ...
    <div class="sec-h pinned">
      ...
      <input type="text" aria-label="Layer name" .value=${n.payload.name??""} placeholder=${Th(n,de(e))}
    ...
    ${i.tap===!1?f:ax(e,n)}`}function $x(e,n){let t=n.payload.id,i=n.kind==="timeline",a=i?an(e.config,t):Xo(e.config,t),r=i?"Clock times":"Timestamp",o=e.activeFamily,s=()=>e.update(p=>{let m=p.elements.find(y=>y.payload.id===t);if(!m)return;let g=m.payload.frame;m.payload.frame={...Ie(p,o,m).frame},i?Pn(p,t):Zo(p,t,ue[o==="inline"?"rectangular":o]),m.payload.frame=g}),l=a.length>0,d=a.map(p=>({el:p,lead:B("clock"),title:r,kind:i?"Times":"Timestamp"})),c=i?"timeline:times":"image:time",u=h`
    ${Sh(i?"timeline":"image")}
    <div class="field list-field"><span>Draw</span>
      <div class="adders" @pointerover=${pn} @focusin=${pn}>
    ...
    ${s===void 0?f:Rh(s,n.partId,de(e),m=>u(g=>{m?g.partId=m:delete g.partId}))}
