...
const DIGEST_OUT = "src/symbol-digest.ts";
...
writeFileSync(
  DIGEST_OUT,
  `// Generated by build-icons.mjs. Do not edit.\nexport const SYMBOL_DIGEST = ${JSON.stringify(digest)};\n`
);

const kb = (n) => `${(n / 1024).toFixed(1)} kB`;
console.log(
  `build-icons: ${Object.keys(icons).length} symbols  ${kb(json.length)} → ${kb(gz.length)} gzipped  (${digest})`
);
console.log("build-icons: run `npm run build` next so the panel picks up the new digest");
