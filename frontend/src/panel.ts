...
export class WristAssistantPanel extends LitElement {
  ...
  private async loadOwners() {
    ...
    // shape, on this first open. Same rules: it reads every owner's records
    // itself, does nothing unless there is something to cut, skips a device
    // whose app is too old to resolve a shared slot by shape, and never writes
    // without proving first that the records can be put back.
