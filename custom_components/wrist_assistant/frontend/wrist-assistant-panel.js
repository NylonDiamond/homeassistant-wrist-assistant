...
`)}function Ml(e){let n;try{n=JSON.parse(e)}catch{return}if(typeof n!="object"||n===null||Array.isArray(n))return;let t=new Map,i=new Set;for(let[a,r]of Object.entries(n))r===null?i.add(a):t.set(a,yp(r));return{values:t,nullKeys:i}}function yp(e){return typeof e=="string"?e:typeof e=="boolean"?e?"true":"false":typeof e=="number"?(Number.isInteger(e)&&Math.abs(e)<1e15,String(e)):JSON.stringify(e)}function Br(e){let n=Yi(e),t=[...n.entities.entries()].sort(([i],[a])=>i<a?-1:i>a?1:0).map(([,i])=>({kind:"entity",entityId:i.entityId,displayName:i.displayName,domain:i.domain,...i.iconName!==void 0?{iconName:i.iconName}:{}}));return n.document&&t.push({kind:"template",value:n.document}),t}function bp(e,n){if(e.values.length!==0)switch(n){case"latest":return e.values[e.values.length-1];case"highest":return Math.max(...e.values);case"lowest":return Math.min(...e.values);case"average":return e.values.reduce((t,i)=>t+i,0)/e.values.length;case"top":return e.domainMax;case"bottom":return e.domainMin;case"first":return e.values[0];case"delta":return e.values[e.values.length-1]-e.values[0];case"sum":return e.values.reduce((t,i)=>t+i,0);case"trend":{let t=e.values[e.values.length-1]-e.values[0],i=Number(cr(t,e.domainMax-e.domainMin));return i>0?1:i<0?-1:0}}}var xp=10800;function vp(e,n,t){let i=n==="always"||n==="auto"&&t<=xp;return new Intl.DateTimeFormat(void 0,{hour:"numeric",...i?{minute:"2-digit"}:{},...e==="h12"?{hourCycle:"h12"}:{},...e==="h24"?{hourCycle:"h23"}:{}})}function Al(e,n,t,i,a){if(e<=0||n.length===0)return[];let r=vp(t,i,e);return n.map(o=>({position:o,text:r.format(new Date(a-e*1e3*(1-o)))}))}function wp(e,n){return Et(e)===void 0?[]:Al(je(e)*60,xr(e.timeLabelCount),e.hourCycle,e.minutes,n)}function kp(e,n){return Ni(e)?Al(Math.round(e.historyMinutes)*60,xr(ni(e.timeLabelCount)),e.hourCycle,e.minutes,n):[]}function Xt(e){let n=e;if(/^[+-]?(\d+\.?\d*|\.\d+)([eE][+-]?\d+)?$/.test(n))return Number(n);let t=n.toLowerCase();if(t==="inf"||t==="+inf"||t==="infinity"||t==="+infinity")return 1/0;if(t==="-inf"||t==="-infinity")return-1/0;if(t==="nan"||t==="+nan"||t==="-nan")return NaN}function Ye(e){let n=e.trim(),t=Xt(n);if(t!==void 0)return t;let i="";for(let r of n)if(/\p{N}/u.test(r)||r==="."||r==="-"||r==="+")i+=r;else if(i.length>0)break;return i.length===0?void 0:Xt(i)}function $p(e){return Number.isInteger(e)&&Math.abs(e)<1e16?e.toFixed(1):String(e)}function Cp(e){let n=Math.max(0,e);return n<60?`${Math.trunc(n)}s`:n<3600?`${Math.trunc(n/60)}m`:n<86400?`${Math.trunc(n/3600)}h`:`${Math.trunc(n/86400)}d`}function Sp(e){let n=e.trim(),t=Xt(n);if(t!==void 0)return t;let i=0,a=n.indexOf(",");if(a>=0){let s=n.slice(0,a).trim().split(" "),l=s.length===2?Xt(s[0]):void 0;if(l===void 0||s[1]!=="day"&&s[1]!=="days")return;i=l,n=n.slice(a+1).trim()}let r=n.split(":");if(r.length!==2&&r.length!==3)return;let o=0;for(let s=0;s<r.length;s++){let l=Xt(r[s]);if(l===void 0)return;o+=l*Math.pow(60,r.length-1-s)}return i*86400+o}function Tp(e){let n=Math.trunc(Math.min(Math.max(0,e)||0,863913600)),i=[[Math.trunc(n/86400),"d"],[Math.trunc(n%86400/3600),"h"],[Math.trunc(n%3600/60),"m"],[n%60,"s"]].filter(([a])=>a>0).slice(0,2).map(([a,r])=>`${a}${r}`);return i.length===0?"0s":i.join(" ")}function Ep(e){return e.replace(/\S+/g,n=>n.charAt(0).toUpperCase()+n.slice(1).toLowerCase())}function Mp(e,n,t){if(Me(n))return e;let i=n,a=e,r=Xt(e.trim()),o=i.duration?Sp(e):void 0;if(o!==void 0)a=Tp(o);else if(i.relativeTime&&r!==void 0)a=Cp(r);else{let s=Ye(e);if(s!==void 0){let l=s*(i.multiply??1)+(i.offset??0);i.decimals!==void 0?a=l.toFixed(Math.max(0,i.decimals)):l!==s&&(a=Number.isInteger(l)?String(l):$p(l))}}switch(i.useEntityUnit&&t&&(a+=t.startsWith("\xB0")||t.startsWith("%")?t:` ${t}`),i.prefix&&(a=i.prefix+a),i.suffix&&(a=a+i.suffix),i.textCase){case"upper":a=a.toUpperCase();break;case"lower":a=a.toLowerCase();break;case"capitalized":a=Ep(a);break}return a}function kn(e){let n=Math.trunc(Math.max(0,e)),t=Math.trunc(n/3600),i=Math.trunc(n%3600/60),a=n%60,r=o=>String(o).padStart(2,"0");return t>0?`${t}:${r(i)}:${r(a)}`:`${i}:${r(a)}`}function Je(e,n=240){return Hl(e,n).map(t=>t.value)}function Hl(e,n=240){let t=[],i="",a=0,r=!1,o=0,s=()=>{if(i!==""){let l=Number(i);Number.isFinite(l)&&t.push({value:l,start:a,end:a+i.length})}i=""};for(let l of e){if(t.length>=n)break;if(l>="0"&&l<="9")i===""&&(a=o),i+=l,r=!0;else if(l===".")i.includes(".")&&s(),i===""&&(a=o),i+=".",r=!0;else if(l==="-"||l==="+"){let d=!r;s(),d&&(a=o,i=l),r=!1}else s(),r=!1;o+=l.length}return t.length<n&&s(),t}function Fl(e,n,t){let i=Hl(e),a=i.map(y=>y.value),r=t.highlight??"none",o=-1,s=-1;a.length>0&&((r==="highest"||r==="both")&&(o=a.indexOf(Math.max(...a))),(r==="lowest"||r==="both")&&(s=a.indexOf(Math.min(...a))),s===o&&(s=-1));let l=t.coloring==="bands"?Ct({bands:t.bands??[]}):[],d=t.bandAboveColorHex??ie,c=t.highColorHex??He,u=t.lowColorHex??Le,p=[],m=(y,v)=>{if(y==="")return;let $=p.at(-1);$&&$.colorHex===v?$.text+=y:p.push({text:y,colorHex:v})},g=0;return i.forEach((y,v)=>{m(e.slice(g,y.start),n);let $=v===o?c:v===s?u:l.length>0?_i(y.value,l,d):n,S=e[y.end-1]==="."?y.end-1:y.end;m(e.slice(y.start,S),$),g=S}),m(e.slice(g),n),p}function Rl(e,n){let t,i;return n.scale==="fixed"?(t=Math.min(n.minValue,n.maxValue),i=Math.max(n.minValue,n.maxValue)):(t=e.length>0?Math.min(...e):0,i=e.length>0?Math.max(...e):1,n.thresholdValue!==void 0&&Number.isFinite(n.thresholdValue)&&(t=Math.min(t,n.thresholdValue),i=Math.max(i,n.thresholdValue))),n.baseline==="zero"&&(t=Math.min(t,0),i=Math.max(i,0)),i>t||(i=t+1),{min:t,max:i}}function Fp(e,n,t){let i=e.thresholdValue;if(!(i===void 0||!Number.isFinite(i)||!(t>n)||i<n||i>t))return(i-n)/(t-n)}function di(e,n=Tt){let t=[];for(let i of e.split(" ")){if(t.length>=n)break;if(i==="")continue;let a=i.indexOf(":");if(a<=0)continue;let r=Number(i.slice(0,a));!Number.isFinite(r)||r<0||t.push({offsetSeconds:Math.round(r),state:Rp(i.slice(a+1))})}return t}function Rp(e){try{return decodeURIComponent(e)}catch{return e}}function Ip(e,n,t){if(e.length===0||!(n>0))return[];let i=[];for(let r=0;r<e.length;r++){let o=e[r],s=Math.min(1,Math.max(0,o.offsetSeconds/n)),l=e[r+1],d=l===void 0?1:Math.min(1,Math.max(s,l.offsetSeconds/n));if(!(d>s))continue;let c=t(o.state),u=i[i.length-1];u!==void 0&&u.colorHex===c?u.end=d:i.push({start:s,end:d,colorHex:c})}let a=i[i.length-1];return a!==void 0&&(a.end=1),i}function Il(e,n,t){if(Number.isNaN(e))return t;let i=e<0?-Math.round(-e):Math.round(e);return Math.min(t,Math.max(n,i))}function Ap(e,n,t){if(e===void 0)return 0;let i=Ye(e);if(i===void 0||Number.isNaN(i))return 0;let a=t-n;return a===0?0:Math.min(1,Math.max(0,(i-n)/a))}var ut=class{constructor(n,t){this.ctx=n;this.charts=new Map;this.named=new Map(n.namedValues.map(i=>[i.id.toUpperCase(),i.value])),t&&this.settleCharts(t)}chartReadings(n){let t=this.chartSeries(n),i=Rl(t,n),a={values:t,domainMin:i.min,domainMax:i.max},r=this.chartEntity(n);return r&&(a.entity=r),a}chartSeries(n){let t=Gt(n),i=Kt(n),a;t!==void 0?a=this.ctx.historySeries?.get(t)??"":i!==void 0?a=this.ctx.historySeries?.get(i)??"":a=this.resolve(n.value)??"";let r=Je(a);return n.limit>0&&r.length>n.limit?n.takeFromEnd?r.slice(r.length-n.limit):r.slice(0,n.limit):r}chartEntity(n){let t=this.dereference(n.value);if(!(!t||!("entityId"in t.kind)))return{entityId:t.kind.entityId,displayName:t.kind.displayName,domain:t.kind.domain}}chartNowIndex(n,t){if(n.nowIndex===void 0||t===0)return;let i=this.resolve(n.nowIndex);if(i===void 0)return;let a=Ye(i);if(!(a===void 0||!Number.isFinite(a)))return Math.min(Math.max(Math.round(a),0),t-1)}settleCharts(n){let t=new Map,i=[];for(let s of n.elements)s.kind!=="chart"||t.has(s.payload.id)||(t.set(s.payload.id,s.payload),i.push(s.payload.id));let a=new Map;for(let s of i)a.set(s,this.chartSeries(t.get(s)));let r=new Map,o=(s,l)=>{let d=r.get(s);if(d)return d;let c=t.get(s);if(!c)return{min:0,max:1};let u=c.scaleFrom,p=u!==void 0&&u!==s&&t.has(u)&&!l.has(u)?o(u,new Set([...l,u])):Rl(a.get(s)??[],c);return r.set(s,p),p};for(let s of i){let l=t.get(s),d=o(s,new Set([s])),c={values:a.get(s)??[],domainMin:d.min,domainMax:d.max},u=this.chartEntity(l);u&&(c.entity=u),this.charts.set(s,c)}}dereference(n){let t=n,i=new Set,a=n.format;for(;t.kind.kind==="named";){let o=t.kind.id.toUpperCase();if(i.has(o))return;i.add(o);let s=this.named.get(o);if(!s)return;a=a&&!Me(a)?a:s.format,t=s}let r={kind:t.kind};return a&&(r.format=a),r}directEntityUnit(n){let t=n.kind;if(t.kind==="entityState"||t.kind==="entityAttribute"||t.kind==="entityAge")return this.ctx.entityStates.get(t.entityId)?.unitOfMeasurement;if(t.kind==="chartStat"){if(t.stat==="trend")return;let i=this.charts.get(t.layer.toUpperCase())?.entity;return i?this.ctx.entityStates.get(i.entityId)?.unitOfMeasurement:void 0}}resolve(n){if(!n)return;let t=this.dereference(n);if(!t)return;let i;switch(t.kind.kind){case"literal":i=t.kind.value;break;case"entityState":i=this.ctx.entityStates.get(t.kind.entityId)?.state;break;case"dataAge":i=this.ctx.dataAgeSeconds===void 0?void 0:String(Math.trunc(this.ctx.dataAgeSeconds));break;case"chartStat":{let a=this.charts.get(t.kind.layer.toUpperCase()),r=a?bp(a,t.kind.stat):void 0;a&&r!==void 0&&(i=t.kind.stat==="trend"?Ws(r):cr(r,a.domainMax-a.domainMin));break}default:{let a=Vr(n,this.named);i=a===void 0?void 0:this.ctx.templateResults.get(a)}}if(i!==void 0)return Mp(i,t.format,this.directEntityUnit(t))}nowMs(){return this.ctx.nowMs??Date.now()}countdownEnd(n){if(!n)return;let t=this.dereference(n);if(!t)return;let i=t.kind;if(i.kind==="entityState"){let o=this.ctx.entityStates.get(i.entityId);if(o?.timerState!==void 0){if(o.timerState!=="active"||!o.finishesAt)return;let s=Date.parse(o.finishesAt);return Number.isFinite(s)&&s>this.nowMs()?s:void 0}}let a=this.resolve(n)?.trim();if(!a)return;let r=Date.parse(a);if(!Number.isFinite(r)){let o=Xt(a);r=o===void 0?NaN:o*1e3}return Number.isFinite(r)&&r>this.nowMs()?r:void 0}canCountDown(n){if(!n)return!1;let t=this.dereference(n);if(t?.kind.kind==="entityState"){let i=t.kind.entityId;if(i.startsWith("timer.")||this.ctx.entityStates.get(i)?.timerState!==void 0)return!0}return this.countdownEnd(n)!==void 0}countdownFallbackText(n){if(!n)return;let t=this.dereference(n);if(!t||t.kind.kind!=="entityState")return;let i=this.ctx.entityStates.get(t.kind.entityId);if(i?.timerState!==void 0)return i.timerState==="paused"?i.remaining!==void 0&&i.remaining>0?kn(i.remaining):"Paused":"Idle"}entityIcon(n){let t=this.dereference(n);return!t||t.kind.kind!=="entityState"?void 0:this.ctx.entityStates.get(t.kind.entityId)?.iconName??t.kind.iconName}isStale(){return(this.ctx.dataAgeSeconds??1/0)>(this.ctx.stalenessThresholdSeconds??1200)}evaluateTest(n){let t=n.comparison;if(t.kind==="isStale")return this.isStale();let i=this.resolve(n.value);if(i===void 0)return t.kind==="isUnavailable";let a=Ye(i),r=()=>this.resolve(t.value),o=()=>{let l=r();return l===void 0?void 0:Ye(l)},s=l=>{let d=o();return a===void 0||d===void 0?!1:l(a,d)};switch(t.kind){case"equals":{let l=r();return l!==void 0&&i===l}case"notEquals":{let l=r();return l!==void 0&&i!==l}case"isOn":return i.toLowerCase()==="on";case"isOff":return i.toLowerCase()==="off";case"isUnavailable":{let l=i.toLowerCase();return l==="unavailable"||l==="unknown"}case"isEmpty":return i.trim().length===0;case"greaterThan":return s((l,d)=>l>d);case"greaterOrEqual":return s((l,d)=>l>=d);case"lessThan":return s((l,d)=>l<d);case"lessOrEqual":return s((l,d)=>l<=d);case"between":{let l=o(),d=this.resolve(t.upper),c=d===void 0?void 0:Ye(d);if(a===void 0||l===void 0||c===void 0)return!1;let[u,p]=l<=c?[l,c]:[c,l];return a>=u&&a<=p}case"timeBetween":{let l=qt(i),d=r(),c=this.resolve(t.upper),u=d===void 0?void 0:qt(d),p=c===void 0?void 0:qt(c);return l===void 0||u===void 0||p===void 0||u===p?!1:u<p?l>=u&&l<p:l>=u||l<p}case"contains":{let l=r();return!!l&&i.toLowerCase().includes(l.toLowerCase())}case"startsWith":{let l=r();return!!l&&i.toLowerCase().startsWith(l.toLowerCase())}case"endsWith":{let l=r();return!!l&&i.toLowerCase().endsWith(l.toLowerCase())}case"matchesRegex":{if(!t.pattern)return!1;try{return new RegExp(t.pattern).test(i)}catch{return!1}}case"isOneOf":return(t.options??[]).some(l=>l.toLowerCase()===i.toLowerCase());default:return!1}}evaluateCondition(n){return n.tests.length===0?!0:n.join==="any"?n.tests.some(t=>this.evaluateTest(t)):n.tests.every(t=>this.evaluateTest(t))}applyRules(n,t){let i=new Map;for(let a of n){let r=t?.get(a.id),o=[];if(r&&r!=="live")r==="otherwise"?o=a.otherwise??[]:o=a.cases.find(s=>s.id===r.caseId)?.then??[];else{let s=a.cases.find(l=>this.evaluateCondition(l.when));o=s?s.then:a.otherwise??[]}for(let s of o)i.set(Ee[s.kind],s)}return i}liveBranches(n){let t=new Map;for(let i of n){let a=i.cases.find(r=>this.evaluateCondition(r.when));t.set(i.id,a?a.id:i.otherwise?"otherwise":"none")}return t}styleColor(n,t){let i=n.get(t);if(!i)return;let a=this.resolve(i.value);return a||void 0}styleText(n,t){let i=n.get(t);return i?this.resolve(i.value):void 0}styleNumber(n,t){return n.get(t)?.number}resolveTextParts(n,t,i,a){let r=[];for(let o of n){let s=this.applyRules(t.filter(d=>d.partId===o.id),a);if(s.get("visibility")?.kind==="hide")continue;let l={text:this.styleText(s,"text")??this.resolve(o.value)??"--",fontSize:this.styleNumber(s,"fontSize")??o.fontSize??i.fontSize,fontWeight:s.get("fontWeight")?.weight??o.fontWeight??i.fontWeight,colorHex:this.styleColor(s,"color")??o.colorHex??i.colorHex};o.coloring==="bands"&&(o.bands?.length??0)>0&&(l.spans=Fl(l.text,l.colorHex,o)),r.push(l)}return r}resolveElement(n,t){let i=n.payload,a=n.kind==="text"?i.rules.filter(p=>p.partId===void 0):i.rules,r=this.applyRules(a,t),o=r.get("visibility"),s=o?o.kind==="hide":i.isHidden,l=this.styleNumber(r,"rotation"),d=l===void 0?i.frame:{...i.frame,rotationDegrees:l},c=this.styleNumber(r,"opacity")??1,u={id:i.id,isHidden:s,frame:d,opacity:c};switch(n.kind){case"text":{let p=n.payload.countdown?this.countdownEnd(n.payload.value):void 0,m=n.payload.countdown?this.countdownFallbackText(n.payload.value):void 0,g=at(n.payload)&&!r.has("text"),y={kind:"text",...u,text:g?"":this.styleText(r,"text")??m??this.resolve(n.payload.value)??"--",fontSize:this.styleNumber(r,"fontSize")??n.payload.fontSize,fontWeight:r.get("fontWeight")?.weight??n.payload.fontWeight,colorHex:this.styleColor(r,"color")??n.payload.colorSlot.baseColorHex,monospacedDigits:n.payload.monospacedDigits===!0,lineLimit:n.payload.lineLimit===2?2:1,alignment:n.payload.alignment??"center"};return p!==void 0&&(y.countdownEnd=p),g?(y.parts=this.resolveTextParts(n.payload.parts,i.rules,y,t),y.text=y.parts.map(v=>v.text).join(""),y):(Vs(n.payload)&&(y.spans=Fl(y.text,y.colorHex,n.payload)),y)}case"icon":{let p=this.entityIcon(n.payload.symbol)??this.resolve(n.payload.symbol)??"questionmark.circle",m=this.styleText(r,"icon"),g=n.payload.symbol.kind.kind==="literal",y=m===void 0&&g&&n.payload.path!==""?n.payload.path:void 0,v=m??p;y===void 0&&v.startsWith("mdi:")&&(v="questionmark.circle");let $={kind:"icon",...u,symbol:v,size:this.styleNumber(r,"fontSize")??n.payload.size,colorHex:this.styleColor(r,"color")??n.payload.colorSlot.baseColorHex};return y!==void 0&&($.path=y),$}case"gauge":{let p=n.payload,m=this.styleText(r,"gaugeValue")??this.resolve(p.value),g=(N,U,te)=>N??(U?Ye(this.resolve(U)??""):void 0)??te,y=g(this.styleNumber(r,"gaugeMin"),p.minSource,p.minValue),v=g(this.styleNumber(r,"gaugeMax"),p.maxSource,p.maxValue),$=m===void 0?void 0:Ye(m),S=this.styleColor(r,"color")??p.colorSlot.baseColorHex;p.coloring==="bands"&&p.bands.length>0&&$!==void 0&&(S=_i($,Ct(p),p.bandAboveColorHex));let k=v-y;if(p.total){let N=Ye(this.resolve(p.total)??"");N!==void 0&&(k=N)}let C=Il(k,1,Li),O={kind:"gauge",...u,fraction:Ap(m,y,v),style:p.style,lineWidth:p.lineWidth,colorHex:S,trackColorHex:p.trackColorHex,thresholdColorHex:p.thresholdColorHex,dotCount:C,filledCount:Il($??0,0,C)};if(p.thresholdValue!==void 0&&v!==y){let N=(p.thresholdValue-y)/(v-y);N>=0&&N<=1&&(O.thresholdFraction=N)}return O}case"chart":{let p=n.payload,m=this.charts.get(p.id)??this.chartReadings(p),g=m.values,y={min:m.domainMin,max:m.domainMax},v=this.styleColor(r,"color")??p.colorSlot.baseColorHex,$=Ct(p),S=Ks(p)?g.map(T=>_i(T,$,p.bandAboveColorHex)):[],k=p.highlight==="highest"||p.highlight==="both",C=p.highlight==="lowest"||p.highlight==="both",O=wt(p),N={kind:"chart",...u,values:g,style:p.style,domainMin:y.min,domainMax:y.max,baseline:p.baseline,barGap:p.barGap,lineWidth:p.lineWidth,colorHex:v,highColorHex:p.highColorHex,lowColorHex:p.lowColorHex,marker:p.marker,highMarker:k?O.high:"none",lowMarker:C?O.low:"none",pointColorHexes:S,fillBands:p.fillBands,thresholdColorHex:p.thresholdColorHex,nowColorHex:p.nowColorHex,labels:kp(p,this.nowMs()),labelSize:p.labelSize,labelColorHex:p.labelColorHex,labelsAbove:p.labelsAbove};if(g.length>0){let T=k?g.indexOf(Math.max(...g)):-1,V=C?g.indexOf(Math.min(...g)):-1;T>=0&&(N.highIndex=T),V>=0&&V!==T&&(N.lowIndex=V)}let U=Fp(p,y.min,y.max);U!==void 0&&(N.thresholdY=U);let te=this.chartNowIndex(p,g.length);return te!==void 0&&(N.nowIndex=te),N}case"timeline":{let p=n.payload,m=Et(p),g=m===void 0?"":this.ctx.historySeries?.get(m)??"",y=di(g,Tt),v=Ip(y,je(p)*60,S=>Zs(S,p.bands,p.otherColorHex));return{kind:"timeline",...u,runs:v,gap:p.gap,cornerRadius:p.cornerRadius,labels:wp(p,this.nowMs()),labelSize:p.labelSize,labelColorHex:p.labelColorHex,labelsAbove:p.labelsAbove}}case"shape":{let p={kind:"shape",...u,shapeKind:n.payload.kind,cornerRadius:n.payload.cornerRadius,thickness:n.payload.thickness,fillColorHex:this.styleColor(r,"color")??n.payload.colorSlot.baseColorHex,borderWidth:this.styleNumber(r,"borderWidth")??n.payload.borderWidth},m=this.styleColor(r,"borderColor")??n.payload.borderColorHex;return m!==void 0&&(p.borderColorHex=m),p}case"image":{let p={kind:"image",...u,entityId:n.payload.entity.entityId,source:n.payload.source,showTimestamp:n.payload.timestamp===!0,contentMode:n.payload.contentMode,zoom:n.payload.zoom,panX:n.payload.panX,panY:n.payload.panY,cornerRadius:n.payload.cornerRadius,timestampCorner:n.payload.timestampCorner,timestampSize:n.payload.timestampSize};rt(n.payload)&&(p.timestampX=n.payload.timestampX,p.timestampY=n.payload.timestampY);let m=this.ctx.entityStates.get(n.payload.entity.entityId)?.entityPicture;return m!==void 0&&(p.url=m),p}case"tap":{let p={kind:"tap",...u,frame:n.payload.frame,opacity:1,action:n.payload.action};return n.payload.openPageId!==void 0&&(p.openPageId=n.payload.openPageId),n.payload.attachedTo!==void 0&&(p.attachedTo=n.payload.attachedTo),p}}}resolveLayout(n,t,i){let a=n.perFamily[t];this.settleCharts(n);let r=ul(n,t).map(y=>this.resolveElement(y,i)),o=a?this.applyRules(a.rules,i):new Map,s={family:t,elements:r,cornerBodyShape:a?.cornerBodyShape??"wedge",borderWidth:this.styleNumber(o,"borderWidth")??a?.borderWidth??2},l=this.styleText(o,"text"),d=a?.bezelCountdown&&l===void 0?this.countdownEnd(a.bezelText):void 0,c=a?.bezelCountdown?this.countdownFallbackText(a.bezelText):void 0,u=l??c??this.resolve(a?.bezelText);u!==void 0&&(s.bezelText=u),d!==void 0&&(s.bezelCountdownEnd=d);let p=this.resolve(a?.curvedText);if(p!==void 0&&(s.curvedText=p),a?.curvedColorHex!==void 0&&(s.curvedColorHex=a.curvedColorHex),a?.bezelGauge){let y=a.bezelGauge,v=this.resolve(y.value),$=v===void 0?void 0:Ye(v);if($!==void 0){let S=Math.min(y.minValue,y.maxValue),k=Math.max(y.minValue,y.maxValue),C={value:Math.min(k,Math.max(S,$)),minValue:S,maxValue:k===S?S+1:k,colorHexes:y.colorHexes},O=this.resolve(y.minLabel);O!==void 0&&(C.minLabel=O);let N=this.resolve(y.maxLabel);N!==void 0&&(C.maxLabel=N),s.bezelGauge=C}}let m=this.styleColor(o,"backgroundColor")??a?.backgroundColorHex;m!==void 0&&(s.backgroundColorHex=m);let g=this.styleColor(o,"borderColor")??a?.borderColorHex;return g!==void 0&&(s.borderColorHex=g),s}};function Hp(e,n,t){let i=new ut(n,t),a=e.countdown?i.countdownEnd(e.value):void 0,o={text:(e.countdown?i.countdownFallbackText(e.value):void 0)??i.resolve(e.value)??"--"};return e.label&&(o.label=e.label),e.symbol&&(o.symbol=e.symbol),a!==void 0&&(o.countdownEnd=a),o}function Ji(e,n,t){let i=new ut(n),a={};for(let r of["rectangular","circular","corner"])e.supportedFamilies.includes(r)&&(a[r]=i.resolveLayout(e,r,t));return e.supportedFamilies.includes("inline")&&e.inline&&(a.inline=Hp(e.inline,n,e)),a}var Ce=xe,ui=[{label:"40 mm",screen:{width:162,height:197},slots:{rectangular:{width:141,height:51},circular:{width:40,height:40},corner:{width:26,height:26}},measured:!1},{label:"41 mm",screen:{width:176,height:215},slots:{rectangular:{width:153,height:55.5},circular:{width:43,height:43},corner:{width:29,height:29}},measured:!1},{label:"42 mm",screen:{width:187,height:223},slots:{rectangular:{width:163,height:59},circular:{width:46,height:46},corner:{width:31,height:31}},measured:!1},{label:"44 mm",screen:{width:184,height:224},slots:{rectangular:{width:160,height:58},circular:{width:45,height:45},corner:{width:30,height:30}},measured:!1},{label:"45 mm",screen:{width:198,height:242},slots:{rectangular:{width:172,height:62.5},circular:{width:48.5,height:48.5},corner:{width:32,height:32}},measured:!1},{label:"46 mm",screen:{width:208,height:248},slots:Ce,measured:!0},{label:"49 mm",screen:{width:205,height:251},slots:{rectangular:{width:178.5,height:64.5},circular:{width:50,height:50},corner:{width:33.5,height:33.5}},measured:!1}],$n=ui.find(e=>e.measured);function Bl(e){if(!e)return;let n=/^(\d+)x(\d+)$/.exec(e.trim());if(!n)return;let t=Number(n[1]),i=Number(n[2]);return ui.find(a=>a.screen.width===t&&a.screen.height===i)}function Zi(e,n){let t=Ce[n];if(e.width<=0||e.height<=0)return{scale:0,x:0,y:0,width:0,height:0};let i=Math.min(e.width/t.width,e.height/t.height),a=t.width*i,r=t.height*i;return{scale:i,x:(e.width-a)/2,y:(e.height-r)/2,width:a,height:r}}var Kr={regular:400,medium:500,semibold:600,bold:700},Lp=1.15;function Xe(e){if(!e)return;let n=e.startsWith("#")?e.slice(1):e;if(!/^[0-9a-fA-F]{6}([0-9a-fA-F]{2})?$/.test(n))return;let t=n.length===8?parseInt(n.slice(6,8),16)/255:1;return{color:`#${n.slice(0,6)}`,opacity:t}}function me(e,n,t="#FFFFFF"){let i=Xe(e)??{color:t,opacity:1};return{[n]:i.color,[`${n}-opacity`]:i.opacity}}function Ul(e,n){let t=Math.max(0,e.frame.width*n.width),i=Math.max(0,e.frame.height*n.height),a=(e.frame.x+e.frame.width/2)*n.width,r=(e.frame.y+e.frame.height/2)*n.height;return{x:a-t/2,y:r-i/2,w:t,h:i,cx:a,cy:r}}var pt=e=>e*.55;function Gl(e,n){switch(e){case"leading":return{anchor:"start",x:n.x};case"trailing":return{anchor:"end",x:n.x+n.w};default:return{anchor:"middle",x:n.cx}}}function _p(e,n){let t=e.split(/\s+/).filter(r=>r!=="");if(t.length<2)return[e];let i="",a=0;for(let r=0;r<t.length-1;r++){let o=i===""?t[r]:`${i} ${t[r]}`;if(i!==""&&o.length>n)break;i=o,a=r+1}return a===0&&(i=t[0],a=1),[i,t.slice(a).join(" ")]}function Pp(e,n,t){if(t<=0||e.length*pt(n)<=t)return e;let i=t-.8*n,a=Math.max(1,Math.floor(i/pt(n)));return`${e.slice(0,a).replace(/\s+$/,"")}\u2026`}function zp(e,n){if(!n||n.map(i=>i.text).join("")!==e)return;let t=[];for(let i of n)for(let a=0;a<i.text.length;a++)t.push(i.colorHex);return t}function Kl(e,n){if(n.length<2)return[0];let t=[...e.matchAll(/\S+/g)].map(i=>i.index);return[t[0]??0,t[n[0].split(" ").length]??e.length]}function Wl(e,n,t,i,a){let r=[],o=n,s=a,l=d=>d<t.length&&/\s/.test(t[d]);for(let d of e){let c=s;if(/\s/.test(d))for(l(o)&&(c=i[o]);l(o);)o++;else{for(;l(o);)o++;t.startsWith(d,o)&&(c=i[o],o+=d.length)}s=c;let u=r.at(-1);u&&u.look===c?u.text+=d:r.push({text:d,look:c})}return r}function Np(e){let n=[];for(let t of e){let i=t.spans!==void 0&&t.spans.map(a=>a.text).join("")===t.text?t.spans:[{text:t.text,colorHex:t.colorHex}];for(let a of i){let r={fontSize:t.fontSize,fontWeight:t.fontWeight,colorHex:a.colorHex};for(let o=0;o<a.text.length;o++)n.push(r)}}return n}function jl(e,n){return e.reduce((t,i)=>t+i.text.length*pt(i.look.fontSize*n),0)}function Op(e,n,t){let i=[...e.matchAll(/\S+/g)];if(i.length<2)return[e];let a=l=>n[l]?.fontSize??0,r=0,o=0;for(let l=0;l<i.length-1;l++){let d=i[l].index??0,c=o===0?0:pt(a(d-1));for(let u=d;u<d+i[l][0].length;u++)c+=pt(a(u));if(o>0&&r+c>t)break;r+=c,o=l+1}let s=l=>l.map(d=>d[0]).join(" ");return[s(i.slice(0,o)),s(i.slice(o))]}function Dp(e,n,t){if(t<=0||jl(e,n)<=t)return[...e];let i=[],a=0,r=0;e:for(let s of e){let l=s.look.fontSize*n,d=t-.8*l,c="";for(let u of s.text){if(r>0&&a+u.length*pt(l)>d){c!==""&&i.push({text:c,look:s.look});break e}c+=u,a+=u.length*pt(l),r+=1}i.push({text:c,look:s.look})}for(;i.length>0;){let s=i.at(-1);if(s.text=s.text.replace(/\s+$/,""),s.text!=="")break;i.pop()}let o=i.at(-1);return o?o.text+="\u2026":e[0]&&i.push({text:"\u2026",look:e[0].look}),i}function Vp(e,n,t){let i=Np(n),a=e.lineLimit===2&&t.w>0?Op(e.text,i,t.w):[e.text],r=Kl(e.text,a),o=a.map((S,k)=>Wl(S,r[k]??0,e.text,i,i[0])),s=Math.max(...o.map(S=>jl(S,1))),l=s>t.w&&t.w>0?Math.max(.5,t.w/s):1,d=o.map(S=>Dp(S,l,t.w)),{anchor:c,x:u}=Gl(e.alignment,t),p=Math.max(0,...d.flat().map(S=>S.look.fontSize))*l||e.fontSize*l,m=.35*p,g=p*1.15,y=S=>S.map(k=>{let C=me(k.look.colorHex,"fill");return w`<tspan font-size=${k.look.fontSize*l} font-weight=${Kr[k.look.fontWeight]??400} fill=${C.fill} fill-opacity=${C["fill-opacity"]}>${k.text}</tspan>`}),v=me(e.colorHex,"fill"),$=d.length>1?w`${d.map((S,k)=>w`<tspan x=${u} y=${t.cy+m+(k-(d.length-1)/2)*g}>${y(S)}</tspan>`)}`:y(d[0]);return w`<text x=${u} y=${t.cy+m} text-anchor=${c}
    ...
    fill=${v.fill} fill-opacity=${v["fill-opacity"]}>${$}</text>`}function Bp(e,n){if(e.parts!==void 0&&e.countdownEnd===void 0&&e.parts.map(y=>y.text).join("")===e.text)return e.text===""?f:Vp(e,e.parts,n);let t=me(e.colorHex,"fill");e.countdownEnd!==void 0&&e.countdownEnd>Date.now()&&(e={...e,text:kn((e.countdownEnd-Date.now())/1e3)});let i=e.lineLimit===2&&n.w>0?_p(e.text,n.w/pt(e.fontSize)):[e.text],a=Math.max(...i.map(y=>y.length))*pt(e.fontSize),r=a>n.w&&n.w>0?Math.max(.5,n.w/a):1,o=e.fontSize*r,s=i.map(y=>Pp(y,o,n.w)),{anchor:l,x:d}=Gl(e.alignment,n),c=zp(e.text,e.spans),u=c?Kl(e.text,i):[],p=(y,v)=>c?Wl(y,u[v]??0,e.text,c,e.colorHex).map($=>{let S=me($.look,"fill");return w`<tspan fill=${S.fill} fill-opacity=${S["fill-opacity"]}>${$.text}</tspan>`}):y,m=o*1.15,g=s.length>1?w`${s.map((y,v)=>w`<tspan x=${d} y=${n.cy+(v-(s.length-1)/2)*m}>${p(y,v)}</tspan>`)}`:p(s[0],0);return w`<text x=${d} y=${n.cy} text-anchor=${l} dominant-baseline="central"
    font-family="-apple-system, 'SF Pro Text', 'Helvetica Neue', Helvetica, Arial, sans-serif"
    font-size=${o} font-weight=${Kr[e.fontWeight]??400}
    style=${e.monospacedDigits?"font-variant-numeric: tabular-nums":f}
    fill=${t.fill} fill-opacity=${t["fill-opacity"]}>${g}</text>`}var Ur=2;function Up(e,n){let t=me(e.colorHex,"stroke"),i=me(e.trackColorHex,"stroke","#FFFFFF"),a=me(e.thresholdColorHex,"stroke","#FFFFFF"),r=e.lineWidth;if(e.style==="dots"){let m=n.w>=n.h,g=Math.max(1,e.dotCount),y=m?n.w:n.h,v=m?n.h:n.w,$=Math.max(1,Math.min(v,y/g-Ur)),S=g*$+(g-1)*Ur,k=(m?n.cx:n.cy)-S/2+$/2;return w`${Array.from({length:g},(C,O)=>{let N=k+O*($+Ur),U=O<e.filledCount?t:i;return w`<circle cx=${m?N:n.cx} cy=${m?n.cy:N} r=${$/2}
    ...
    <g transform="rotate(${c} ${n.cx} ${n.cy})">
      ...
      ${e.thresholdFraction===void 0?f:Gp(n,s,r,d*360*e.thresholdFraction,e.thresholdColorHex)}
    </g>`}function Gp(e,n,t,i,a){let r=me(a,"stroke","#FFFFFF"),o=i*Math.PI/180,s=Math.cos(o),l=Math.sin(o),d=t/2+1;return w`<line x1=${e.cx+s*(n-d)} y1=${e.cy+l*(n-d)}
    x2=${e.cx+s*(n+d)} y2=${e.cy+l*(n+d)}
    stroke-width="1" stroke=${r.stroke} stroke-opacity=${r["stroke-opacity"]} />`}var Kp=5;function Wp(e,n){let t=e.values,i=Math.max(t.length,1),r=e.highIndex!==void 0&&e.highMarker!=="none"||e.lowIndex!==void 0&&e.lowMarker!=="none"?Kp:0,o=e.style==="bars"?0:e.lineWidth/2,s=n.x,l=Math.max(n.w,0),d=n.y+r+o,c=Math.max(n.h-r-o*2,1),u=d+c,p=Math.max(e.domainMax-e.domainMin,Number.EPSILON),m=e.baseline==="lowest",g=m?c*.12:0,y=Math.min(Math.max(e.barGap,0),l/(i*2)),v=Math.max((l-y*(i-1))/i,.5),$=k=>Math.min(1,Math.max(0,(k-e.domainMin)/p)),S=k=>u-$(k)*c;return{count:t.length,barWidth:v,plotTop:d,plotBottom:u,plotLeft:s,plotRight:s+l,baselineY:m?u:S(0),yAtFraction(k){return u-Math.min(Math.max(k,0),1)*c},barRect(k){let C=s+k*(v+y),O=t[k],N,U;if(m){let te=g+$(O)*(c-g);N=u-te,U=u}else N=S(O),U=m?u:S(0),N>U&&([N,U]=[U,N]);return{x:C,y:N,w:v,h:Math.max(U-N,.5)}},point(k){let C=Math.max(l-o*2,0);return{x:t.length>1?s+o+C*k/(t.length-1):s+l/2,y:S(t[k])}},markerCenter(k,C){let O=C?this.barRect(k):void 0;return{x:O?O.x+O.w/2:this.point(k).x,y:n.y+r/2}}}}function jp(e,n){let{labelSize:t,rowHeight:i,body:a,showsLabels:r}=ql(e,n),o=r?Yl(e,n,t,i):void 0;if(e.values.length===0)return o===void 0?f:w`${o}`;let s=qp(e,a);return o===void 0?s:w`${s}${o}`}function qp(e,n){let t=Wp(e,n),i=me(e.colorHex,"fill"),a=me(e.highColorHex,"fill",e.colorHex),r=me(e.lowColorHex,"fill",e.colorHex),o=(u,p)=>w`<circle cx=${u.x} cy=${u.y} r="1.7" fill=${p.fill} fill-opacity=${p["fill-opacity"]} />`,s=[],l=e.pointColorHexes.length===t.count,d=u=>l?me(e.pointColorHexes[u],"fill",e.colorHex):i;if(e.style==="bars")for(let u=0;u<t.count;u++){let p=t.barRect(u),m=u===e.highIndex?a:u===e.lowIndex?r:d(u),g=Math.min(1.2,p.w/2,p.h/2);s.push(w`<rect x=${p.x} y=${p.y} width=${p.w} height=${p.h} rx=${g}
      ...
      fill=${y.fill} fill-opacity=${y["fill-opacity"]} />`});return r?w`${s}${Yl(e,n,t,i)}`:w`${s}`}function Jp(e,n){let t=me(e.fillColorHex,"fill"),i=e.borderColorHex?Xe(e.borderColorHex):void 0,a=i?e.borderWidth:0,r=a/2,o=i?i.color:"none",s=i?i.opacity:0;switch(e.shapeKind){case"circle":{let l=Math.min(n.w,n.h)/2-r;return w`<circle cx=${n.cx} cy=${n.cy} r=${Math.max(0,l)}
        ...
        fill=${t.fill} fill-opacity=${t["fill-opacity"]} stroke="none" />`}}}function Xp(e,n,t){if(e.path!==void 0&&e.path!==""){let o=me(e.colorHex,"fill"),s=e.size*Lp;return w`<g transform="translate(${n.cx-s/2} ${n.cy-s/2}) scale(${s/24})">
    ...
    <text x=${n.cx} y=${n.cy} text-anchor="middle" dominant-baseline="central" font-size=${r*.5}
      fill=${a.stroke} fill-opacity=${a["stroke-opacity"]} font-family="sans-serif">?</text>`}var Jr=.25,Zp=8;function Qp(e,n,t,i,a,r,o,s){let l={x:0,y:0,width:e,height:n};if(!(e>0)||!(n>0)||!(t>0)||!(i>0))return l;let d=Math.min(Math.max(Number.isFinite(r)?r:1,Jr),Zp),c=Math.max(e/t,n/i),u=Math.min(e/t,n/i),p=(a==="fit"?u:c)*d,m=t*p,g=i*p,y=Math.min(Math.max(Number.isFinite(o)?o:0,-1),1),v=Math.min(Math.max(Number.isFinite(s)?s:0,-1),1);return{x:-(m-e)/2*(1+y)+0,y:-(g-n)/2*(1+v)+0,width:m,height:g}}function Qi(e){let n=e.getHours()%12||12,t=i=>String(i).padStart(2,"0");return`${n}:${t(e.getMinutes())}:${t(e.getSeconds())}`}var Xi=4;function ea(e,n,t){let i=Math.min(Math.max(e.timestampSize,4),40),a=t.length*i*.578+i*.89,r=i*1.25;if(!(Number.isFinite(e.timestampX)&&Number.isFinite(e.timestampY))){let l=e.timestampCorner.endsWith("Leading")?n.x+Xi:n.x+n.w-Xi-a,d=e.timestampCorner.startsWith("top")?n.y+Xi:n.y+n.h-Xi-r;return{x:l,y:d,w:a,h:r,size:i,label:t}}let s=(l,d,c,u)=>u>=c?d+(c-u)/2:Math.min(d+c-u,Math.max(d,l-u/2));return{x:s(n.x+e.timestampX*n.w,n.x,n.w,a),y:s(n.y+e.timestampY*n.h,n.y,n.h,r),w:a,h:r,size:i,label:t}}function eh(e,n){if(e==="camera")return"camera.fill";switch(n.split(".")[0]){case"camera":return"camera.fill";case"person":return"person.crop.circle";case"media_player":return"music.note";default:return"photo"}}function th(e,n,t){let i=t.icons,a=`imgclip-${e.id}`,r=Math.max(0,e.cornerRadius),o=e.showTimestamp&&e.url?ea(e,n,Qi(new Date)):void 0,s=o?w`
        ...
        ${[["nw",o.x,o.y],["ne",o.x+o.w,o.y],["sw",o.x,o.y+o.h],["se",o.x+o.w,o.y+o.h]].map(([p,m,g])=>w`<rect data-ts-corner=${p} x=${m-l/2} y=${g-l/2} width=${l} height=${l}
            fill="#FFFFFF" stroke="#0A84FF" stroke-width="0.5" style="cursor:${p}-resize" />`)}`:f,c=e.url?t.imageSizes?.size(e.url):void 0,u;if(e.url&&c){let p=Qp(n.w,n.h,c.width,c.height,e.contentMode,e.zoom,e.panX,e.panY);u=w`<image href=${e.url} x=${n.x+p.x} y=${n.y+p.y} width=${p.width} height=${p.height}
      ...
      <g transform="translate(${n.cx-7} ${n.cy-7})">${i.render(eh(e.source,e.entityId),14,"#FFFFFF99")??f}</g>`;return w`
    <defs><clipPath id=${a}><rect x=${n.x} y=${n.y} width=${n.w} height=${n.h} rx=${r} /></clipPath></defs>
    <g clip-path=${`url(#${a})`}>${u}${s}</g>${d}`}function nh(e,n,t,i,a){if(!i)return f;let r=Math.min(10,n.w*.5,n.h*.5),o=a!==void 0?ih(a,n):void 0;return w`
    ...
    ${o!==void 0?w`<text x=${n.cx} y=${n.cy} text-anchor="middle" dominant-baseline="central"
          font-family="-apple-system, 'SF Pro Text', 'Helvetica Neue', Helvetica, Arial, sans-serif"
          font-size=${Wr} font-weight="600" fill="#FFD60A" fill-opacity="0.95">${o}</text>`:r>=5?w`<g transform="translate(${n.cx-r/2} ${n.cy-r/2})" opacity="0.8">${t.render("hand.tap.fill",r,"#FFD60A")??f}</g>`:f}`}var Wr=5;function ih(e,n){let t=Wr*.55,i=n.w-2;if(n.h<Wr*1.6||i<t*4)return;if(e.length*t<=i)return e;let a=Math.max(1,Math.floor(i/t)-1);return`${e.slice(0,a).replace(/\s+$/,"")}\u2026`}function jr(e,n,t){if(e.isHidden&&!t.showHidden)return f;let i=t.tapReview===!0,a=t.tapAreas===!0||i,r=i?t.tapFocusId:void 0,o=r!==void 0&&e.id===r,s=r!==void 0;if(e.kind==="tap"&&!a)return f;if(e.kind==="tap"&&e.attachedTo!==void 0&&(!i||s&&!o))return f;let l=Ul(e,n),d=i&&(!s||o),c;switch(e.kind){case"text":c=Bp(e,l);break;case"icon":c=Xp(e,l,t.icons);break;case"gauge":c=Up(e,l);break;case"chart":c=jp(e,l);break;case"timeline":c=Yp(e,l);break;case"shape":c=Jp(e,l);break;case"image":c=th(e,l,t);break;case"tap":c=nh(e,l,t.icons,a,d?ot(e.action):void 0);break}let u=i&&(e.kind!=="tap"||s&&!o)?.35:1,p=Math.min(1,Math.max(0,e.opacity))*(e.isHidden?.35:1)*u,m=t.highlightId===e.id,g=m||t.highlightIds?.includes(e.id)===!0,y=t.handles===!0&&(!s||o),v=g?w`<rect x=${l.x} y=${l.y} width=${l.w} height=${l.h} fill="none" stroke="#0A84FF" stroke-width="0.75" stroke-dasharray="2 1" vector-effect="non-scaling-stroke" />`:f,$=t.hoverId===e.id||t.hoverIds?.includes(e.id)===!0?w`<rect x=${l.x} y=${l.y} width=${l.w} height=${l.h} fill="#0A84FF" fill-opacity="0.22"
    ...
    transform="rotate(${e.frame.rotationDegrees} ${l.cx} ${l.cy})">${S}${c}${$}${v}${C}</g>`}function ta(e,n){return{quad:{width:104*e,height:124*e},cornerRadius:52*e,tile:n?{cx:(104-29.75)*e,cy:24*e}:{cx:70*e,cy:29.5*e},dial:{cx:0,cy:124*e,r:100.5*e},labelArc:{start:-90,end:-24}}}function Xr(e,n){return(n?23.5:34)*e}var Ll=10.5;function Jl(e,n){return(e===" "?.35:/[ILJ1.,:;'!|]/.test(e)?.34:/[MW]/.test(e)?.92:.66)*n}function _l(e,n){let t=0;for(let i of e)t+=Jl(i,n);return t}function Pl(e,n,t){let i=e.toUpperCase(),a=d=>Jl(d,t),r=.9*t,o=0;for(let d of i)o+=a(d);if(o<=n)return i;let s=0,l="";for(let d of i){if(s+a(d)+r>n)break;l+=d,s+=a(d)}return`${l.replace(/\s+$/,"")}\u2026`}function qr(e,n,t){let i=t*Math.PI/180;return{x:e.cx+n*Math.cos(i),y:e.cy+n*Math.sin(i)}}function Yr(e,n,t,i){let a=qr(e,n,t),r=qr(e,n,i);return`M ${a.x} ${a.y} A ${n} ${n} 0 0 1 ${r.x} ${r.y}`}function Xl(e,n,t,i){let{dial:a}=ta(e,!0),r=(i.end-i.start)*Math.PI/180;return{id:n,d:Yr(a,t,i.start,i.end),length:t*r}}function ah(e,n){let t=ta(e,!0);return Xl(e,n,t.dial.r,t.labelArc)}var zl=18.5,rh=113,oh={start:-71,end:-36},Nl=104,sh=6.2,Ol={start:-77,end:-30.5};function Dl(e){let n=e.replace("#",""),t=i=>parseInt(n.slice(i,i+2),16)||0;return[t(0),t(2),t(4)]}function Vl(e,n){if(e.length===0)return"#34C759";if(e.length===1)return e[0];let t=Math.min(1,Math.max(0,n))*(e.length-1),i=Math.min(e.length-2,Math.floor(t)),a=t-i,r=Dl(e[i]),o=Dl(e[i+1]),s=(l,d)=>Math.round(l+(d-l)*a);return`rgb(${s(r[0],o[0])}, ${s(r[1],o[1])}, ${s(r[2],o[2])})`}var Gr=11;function lh(e,n,t){let{dial:i}=ta(n,!0),a=Nl*n,r=180/(Math.PI*Nl),o=e.minLabel!==void 0?_l(e.minLabel,Gr)*r:0,s=e.maxLabel!==void 0?_l(e.maxLabel,Gr)*r:0,l=Ol.start+(o>0?Math.max(0,o-1.8):0),d=Ol.end-(s>0?Math.max(0,s-1.8):0),c=d-l,u=24,p=[];for(let $=0;$<u;$++){let S=l+c*$/u,k=Math.min(d,l+c*($+1)/u+.4);p.push(w`<path d=${Yr(i,a,S,k)} fill="none"
      stroke=${Vl(e.colorHexes,($+.5)/u)} stroke-width=${sh*n}
    ...
    <circle cx=${g.x} cy=${g.y} r=${3.2*n} fill=${Vl(e.colorHexes,m)}
      stroke="#000000" stroke-width=${1.2*n} />
    ${e.minLabel!==void 0?v(`${t}-gmin`,l-y-Math.max(o,3),l-y,e.minLabel):f}
    ${e.maxLabel!==void 0?v(`${t}-gmax`,d+y,d+y+Math.max(s,3),e.maxLabel):f}`}function na(e,n){let t=e.family in Ce?e.family:"rectangular",i=n.slot??Ce[t],a=Ce[t],r=Zi(i,t),o=`clip-${t}-${Math.random().toString(36).slice(2,8)}`,s=Xe(e.backgroundColorHex),l=Xe(e.borderColorHex),d=e.borderWidth*r.scale;if(t==="corner"){let g=r.scale,y=!!e.bezelText||!!e.bezelGauge,v=e.curvedText??"",$=v!=="",S=ta(g,y),k=Xr(g,y),C=k/(a.width*g),O=S.tile.cx-k/2,N=S.tile.cy-k/2,U=`M 0 0 H ${S.quad.width-S.cornerRadius} A ${S.cornerRadius} ${S.cornerRadius} 0 0 1 ${S.quad.width} ${S.cornerRadius} V ${S.quad.height} H 0 Z`,te=f;if(e.bezelGauge)te=lh(e.bezelGauge,g,o);else if(e.bezelText){let V=ah(g,`${o}-bezel`),Z=e.bezelCountdownEnd!==void 0&&e.bezelCountdownEnd>Date.now()?kn((e.bezelCountdownEnd-Date.now())/1e3):e.bezelText;te=w`<defs><path id=${V.id} d=${V.d} /></defs>
        <text font-size=${Ll*g} font-weight="600" fill="#FFFFFF" font-family="-apple-system, 'SF Pro Text', Helvetica, Arial, sans-serif">
          <textPath href="#${V.id}" startOffset="50%" text-anchor="middle">${Pl(Z,V.length,Ll*g)}</textPath></text>`}let T=f;if($){let V=Xe(e.curvedColorHex??"#FFFFFF")??{color:"#FFFFFF",opacity:1},Z=Xl(g,`${o}-curved`,rh*g,oh);T=w`<defs><path id=${Z.id} d=${Z.d} /></defs>
        <text font-size=${zl*g} font-weight="600" fill=${V.color} fill-opacity=${V.opacity}
          font-family="-apple-system, 'SF Pro Rounded', 'SF Pro Text', Helvetica, Arial, sans-serif">
          <textPath href="#${Z.id}" startOffset="50%" text-anchor="middle">${Pl(v,Z.length,zl*g*.88)}</textPath></text>`}else{let V=e.borderWidth*r.scale*C,Z=l?w`<circle cx=${k/2} cy=${k/2} r=${k/2-V/2} fill="none" stroke=${l.color} stroke-opacity=${l.opacity} stroke-width=${V} />`:f;T=w`<g transform="translate(${O} ${N})">
        <g clip-path=${`url(#${o})`}>
    ...
    stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${Sh(e)}</svg>`}var Qe="color-mix(in srgb, var(--k) 45%, #6b7280)",pa='system-ui, -apple-system, "Segoe UI", sans-serif';function Cd(e,n,t,i){let r=135+270*Math.max(0,Math.min(1,i)),o=c=>{let u=c*Math.PI/180;return{x:(e-t*Math.cos(u)).toFixed(2),y:(n-t*Math.sin(u)).toFixed(2)}},s=o(135),l=o(r),d=r-135>180?1:0;return`M${s.x} ${s.y}A${t} ${t} 0 ${d} 1 ${l.x} ${l.y}`}function lo(e,n,t,i){return w`<g fill="none" stroke-linecap="round">
    <path d=${Cd(e,n,t,1)} stroke=${Qe} stroke-width="2.6" opacity=".5" />
    <path d=${Cd(e,n,t,i)} stroke="var(--k)" stroke-width="2.6" />
  </g>`}function Th(e){switch(e){case"text":return w`<g font-family=${pa} text-anchor="middle">
    ...
    <input type="text" .value=${n} placeholder=${i.placeholder??""} list=${i.list??f}
      class=${i.mono?"mono":""} @input=${he(t)} /></label>`}function rc(e,n,t,i=3){return h`<label class="field"><span>${e}</span>
    <textarea rows=${i} .value=${n} class="mono" @input=${he(t)}></textarea></label>`}function oe(e,n,t,i={}){return h`<label class="field num">${Ie(e,Rt(n,i.def,t),Aa(n,t,i))}${Ha(n,t,i)}</label>`}function Ha(e,n,t){let i=e===void 0||Number.isNaN(e)?"":String(e),a=h`<input type="number" .value=${i} step=${t.step??"any"} min=${t.min??f} max=${t.max??f}
    ...
    </div>`}function Nh(e,n){let t=i=>Rt(i.value,i.def,i.set,a=>i.options.find(([r])=>r===a)?.[1]??a);return h`<div class="field seg-field pair">${Ie(e.label,t(e))}
    <div class="pair-row">
    ...
    <div class="color-row">
      ...
      <span class="num-box alpha" style="--wa-unit:1">
        <input type="number" min="0" max="100" step="1" .value=${String(s)} title="Opacity" aria-label=${`${e}: opacity`} ?disabled=${i}
          @input=${he(l=>{let d=Number(l);l.trim()!==""&&d>=0&&d<=100&&t(Co(o,Math.round(d)))})} />
        <span class="unit" aria-hidden="true">%</span>
      </span>
    </span>`}function Ao(e,n){return e===void 0||n===void 0?e===n:e.replace(/^#/,"").toUpperCase()===n.replace(/^#/,"").toUpperCase()}function La(e,n){let t=e[n],i=t&&typeof t.attributes.friendly_name=="string"?t.attributes.friendly_name:n;return{entityId:n,displayName:i,domain:n.split(".")[0]??""}}function Oh(e,n,t){let i=n===void 0?void 0:typeof n=="string"?[n]:n,a=[];for(let[r,o]of Object.entries(e)){let s=r.split(".")[0]??"";if(i!==void 0&&!i.includes(s))continue;let l=typeof o?.attributes?.friendly_name=="string"?o.attributes.friendly_name.trim():"",d=t?.(r);a.push({entityId:r,name:l||r,state:o?.state??"",domain:s,...d?{area:d}:{}})}return a.sort((r,o)=>r.name.localeCompare(o.name)||r.entityId.localeCompare(o.entityId)),a}function Od(e){let{entities:n,devices:t,areas:i}=e;if(!n||!i)return;let a=r=>{if(!r)return;let o=i[r]?.name;return typeof o=="string"&&o.trim()!==""?o.trim():void 0};return r=>{let o=n[r];if(o)return a(o.area_id)??a(o.device_id?t?.[o.device_id]?.area_id:void 0)}}var sc=50;function Dh(e){let n=e.state.trim().split(/\s+/)[0]??"";return n!==""&&Number.isFinite(Number(n))}function Vh(e,n,t=sc,i){let a=n.trim().toLowerCase(),r=l=>i===void 0||i(l)?0:1;if(a==="")return(i===void 0?e.slice():[...e].sort((l,d)=>r(l)-r(d))).slice(0,t);let o=a.split(/\s+/),s=[];for(let l of e){let d=l.entityId.toLowerCase(),c=l.name.toLowerCase(),u=(l.area??"").toLowerCase(),p=-1;d===a?p=0:d.startsWith(a)?p=1:c.startsWith(a)?p=2:d.includes(a)?p=3:c.includes(a)?p=4:o.length>1&&o.every(m=>d.includes(m)||c.includes(m))?p=5:u!==""&&(u.includes(a)||o.length>1&&o.every(m=>d.includes(m)||c.includes(m)||u.includes(m)))&&(p=6),p>=0&&s.push({c:l,rank:p})}return s.sort((l,d)=>l.rank-d.rank||r(l.c)-r(d.c)||l.c.name.localeCompare(d.c.name)||l.c.entityId.localeCompare(d.c.entityId)),s.slice(0,t).map(l=>l.c)}var Bh=/^[a-z0-9_]+\.[a-z0-9_]+$/i;function lc(e){return Bh.test(e.trim())}function Uh(e,n,t){let i=e.trim();if(i!==n.entityId){if(i==="")return{entityId:"",displayName:"",domain:""};if(i in t)return La(t,i);if(lc(i))return{...n,entityId:i,domain:i.split(".")[0]??""}}}var tn=new Map;function ke(e){let n=e instanceof Node?e:null;for(let t=0;n&&t<8;t+=1){let i=n.getRootNode();if(!(i instanceof ShadowRoot))return;let a=i.host;if(typeof a.requestUpdate=="function"){a.requestUpdate();return}n=a}}function Ho(e){return tn.has(e)}function et(e,n,t,i,a,r={}){let o=e.hass.states,s=tn.get(a),l=s?Vh(Oh(o,r.domain,Od(e.hass)),s.query,sc,r.preferNumeric?Dh:void 0):[],d=s?Math.max(0,Math.min(s.index,l.length-1)):0,c=t.entityId?o[t.entityId]:void 0,u=(k,C,O=0)=>{tn.set(a,{query:C,index:O}),ke(k)},p=k=>{tn.delete(a),ke(k)},m=k=>{let C=Uh(k,t,o);C&&i(C)},g=(k,C)=>{i(La(o,k.entityId)),p(C)},y=()=>Math.max(0,Math.min(tn.get(a)?.index??0,l.length-1)),v=k=>{let C=k.target;if(k.key==="ArrowDown"||k.key==="ArrowUp"){k.preventDefault();let O=tn.get(a);if(!O){u(C,C.value);return}let N=k.key==="ArrowDown"?y()+1:y()-1;u(C,O.query,Math.max(0,Math.min(l.length-1,N))),Gh(C);return}if(k.key==="Enter"){k.preventDefault();let O=l[y()];s&&O?g(O,C):(m(C.value),p(C));return}if(k.key==="Escape"){if(!s)return;k.preventDefault(),k.stopPropagation(),p(C)}},$=t.entityId?Od(e.hass)?.(t.entityId):void 0,S=t.entityId===""?h`<div class="hint">Type part of a name, a room, or an id.</div>`:c?h`<div class="entity-current">
    ...
    ${s?h`<div class="entity-results" role="listbox">
          ${l.length===0?h`<div class="hint keep" style="padding:6px 8px">${lc(s.query)?"Nothing here has that id. Press Enter to use it anyway.":"Nothing matches that search."}</div>`:l.map((k,C)=>h`<button type="button" role="option" aria-selected=${C===d?"true":"false"} class="ent ${C===d?"hl":""}"
                ...
                <span class="ent-right">
                  <span class="ent-type">${Nd(k.domain)}</span>
                  <span class="ent-state">${k.state}</span>
                </span>
              </button>`)}
        </div>`:S}
  </div>`}function Gh(e){requestAnimationFrame(()=>{e.closest(".entity-field")?.querySelector("button.ent.hl")?.scrollIntoView({block:"nearest"})})}var Dd=120;function Kh(e,n,t,i){let a=r=>i.size===0?[...r]:r.filter(o=>i.has(o));return e!==""?{names:a(la.find(r=>r.name===e)?.symbols??[]),fromPack:!1}:n.trim()!==""&&t.length>0?{names:[...t],fromPack:!0}:{names:a(da),fromPack:!1}}function Vd(e,n){return n.size===0?e.length:e.filter(t=>n.has(t)).length}function Wh(e){return[{value:"",label:`Starter set (${Vd(da,e)})`},...la.map(n=>({value:n.name,label:`${n.name} (${Vd(n.symbols,e)})`}))]}function jh(e){return e.length>0?e.length:da.length}function Bd(e,n,t,i){return t?n>e?`Showing ${e} of ${n}. Type more to narrow it down.`:n===1?"1 symbol matches.":`${n} symbols match.`:i===1?"1 symbol available.":`${i} symbols available.`}function fa(e,n,t,i){let a=e.icons.render(n,22,"#FFFFFF");return h`<button type="button" class="sym ${t?"on":""}" title=${n} @click=${()=>i(n)}>
    <span class="sym-glyph">${a??h`<span class="sym-none">?</span>`}</span>
    <span class="sym-name">${n}</span>
  </button>`}function dc(e,n,t,i,a){let r=e.symbols,o=r.isOpen(i),s=r.query(i),l=e.icons.names(),d=l??[],c=new Set(d),u=n.trim(),p=u.startsWith(Fe),m=a!==void 0,g=m?r.pack(i)??(p?"mdi":"sf"):"sf",y=qh(u,c),v=k=>{t(k),a?.(k.startsWith(Fe)?e.icons.mdiPath?.(k):void 0),r.noteUsed(k)},$=k=>{if(t(k),!m)return;let C=k.trim();a?.(C.startsWith(Fe)?e.icons.mdiPath?.(C):void 0)},S=f;if(o&&g==="mdi"){let k=e.icons.mdiNames?.(),C=Yh(k??[],s),O=C.slice(0,Dd),N=r.recent.filter(U=>U.startsWith(Fe));S=h`<div class="sym-browse">
      ${Ud(r,i,g)}
      ...
      ${k===void 0?h`<div class="hint keep">Loading the Material Design catalogue.</div>`:C.length===0?h`<div class="hint keep">Nothing matches that search. Any<code>mdi:</code> name can still be typed above.</div>`:h`<div class="hint keep">${Bd(O.length,C.length,!0,k.length)}</div>`}
    ...
    </div>`}else if(o){let k=r.category(i),C=Kh(k,s,d,c),O=no(C.names,s),N=C.fromPack?O.slice(0,Dd):O,U=r.recent.filter(T=>!T.startsWith(Fe)),te=c.size===0?U:U.filter(T=>c.has(T));S=h`<div class="sym-browse">
      ${m?Ud(r,i,g):f}
      <div class="sym-controls">
        <input type="search" placeholder="Search symbols" .value=${s} @input=${he(T=>r.setQuery(i,T))} />
        <select @change=${he(T=>r.setCategory(i,T))}>
          ${Wh(c).map(T=>h`<option value=${T.value} ?selected=${T.value===k}>${T.label}</option>`)}
        </select>
      </div>
      ...
      ${O.length===0?h`<div class="hint keep">Nothing matches that search. Anyname can still be typed above.</div>`:h`<div class="hint keep">
            ${Bd(N.length,O.length,s.trim()!=="",jh(d))}
          </div>`}
    ...
    ${S}`}function qh(e,n){let t=e.trim();return t!==""&&!t.startsWith(Fe)&&n.size>0&&!n.has(t)}function Yh(e,n){let t=n.trim(),i=t.startsWith(Fe)?t.slice(Fe.length):t,a=e.map(r=>r.startsWith(Fe)?r.slice(Fe.length):r);return no(a,i).map(r=>Fe+r)}function Ud(e,n,t){return h`<div class="seg wide" role="radiogroup" aria-label="Icon set">
  ...
  </div>`}var Jh=[["literal","Fixed text"],["entityState","Entity state"],["entityAttribute","Entity attribute"],["entityAge","Entity age (seconds)"],["aggregate","Aggregate"],["time","Time"],["dataAge","Data age (seconds)"],["jinja","Jinja template"],["named","Named value"],["chartStat","A chart's number"]],Xh=[["bars","Bars"],["line","Line"],["area","Area"]],Zh=[["auto","Auto"],["fixed","Fixed range"]],Qh=[["lowest","Lowest value"],["zero","Zero"]],Lo=[["none","None"],["highest","Highest"],["lowest","Lowest"],["both","Both"]],Gd=[["none","None"],["dot","Dot"],["triangle","Triangle"]],To=[["uniform","One colour"],["bands","By value"]];function Sa(e){let n=[dr,"#FFD60A"];if(e.length<2)return n.map((o,s)=>({id:Y(),upTo:(s+1)*33,colorHex:o}));let t=Math.min(...e),a=Math.max(...e)-t,r=o=>Number(o.toFixed(a>=10?0:2));return n.map((o,s)=>({id:Y(),upTo:r(t+a*(s+1)/3),colorHex:o}))}function em(e){if(e.length===0)return 0;let n=Math.min(...e),t=Math.max(...e),i=t-n;return Number(((n+t)/2).toFixed(i>=10?0:2))}function tm(e,n){let t=Ct({bands:e}),i=t.at(-1),a=e.length>1?Math.abs(t[1].upTo-t[0].upTo):10;return{id:Y(),upTo:(i?.upTo??0)+(a||10),colorHex:n}}function nm(e,n){let t=[...e].sort((l,d)=>l-d),i=t[0],a=t.at(-1),r=n!==void 0&&Number.isFinite(n)?n:void 0,o=(r??0)-1,s=(r??0)+1;if(i!==void 0&&a!==void 0){let l=(t.length>1?(a-i)/(t.length-1):Math.abs(i)/2)||1;o=i-l,s=a+l}return r!==void 0&&(o=Math.min(o,r),s=Math.max(s,r)),{lo:o,hi:s}}function im(e,n,t){let{lo:i,hi:a}=nm(e.map(l=>l.upTo),t),r=l=>Math.max(0,Math.min(100,(l-i)/(a-i)*100)),o=i,s=e.map(l=>{let d=Math.max(0,r(l.upTo)-r(o));return o=Math.max(o,l.upTo),h`<i style=${`width:${d}%;background:${l.colorHex}`}></i>`});return h`<div class="band-bar">
    ...
    ${le("Otherwise",e.otherColorHex,o=>n(s=>{s.otherColorHex=o??St},"tother"),!1,St)}`}var om=[["arc","Arc"],["ring","Ring"],["bar","Bar"],["dots","Dots"]],sm={arc:"A 270\xB0 arc, open at the bottom",ring:"A full circle",bar:"A straight bar",dots:"One dot per unit, the first few filled"};function Kd(e){let n=e.value.kind;if(n.kind==="aggregate"){let{stateFilter:t,...i}=n.aggregate;return{kind:{kind:"aggregate",aggregate:{...i,function:"count"}}}}return H(String(Math.max(1,Math.round(e.maxValue-e.minValue))))}var lm=[["now","Now (HH:mm)"],["hour","Hour"],["minute","Minute"],["weekday","Weekday"],["day","Day"],["month","Month"],["timestamp","Unix timestamp"]];function dm(e,n){let t="entityId"in e?{entityId:e.entityId,displayName:e.displayName,domain:e.domain}:{entityId:"",displayName:"",domain:""};switch(n){case"literal":return{kind:n,value:e.kind==="literal"?e.value:""};case"entityState":return{kind:n,...t};case"entityAttribute":return{kind:n,...t,attribute:""};case"entityAge":return{kind:n,...t};case"aggregate":return{kind:n,aggregate:{function:"count",scope:{kind:"filter",domains:[],areaIds:[],labelIds:[],floorIds:[]},stateFilter:{kind:"isOn"}}};case"time":return{kind:n,timeField:"now"};case"dataAge":return{kind:n};case"jinja":return{kind:n,value:e.kind==="jinja"?e.value:"{{ states('sensor.example') }}"};case"named":return{kind:n,id:""};case"chartStat":return{kind:n,layer:"",stat:"latest"}}}function se(e,n,t,i){if(i.inline||!cm())return h`<div class="value-editor">${hc(e,n,t,i)}</div>`;let a=_a(i.key),r=i.label??"Value",o=i.showResolved?e.resolve(n):void 0,s=$e(n,pe(e)),l="entityId"in n.kind;return h`<div class="field value-chip-field ${i.compact?"compact":""}">
    ...
    ${cc(e,a,r,n,t,i)}
  </div>`}function cc(e,n,t,i,a,r){return h`<div class="value-pop" id=${n} popover role="dialog" aria-label=${t} @toggle=${pc}>
    <div class="pop-head">
      <b>${t}</b>
      <span class="spacer"></span>
      <button type="button" class="small" popovertarget=${n} popovertargetaction="hide">Done</button>
    </div>
    ${vi.has(n)?hc(e,i,a,r):f}
  </div>`}function pe(e){return{values:e.config.values,hass:e.hass,elements:e.config.elements}}function _a(e){return`wa-pop-${e.replace(/[^a-zA-Z0-9_-]/g,"")}`}function cm(){return typeof HTMLElement<"u"&&typeof HTMLElement.prototype.showPopover=="function"}var vi=new Set,gi=new WeakMap;function um(e){let n=e.getRootNode();return(n instanceof ShadowRoot||n instanceof Document?n:e.ownerDocument).querySelector(`[popovertarget="${e.id}"]`)}function uc(e,n,t=!1){let i=e instanceof Node?e:null;if(!i)return;let a=i.getRootNode();!(a instanceof ShadowRoot)&&!(a instanceof Document)||requestAnimationFrame(()=>requestAnimationFrame(()=>{let r=a.querySelector(`#${CSS.escape(n)}`);r&&typeof r.showPopover=="function"&&!r.matches(":popover-open")&&r.showPopover(),r&&t&&requestAnimationFrame(()=>requestAnimationFrame(()=>{r.querySelector("textarea, input[type=text], input[type=search], input:not([type])")?.focus()}))}))}function pc(e){let n=e.currentTarget,t=e.newState==="open",i=gi.get(n);if(i&&(i(),gi.delete(n)),!t){vi.delete(n.id)&&ke(n);return}let a=um(n);if(!a)return;let r=()=>{if(!n.isConnected||!n.matches(":popover-open")){gi.get(n)?.(),gi.delete(n);return}let o=a.getBoundingClientRect();if(o.bottom<0||o.top>window.innerHeight){n.hidePopover();return}bo(n,o)};window.addEventListener("scroll",r,!0),window.addEventListener("resize",r),gi.set(n,()=>{window.removeEventListener("scroll",r,!0),window.removeEventListener("resize",r)}),bo(n,a.getBoundingClientRect()),vi.has(n.id)||(vi.add(n.id),ke(n),requestAnimationFrame(()=>{n.isConnected&&bo(n,a.getBoundingClientRect())}))}function bo(e,n){e.style.maxHeight="";let t=e.getBoundingClientRect(),i=pm({left:n.left,top:n.top,bottom:n.bottom,width:n.width},{width:t.width,height:t.height},{width:window.innerWidth,height:window.innerHeight});e.style.left=`${i.left}px`,e.style.top=`${i.top}px`,e.style.maxHeight=`${i.maxHeight}px`}var en=8,ga=6,Wd=140;function pm(e,n,t){let i=t.height-e.bottom-ga-en,a=e.top-ga-en,r=n.height>i&&a>i&&i<Wd,o=Math.max(Wd,r?a:i),s=Math.min(n.height,o),l=Math.max(en,Math.min(e.left,t.width-n.width-en)),d=r?Math.max(en,e.top-ga-s):Math.max(en,Math.min(e.bottom+ga,t.height-s-en));return{left:l,top:d,maxHeight:o,above:r}}function hc(e,n,t,i){let a=n.kind,r=c=>t({...n,kind:c}),o=i.key,s=Jh.filter(([c])=>i.allowNamed!==!1||c!=="named"),l=f;switch(a.kind){case"literal":l=i.symbol?dc(e,a.value,c=>r({...a,value:c}),o,i.setSymbolPath):fe("Text",a.value,c=>r({...a,value:c}));break;case"entityState":case"entityAge":l=et(e,"Entity",a,c=>r({...a,...c}),`${o}-entity`);break;case"entityAttribute":{let c=Object.keys(e.hass.states[a.entityId]?.attributes??{}).sort(),u=`wa-attrs-${o.replace(/[^a-z0-9]/gi,"")}`;l=h`${et(e,"Entity",a,p=>r({...a,...p}),`${o}-entity`)}
        ${fe("Attribute",a.attribute,p=>r({...a,attribute:p}),{list:u,mono:!0})}
        <datalist id=${u}>${c.map(p=>h`<option value=${p}></option>`)}</datalist>`;break}case"aggregate":l=mm(e,a.aggregate,c=>r({...a,aggregate:c}),o);break;case"time":l=ze("Field",a.timeField,lm,c=>r({...a,timeField:c}));break;case"dataAge":l=h`<div class="hint">Seconds since the watch last fetched values.</div>`;break;case"jinja":l=h`${rc("Template",a.value,c=>r({...a,value:c}),4)}
        <div class="hint">Rendered by Home Assistant. The result should be one value, not a whole document.</div>`;break;case"named":l=e.config.values.length===0?h`<div class="hint warn">There are no named values yet. Add one in the Data card first.</div>`:ze("Value",a.id,[["","(choose)"],...e.config.values.map(c=>[c.id,c.name||c.id.slice(0,8)])],c=>r({...a,id:c}));break;case"chartStat":{let c=pe(e),u=e.config.elements.filter(p=>p.kind==="chart");l=u.length===0?h`<div class="hint warn">There is no chart layer yet. Add one first, then this can print one of its numbers.</div>`:h`
    ...
    ${ze("Source",a.kind,s,c=>r(dm(a,c)))}
    ${l}
    ${i.noFormat?f:hm(n.format,c=>t(Me(c)?{kind:n.kind}:{...n,format:c}))}
    ${i.showResolved?h`<div class="hint keep">Now:${d===void 0?h`<span class="warn">unresolved</span>`:h`<code>${d}</code>`}</div>`:f}`}function hm(e,n){let t=e??{},i=a=>{let r={...t,...a};for(let o of Object.keys(r))(r[o]===void 0||r[o]===!1||r[o]==="")&&delete r[o];n(r)};return h`<details class="sub" ?open=${!Me(e)}>
    ...
    ${Oe("Add unit",!!t.useEntityUnit,a=>i({useEntityUnit:a}))}
  ...
  </details>`}function mm(e,n,t,i){let a=s=>s.join(", "),r=s=>s.split(",").map(l=>l.trim()).filter(Boolean),o=n.scope;return h`
    ...
    ${n.function==="count"?f:fe("Attribute (blank = state)",n.attribute??"",s=>{let l={...n};s?l.attribute=s:delete l.attribute,t(l)})}`}var mc=Sr,fm=mc.filter(([e])=>e!=="none");function gm(e){if("entityId"in e)return{entityId:e.entityId,displayName:e.displayName,domain:e.domain};if(e.type==="callService")return e.target}function fc(e,n){let t=gm(n)??{entityId:"",displayName:"",domain:""};if(e==="callService"){let i=n.type==="callService"?{...n}:{type:"callService",serviceDomain:"",serviceName:""};return t.entityId!==""&&(i.target=t),i}return el(e)?{type:e,...t}:{type:e}}var ym=[["Open a cover","cover","open_cover",""],["Close a cover","cover","close_cover",""],["Stop a cover","cover","stop_cover",""],["Lock","lock","lock",""],["Unlock","lock","unlock",""],["Light brightness","light","turn_on",'{"brightness_pct": 50}'],["Climate target","climate","set_temperature",'{"temperature": 21}'],["Play / pause","media_player","media_play_pause",""],["Start a vacuum","vacuum","start",""],["Send a vacuum home","vacuum","return_to_base",""]];function gc(e,n,t,i){let a=n.serviceDataJSON??"",r=tl(a),o=n.target??{entityId:"",displayName:"",domain:""};return h`
    <div class="gen-row">
    ...
    <div class="gen-row">
      ...
      <div class="field flash-cell"><span title="Flash when a tap works">Flash</span>
        <div class="flash-row">
          ...
          ${o?h`<input type="color" class="flash-color" title="Flash colour. Click to change it." .value=${(n.successFlashColorHex??xm).slice(0,7)}
    ...
    ${t.type==="callService"?gc(e,t,(s,l)=>e.update(d=>{d.tapAction=s},l),"general-tap"):f}
    ${t.type==="openPage"?vm(e):f}`}var xm="#808080",jd=[0,15,30,60,120];function qd(e){return e===0?"None":e%60===0?e===60?"Every hour":`Every ${e/60} hours`:e===1?"Every minute":`Every ${e} minutes`}function vm(e){let n=e.config;return bc(e,n.openPageId,n.openPageName,(t,i)=>e.update(a=>{if(t===void 0){delete a.openPageId,delete a.openPageName;return}a.openPageId=t,i?a.openPageName=i:delete a.openPageName}))}function bc(e,n,t,i){let a=n??"",r=e.pages.map(o=>[o.id,o.name||"Unnamed page"]);return a&&!e.pages.some(o=>o.id.toUpperCase()===a.toUpperCase())&&r.unshift([a,`${t||"Unknown page"} (not on the watch)`]),a||r.unshift(["","Choose a page\u2026"]),r.length<=1&&!a?h`<div class="hint keep">No pages reported yet. Open the watch app once so it can send its page list.</div>`:h`${ze("Page",a,r,o=>{if(!o){i(void 0,void 0);return}i(o,e.pages.find(s=>s.id===o)?.name)})}
  ${a?f:h`<div class="hint keep">Without a page the tap falls back to the complication list.</div>`}`}function xc(e,n){let t=e.config.values.findIndex(a=>a.id===n.id),i=`nv-${n.id}`;return h`
  ...
  </section>`}function Fm(e){if(e.length===0)return"nothing";let n=t=>Number.isInteger(t)?String(t):String(Math.round(t*100)/100);return e.length<=12?e.map(n).join(" "):`${e.slice(0,6).map(n).join(" ")} \u2026 ${e.slice(-3).map(n).join(" ")}`}function Rm(e){if(e<60)return`${Math.max(0,Math.round(e))}s`;let n=Math.round(e/60);if(n<90)return`${n}m`;let t=Math.floor(n/60),i=n%60;return i===0?`${t}h`:`${t}h ${i}m`}function Im(e,n){let t=xe[e==="inline"?"rectangular":e],i=n.height*t.height>n.width*t.width,a=Math.round((n.rotationDegrees%180+180)%180)===90;return i!==a}function Am(e,n,t){let i=Im(e,n),a=xe[e==="inline"?"rectangular":e],r=n.height*a.height>n.width*a.width;return h`<div class="grid2">
  ...
  <div class="hint">Direction sets the frame's rotation. A line runs along the frame's long side, so
    ...
    <span class="unit" aria-hidden="true">%</span>
  </label>`}function Pm(e,n,t){let i=n.payload.id,a=`el-${i}`,r=Te(e.config,t,n),o=r.frame,s=(d,c)=>e.update(u=>Se(u,t,i,{frame:co(o,d)}),`${a}-${c}-${t}`),l=!Ea(o,Zn)||r.isHidden;return ge(e,"placement","Position",h`
    ...
    ${Oe("Hidden",r.isHidden,d=>e.update(c=>Se(c,t,i,{isHidden:d})),!1)}
    <div class="hint">X, Y, W and H are a percent of the face, on the ${ee(t)} shape only. Drag a letter left or right to change its number. Arrow keys nudge 1 pt, shift for 10.</div>`,{color:J.position,icon:"place",summary:`${Math.round(o.width*100)}% wide \xB7 ${ee(t)}`,...l?{resetTitle:`Put this layer back to the middle of the ${ee(t)} face at half size, unrotated and shown`,reset:()=>e.update(d=>Se(d,t,i,{frame:{...Zn},isHidden:!1}))}:{}})}function zm(e,n){if(n.kind==="tap")return f;let t=n.payload.id,i=_e(e.config,t)[0];return ge(e,"tappable","Tap",df(e,n,`el-${t}`),{color:J.tap,icon:"tap",summary:i?ot(i.payload.action):"Not tappable",...i?{reset:()=>e.update(a=>Gi(a,t))}:{}})}function Zd(e,n,t,i,a){return h`
    ...
    ${e.timeLabelCount<=0?f:h`
      ...
      ${X("Clock",e.hourCycle,Ys,r=>n(o=>{o.hourCycle=r}),{titles:{auto:"Whatever clock the watch is set to"},def:t.hourCycle})}
      ${X("Minutes",e.minutes,Js,r=>n(o=>{o.minutes=r}),{titles:{auto:"Kept up to a three hour span, dropped past it"},def:t.minutes})}
      ${a}`}`}var Nm=[["number","Number"],["entity","Entity"]];function Om(e,n){return(n==="min"?e.minSource:e.maxSource)===void 0?"number":"entity"}function Dm(e,n,t){let i=n==="min"?"minSource":"maxSource";t==="number"?delete e[i]:e[i]===void 0&&(e[i]={kind:{kind:"entityState",entityId:"",displayName:"",domain:""}})}function Vm(e,n,t,i,a){let r=o=>{let s=o==="min",l=s?"Min":"Max",d=Om(n,o),c=s?n.minValue:n.maxValue,u=y=>a(v=>{s?v.minValue=y??0:v.maxValue=y??100},o),p={number:`${l} is a fixed number`,entity:`${l} reads a number from an entity`},m=h`<div class="gauge-end-head">
      ${Ie(l,d==="number"?Rt(c,t[o],u):void 0)}
      <span class="seg" role="radiogroup" aria-label=${`${l} comes from`}>
        ${Nm.map(([y,v])=>h`<button type="button" role="radio" aria-checked=${y===d?"true":"false"}
          class=${y===d?"on":""} title=${p[y]}
          @click=${()=>{y!==d&&a($=>Dm($,o,y))}}>${v}</button>`)}
      </span>
    ...
    ${r==="none"?f:h`
      ...
      ${a==="bands"?f:h`<div class="hint">${l}, and other text keeps the layer colour.</div>`}`}`}var bi=new Map,yi=new Map,ht=new Map,xa=new Map,ko=4,$o=40,Um=[["layer","Layer"],["pick","Pick"],["bands","By value"]],Gm=[["plain","Plain"],["rich","Rich"]],Km={plain:"One line: typed words, a live value or a template",rich:"Parts, each with its own colour, weight and size"};function Po(e,n,t,i){let a=t!==void 0&&e.canCountDown(t);return!n&&!a?f:h`${Oe("Count down",n,i)}
    ...
    ${n&&!a?h`<div class="hint warn">This value is not a timer or a future time, so nothing counts down. The watch shows it as plain text.</div>`:f}`}function Wm(e){return e.countdown===!0?"countdown":at(e)?"rich":"plain"}function jm(e){return(e.match(/ +|[^ ]+/g)??[]).map(n=>({text:n,space:n.startsWith(" ")}))}function qm(e,n){let t=un(e);return t!==void 0?{kind:"text",label:t}:e.kind.kind==="jinja"?{kind:"template",label:Ue(e.kind.value,40)||"template"}:{kind:"value",label:Na(e,n)}}function Cc(e,n,t){let i=un(e.value),a=i===void 0?Ue(Na(e.value,t),28):i.trim()===""?i===""?"empty":"spaces":`"${Ue(i,24)}"`;return`Part ${n+1}: ${a}`}function Ym(e,n,t){let i=[["","Whole text"],...e.map((a,r)=>[a.id,Cc(a,r,t)])];return n!==void 0&&!e.some(a=>a.id===n)&&i.push([n,"A part that is gone"]),i}function Mo(e){return e.coloring==="bands"?"bands":e.colorHex===void 0?"layer":"pick"}function Jm(e,n){if(Mo(e)==="bands"&&(e.bands?.length??0)>0){let t=[...Ct({bands:e.bands}).map(r=>r.colorHex),e.bandAboveColorHex??ie],i=100/t.length,a=r=>`${Math.round(r*10)/10}%`;return`conic-gradient(${t.map((r,o)=>`${r} ${a(o*i)} ${a((o+1)*i)}`).join(", ")})`}return e.colorHex??n}function Qd(e){let n=r=>r.length===1?`Part ${r[0].index+1}`:`Parts ${za(r.map(o=>String(o.index+1)))}`,t=e.filter(r=>r.reason==="kind"),i=e.filter(r=>r.reason==="format"),a=[];return t.length>0&&a.push(`${n(t)} ${t.length===1?"shows":"show"} a value a template cannot read, such as data age or a chart's number.`),i.length>0&&a.push(`${n(i)} ${i.length===1?"uses":"use"} a relative time or duration format, which a template cannot print.`),`Rich text stays on, because the parts cannot join into one line. ${a.join(" ")} Change or remove ${e.length===1?"that part":"those parts"} first.`}var Xm={fontSize:"font size",fontWeight:"weight",color:"colour",bands:"colour bands"};function Zm(e){return e.joined?e.template?"Rich text is off. The parts joined into one template, so the live values still update.":"Rich text is off. The parts joined into one line of text.":e.moved.length===0?"Rich text is off.":`Rich text is off. The part's ${za(e.moved.map(n=>Xm[n]))} moved into Look.`}function Qm(e,n,t,i,a){let r=n.payload,o=r.id,s=Wm(r),l=(r.parts?.length??0)>0,d=Tr(e.config,r.value),c=ht.get(o),u=c&&c.rich===l?c:void 0,p=s==="rich"&&(r.parts?.length??0)>=2?yi.get(o):void 0,m=(v,$)=>{let S=!(r.parts??[]).every(C=>C.value.kind.kind==="literal"),k=ma(structuredClone(r),e.config.values);if(yi.delete(o),!k.ok){ht.set(o,{text:Qd(k.blocked),rich:!0,warn:!0}),ke($);return}ht.set(o,{text:Zm(k.joined?{joined:!0,template:S}:k),rich:!1}),i(C=>{ma(C,e.config.values),v==="countdown"&&(C.countdown=!0)})},g=(v,$)=>{if(yi.delete(o),s==="rich"){let S=v==="countdown"?"countdown":"plain",k=r.parts??[];if(k.length<2){m(S,$);return}let C=fo(k,e.config.values);C.ok?(ht.delete(o),yi.set(o,S)):ht.set(o,{text:Qd(C.blocked),rich:!0,warn:!0}),ke($);return}if(v==="rich"){let S=r.parts?.[0]?.id??Y();bi.set(o,S),ht.delete(o),i(k=>{delete k.countdown,zd(k,S)});return}ht.delete(o),i(S=>{if(v==="countdown"){S.countdown=!0;return}(S.parts?.length??0)>0&&ma(S,e.config.values),delete S.countdown})},y=p==="countdown"?"Switch to Countdown?":"Switch to Plain?";return h`
    ${X("Type",s==="rich"?"rich":"plain",Gm,g,{titles:Km})}
    ...
    ${s==="rich"?ef(e,n,t,i,a):h`
        ${$c(e,n,a)}
        ${se(e,r.value,v=>i($=>{$.value=v},"value"),{showResolved:!0,label:s==="countdown"?"Until":"Text",key:`${a}-value`})}
        ${Po(e,s==="countdown",r.value,v=>g(v?"countdown":"plain",null))}
        ${d?h`<div class="hint keep">Prints a number from the chart <button type="button" class="link" @click=${()=>e.selectLayer(d.payload.id)}>${Ae(d,pe(e))}</button>. It stays in the chart's group and moves with it.</div>`:f}`}`}function ef(e,n,t,i,a){let r=n.payload,o=r.parts??[],s=r.id,l=pe(e),d=Math.max(0,o.findIndex(R=>R.id===bi.get(s))),c=o[d],u=o.length,p=r.colorSlot.baseColorHex,m=Te(e.config,t,n).size??r.fontSize,g=(R,b)=>{ht.delete(s),i(x=>{R(x),Bs(x)},b)},y=(R,b)=>g(x=>{let M=x.parts?.find(E=>E.id===c.id);M&&R(M)},b?`part-${c.id}-${b}`:void 0),v=(R,b)=>{bi.set(s,R),ht.delete(s),ke(b)},$=(R,b)=>{let x=Y();bi.set(s,x),g(M=>{(M.parts??=[]).push({id:x,value:R})}),uc(b,_a(`${a}-part-${x}`),!0)},S=R=>g(b=>{b.parts&&wi(b.parts,d,R)}),k=()=>{let R=o[d+1]??o[d-1];R&&bi.set(s,R.id),g(b=>{b.parts=(b.parts??[]).filter(x=>x.id!==c.id)})},C=o.map((R,b)=>{let x=qm(R.value,l),M=R.id===c.id,E=Mo(R),P=x.kind==="value"?e.resolve(R.value):void 0,B=R.fontWeight===void 0?void 0:Mn.find(([K])=>K===R.fontWeight)?.[1];return h`<button type="button" role="option" aria-selected=${M?"true":"false"} class="part-chip ${x.kind} ${M?"on":""}"
      aria-label=${Cc(R,b,l)} @click=${K=>v(R.id,K.currentTarget)}>
      <span class="part-dot" style=${`background:${Jm(R,p)}`}
        title=${E==="bands"?"By value, with its own bands":E==="pick"?"Its own colour":"The layer colour"}></span>
      ${x.kind==="text"?h`<span class="part-txt">${x.label===""?h`<span class="part-empty">empty</span>`:jm(x.label).map(K=>K.space?h`<span class="part-sp">${"\xB7".repeat(K.text.length)}</span>`:K.text)}</span>`:h`<span class="part-txt">${x.label}</span>`}
    ...
    <div class="part-editor">
      ...
      ${X("Colour",U,Um,R=>y(b=>{if(R==="layer"){delete b.colorHex,delete b.coloring;return}if(R==="pick"){delete b.coloring,b.colorHex=Ao(p,"#FFFFFF")?"#64D2FF":p;return}delete b.colorHex,b.coloring="bands",(b.bands?.length??0)===0&&(b.bands=Sa(Je(e.resolve(b.value)??"")))}),{def:"layer",titles:W,...N&&U!=="bands"?{disabled:{bands:!0}}:{}})}
      ...
      <label class="field num part-size">${Ie("Font size",te?{atDefault:!1,title:`Back to the layer size (${m} pt)`,reset:()=>y(R=>{delete R.fontSize})}:void 0,Aa(c.fontSize??m,V,{step:1,min:ko,max:$o}))}
  ...
  </div>`}function Sc(e,n,t,i={}){let a=n.payload.id,r=e.config.elements.findIndex(b=>b.payload.id===a),o=`el-${a}`,s=(b,x)=>e.update(M=>b(M.elements[r]),x?`${o}-${x}`:void 0),l=Te(e.config,t,n),d=l.frame,c=(b,x)=>e.update(M=>Se(M,t,a,{frame:co(d,b)}),`${o}-${x}-${t}`),u=We(n.kind).payload,p=u.colorSlot?.baseColorHex??"#FFFFFF",m=b=>u[b],g=!1,y=b=>n.kind==="image"||n.kind==="tap"||n.kind==="timeline"?f:le(b,n.payload.colorSlot.baseColorHex,x=>s(M=>{M.kind!=="image"&&M.kind!=="tap"&&M.kind!=="timeline"&&(M.payload.colorSlot.baseColorHex=x??"#FFFFFF")},"color"),!1,p),v,$;switch(n.kind){case"text":{let b=(x,M)=>s(E=>x(E.payload),M);v=Qm(e,n,t,b,o),g=!n.payload.countdown&&!at(n.payload),$=h`
        ...
        ${Nh({label:"Align",value:n.payload.alignment??"center",options:$m,def:"center",set:x=>s(M=>{let E=M.payload;x==="center"?delete E.alignment:E.alignment=x})},{label:"Lines",value:n.payload.lineLimit===2?"2":"1",options:Cm,def:"1",set:x=>s(M=>{let E=M.payload;x==="2"?E.lineLimit=2:delete E.lineLimit})})}
        ${Oe("Mono digits",n.payload.monospacedDigits===!0,x=>s(M=>{let E=M.payload;x?E.monospacedDigits=!0:delete E.monospacedDigits}),u.monospacedDigits===!0)}
        ...
        ${g?Bm(e,n.payload,b,y("Main colour")):f}`;break}case"icon":v=h`
        ...
        ${M?h`
            ${se(e,b.total??Kd(b),E=>x(P=>{P.total=E},"total"),{showResolved:!0,label:"Total",key:`${o}-total`})}
            <div class="hint">How many dots to draw. Left as it is, a count of the same
              ...
              total over one scope. At most ${Li} dots are drawn.</div>`:Vm(e,b,{min:u.minValue,max:u.maxValue},o,x)}`,g=!0,$=h`
        <div class="grid2">
          ${X("Style",b.style,om,E=>x(P=>{E==="dots"&&P.total===void 0&&(P.total=Kd(P)),E!=="dots"&&delete P.total,P.style=E}),{titles:sm,def:u.style})}
        ...
        ${M?f:h`
          ...
          <div class="hint">A short tick on the scale at that value, so the fill reads
            ...
            nothing. Leave it empty for no mark.</div>`}`;break}case"chart":{let b=n.payload,x=(F,_)=>s(on=>F(on.payload),_),M=wt(b),E=sr(u.marker),P=u.historyMinutes,B=u.historyPoints,K=b.historyMinutes>0,ve=K&&b.source==="statistics",_t=K&&!ve,In=K?ve?"statistics":"history":"value",An=ve?ei:Ut,we=Gt(b)??Kt(b),ce=b.value.kind.kind==="entityState",tt=we===void 0?void 0:e.historySeries(we),cu=K&&ce?tt??"":e.resolve(b.value)??"",nt=b.historyPoints<1,Jo=Eo(a,b.historyMinutes,An),Ge=Je(cu),Pt=b.limit>0&&Ge.length>b.limit?b.takeFromEnd?Ge.slice(Ge.length-b.limit):Ge.slice(0,b.limit):Ge,uu=!K&&ce&&Ge.length===1,Va=e.config.elements.filter(F=>F.kind==="chart"&&F.payload.id!==a),pu=pe(e),Ba=b.scaleFrom!==void 0&&Va.some(F=>F.payload.id===b.scaleFrom);v=h`
        ...
        ${Ge.length>0?h`<div class="field readout"><span>Reads</span>
              <span class="readout-v"><span class="nums">${Fm(Pt)}</span>${Ge.length===Pt.length?h` · ${Pt.length} ${Pt.length===1?"value":"values"}`:h` · ${Pt.length} of ${Ge.length}`}</span></div>`:f}
        ${uu?h`<div class="hint warn">This entity holds one number, so the chart draws one bar.
        ...
        <div class="grid2">
          ${X("Style",b.style,Xh,F=>x(_=>{_.style=F}),{def:u.style})}
        ...
        <div class="grid2">
          ${X("Scale",b.scale,Zh,F=>x(_=>{_.scale=F}),{titles:{auto:"The plot stretches to fit the readings it has",fixed:"The plot always runs from Min to Max"},def:u.scale})}
          ${X("Baseline",b.baseline,Qh,F=>x(_=>{_.baseline=F}),{def:u.baseline})}
        </div>
        ${Va.length===0?f:ze("Same scale as",Ba?b.scaleFrom:"",[["","Its own"],...Va.map(F=>[F.payload.id,Ae(F,pu)])],F=>x(_=>{F?_.scaleFrom=F:delete _.scaleFrom}),{def:""})}
        ...
        <div class="field"><span>Series</span>
          <div class="row-acts">
            <button class="small" title="Add a second chart layer on this frame, drawn against this chart's range"
              @click=${()=>{let F;e.update(_=>{F=gl(_,a)}),F&&e.selectLayer(F)}}>${D("plus")}<span>Add a second series</span></button>
          </div>
        </div>
        ...
        ${b.coloring==="bands"?h`
          ...
          ${b.style==="area"?h`${Oe("Band fill",b.fillBands,F=>x(_=>{_.fillBands=F}),u.fillBands)}
        ...
        ${b.highlight==="none"?f:h`
          <div class="grid2">
            ${b.highlight==="lowest"?f:X("Highest marker",M.high,Gd,F=>x(_=>{lr(_,{...wt(_),high:F})}),{def:E.high})}
            ${b.highlight==="highest"?f:X("Lowest marker",M.low,Gd,F=>x(_=>{lr(_,{...wt(_),low:F})}),{def:E.low})}
          </div>
          <div class="grid2">
        ...
        ${Oe("Threshold",b.thresholdValue!==void 0,F=>x(_=>{F?_.thresholdValue=em(Pt):delete _.thresholdValue}))}
        ...
        ${Oe("Now marker",b.nowIndex!==void 0,F=>x(_=>{F?_.nowIndex={kind:{kind:"time",timeField:"hour"}}:delete _.nowIndex}))}
        ...
        ${Ni(b)?Zd(b,x,u,"cl",h`
            <div class="hint">Clock times from the start of the span to now, evenly spaced, in a row of
              ...
              Recorded history.</div>`}`;break}case"timeline":{let b=n.payload,x=(we,ce)=>s(tt=>we(tt.payload),ce),M=u.historyMinutes,E=b.value.kind.kind==="entityState",P=Et(b),B=P===void 0?void 0:e.historySeries(P),K=je(b)*60,ve=di(B??"",Tt),_t=Eo(a,b.historyMinutes),In=b.value.kind.kind==="entityState"?b.value.kind.entityId:void 0,An=am(ve,K,In===void 0?void 0:e.hass.states[In]?.state,In?.split(".")[0]);v=h`
        ${se(e,b.value,we=>x(ce=>{ce.value=we},"value"),{label:"States",key:`${o}-value`})}
        ...
        ${vo(a,b.historyMinutes,M,we=>x(ce=>{ce.historyMinutes=we}))}
        ${_t?wo(b.historyMinutes,we=>x(ce=>{ce.historyMinutes=we},"span")):f}
        ...
        ${ve.length>0?h`<div class="field readout"><span>Reads</span><span class="readout-v"><span class="nums">${Lm(ve,K)}</span></span></div>`:f}
        ${Hm(ve)?h`<div class="hint warn">This entity reports numbers, so every reading is its own
        ...
        ${rm(b,x,An,`wa-tl-states-${o.replace(/[^a-z0-9]/gi,"")}`)}
        ...
        <div class="grid2">
          ${oe("Gap",b.gap,we=>x(ce=>{ce.gap=Math.min(Oi,Math.max(0,we??0))},"tgap"),{step:.5,min:0,max:Oi,def:u.gap,unit:"pt"})}
          ${oe("Corner radius",b.cornerRadius,we=>x(ce=>{ce.cornerRadius=Math.max(0,we??0)},"tradius"),{step:.5,min:0,def:u.cornerRadius,unit:"pt"})}
        </div>
        ...
        ${n.payload.kind==="line"?Am(t,d,c):f}`,$=n.payload.kind==="line"?oe("Thickness",n.payload.thickness,b=>s(x=>{x.payload.thickness=b??1},"thick"),{step:.5,min:.5,def:u.thickness,unit:"pt"}):h`
        ...
        <div class=${b.contentMode==="fit"&&b.zoom===1?"hint keep":"hint"}>${Em(b)}</div>
        ...
        ${Tc(e,n.payload,(b,x)=>s(M=>b(M.payload),x),o)}
        ...
        <div class="hint">To make one layer tappable, tick Tappable on that layer instead.</div>`;break}}let S=g||n.kind==="image"||n.kind==="tap"||n.kind==="timeline"?void 0:y(n.kind==="shape"?"Fill colour":n.kind==="text"&&at(n.payload)?"Layer colour":"Colour"),k=Ir(e.config,n),C=k?{kind:{kind:"entityState",...k}}:void 0,O=n.kind==="image"?n.payload.timestamp===!0:!1,N=n.kind==="text"&&(n.payload.parts?.length??0)>0?n.payload.parts:void 0,U=nf[n.kind],te=af[n.kind],T=Xd(n.payload,u,U),V=n.kind==="text"?"fontSize":n.kind==="icon"?"size":n.kind==="gauge"||n.kind==="chart"?"lineWidth":void 0,Z=e.config.perFamily[t]?.placements[a]?.size!==void 0,W=Xd(n.payload,u,te)||V!==void 0&&l.size!==void 0&&l.size!==u[V],ye=Wt(e.config,a),R=(b,x)=>()=>s(M=>xo(M.payload,u,b),x);return h`
    ${ge(e,"content","Content",h`${n.kind==="tap"||n.kind==="text"?f:$c(e,n,o)}${v}`,{color:J.content,icon:"content",summary:_o(e,n),...T?{reset:()=>s(b=>{xo(b.payload,u,U),b.kind==="text"&&fi(b.payload.rules)},"reset-content")}:{}})}
    ...
    ${n.kind==="chart"?ge(e,"numbers","Numbers",lf(e,n),{color:J.numbers,icon:"text",summary:sf(e,n),...ye.length>0?{reset:()=>e.update(b=>{for(let x of Wt(b,a))ct(b,x.payload.id)})}:{}}):f}
    ${n.kind==="image"?ge(e,"timestamp","Timestamp",Mm(n.payload,(b,x)=>s(M=>b(M.payload),x)),{color:J.numbers,icon:"clock",summary:O?`Shown \xB7 ${n.payload.timestampSize} pt`:"Hidden",...O?{reset:R(tf,"reset-stamp")}:{}}):f}
    ${ge(e,"states","States",Hc(e,n.payload.rules,n.kind,b=>b.elements.find(x=>x.payload.id===a)?.payload.rules,`rules-${a}`,C,N),{color:J.states,icon:"states",summary:pi(n.payload.rules).replace(/\.$/,""),...n.payload.rules.length>0?{reset:()=>s(b=>{b.payload.rules=[]})}:{}})}
    ${i.placement===!1?f:Pm(e,n,t)}
    ${i.tap===!1?f:zm(e,n)}`}var tf=["timestamp","timestampCorner","timestampSize"],nf={text:["value","countdown","parts"],icon:["symbol","path"],gauge:["value","minValue","maxValue","total","minSource","maxSource"],chart:["value","historyMinutes","historyPoints","source","statPeriod","statType","limit","takeFromEnd"],timeline:["value","historyMinutes"],shape:["kind","cornerRadius"],image:["entity","source"],tap:["action","openPageName"]},af={text:["fontSize","fontWeight","colorSlot","alignment","lineLimit","monospacedDigits","coloring","bands","bandAboveColorHex","highlight","highColorHex","lowColorHex"],icon:["size","colorSlot"],gauge:["style","lineWidth","trackColorHex","colorSlot","coloring","bands","bandAboveColorHex","thresholdValue","thresholdColorHex"],chart:["style","scale","minValue","maxValue","baseline","barGap","lineWidth","highlight","highColorHex","lowColorHex","marker","highMarker","lowMarker","coloring","bands","bandAboveColorHex","fillBands","thresholdValue","thresholdColorHex","nowIndex","nowColorHex","scaleFrom","colorSlot","timeLabelCount","labelSize","labelColorHex","labelsAbove","hourCycle","minutes"],timeline:["bands","otherColorHex","gap","cornerRadius","labelSize","labelColorHex","labelsAbove","timeLabelCount","hourCycle","minutes"],shape:["colorSlot","borderColorHex","borderWidth","thickness"],image:["contentMode","zoom","panX","panY","cornerRadius"],tap:[]};function Tc(e,n,t,i){let a=n.action;return h`
    ${ze("Tap action",a.type,fm,r=>t(o=>{o.action=fc(r,o.action),r!=="openPage"&&(delete o.openPageId,delete o.openPageName)}))}
    ${"entityId"in a?et(e,"Target",a,r=>t(o=>{o.action={type:a.type,...r}},"tap-entity"),`${i}-tap`):f}
    ${a.type==="callService"?gc(e,a,(r,o)=>t(s=>{s.action=r},o),`${i}-tap`):f}
    ${a.type==="openPage"?bc(e,n.openPageId,n.openPageName,(r,o)=>t(s=>{if(r===void 0){delete s.openPageId,delete s.openPageName;return}s.openPageId=r,o?s.openPageName=o:delete s.openPageName},"tap-page")):f}`}var rf=24;function of(e,n){let t=[],i=1/0;for(let r of ne){if(r==="inline"||!e.config.supportedFamilies.includes(r))continue;let o=ml(e.config,n,r);o&&(t.push(`${ee(r)} ${Math.round(o.width)} x ${Math.round(o.height)} pt`),i=Math.min(i,o.width,o.height))}if(t.length===0)return f;let a=i<rf;return h`<div class="field readout"><span>Tap size</span><span class="readout-v">${t.join(" \xB7 ")}</span></div>
    ${a?h`<div class="hint warn">That is small for a wrist. Show the tap area and drag its corners out.</div>`:f}`}function sf(e,n){let t=Wt(e.config,n.payload.id);return t.length===0?"None yet":t.map(i=>{let a=i.payload.value.kind;return a.kind==="chartStat"?(cn.find(([r])=>r===a.stat)?.[1]??"number").toLowerCase():"number"}).join(" \xB7 ")}function lf(e,n){let t=pe(e),i=Wt(e.config,n.payload.id),a=o=>e.update(s=>{ll(s,n.payload.id,o)}),r=new Set(i.map(o=>o.payload.value.kind.kind==="chartStat"?o.payload.value.kind.stat:""));return h`
    ...
    ${a?h`<div class="value-editor">
        ...
        ${of(e,a.payload.id)}
        <div class="hint">The tap area follows this layer in every shape, so there is nothing to line up. Show it to drag its corners past the layer, so a small layer is still an easy target. Where two tap areas overlap, the one higher in Layers wins.</div>`:h`<div class="hint">Tapping this layer runs an action of its own, instead of the complication's tap action. It starts as <b>${ot(o)}</b>.</div>`}`}function ec(e){return e.length>=2&&e.startsWith('"')&&e.endsWith('"')?e.slice(1,-1):e}function Ae(e,n){switch(e.kind){case"text":return ec($e(e.payload.value,n));case"icon":return ec($e(e.payload.symbol,n));case"gauge":return $e(e.payload.value,n);case"chart":return $e(e.payload.value,n);case"timeline":return $e(e.payload.value,n);case"shape":return e.payload.kind==="roundedRectangle"?"Rounded rectangle":e.payload.kind;case"image":{let t=e.payload.entity;return t.displayName||t.entityId||(e.payload.source==="camera"?"camera":"picture")}case"tap":{let t=e.payload.action,i="entityId"in t?t.displayName||t.entityId:t.type==="callService"?[t.serviceDomain,t.serviceName].filter(a=>a!=="").join("."):t.type==="openPage"&&e.payload.openPageName||"";return i?`${t.type} \xB7 ${i}`:t.type}}}function Ec(e,n){let t=lt(e.config,n.id),i=pe(e),a=(r,o)=>e.update(s=>{let l=s.groups?.find(d=>d.id===n.id);l&&r(l)},o?`group-${n.id}-${o}`:void 0);return ge(e,"content","Group",h`
    ...
    <div class="hint">Click a layer in the list to edit it.</div>`,{color:J.group,icon:"folder",summary:`${t.length} layers \xB7 ${n.locked?"moves as one":"unlocked"}`})}function Mc(e,n){if(n==="inline")return cf(e);let t=e.config.perFamily[n];if(!t)return h`<div class="hint">No settings stored for ${ee(n)} yet.</div>
    ...
    ${ge(e,"content","Inline text",h`
      ...
      ${Po(e,n.countdown===!0,n.value,a=>t(r=>{a?r.countdown=!0:delete r.countdown}))}`,{color:J.content,icon:"text",summary:Ue(`${n.label?`${n.label}: `:""}${$e(n.value,i)}`,48)})}
    ${ge(e,"symbol","Symbol",h`
      ${dc(e,n.symbol??"",a=>t(r=>{a?r.symbol=a:delete r.symbol},"symbol"),"inline-symbol")}
      <div class="hint">Drawn before the text. Leave it blank for text only.</div>
      <div class="field readout"><span>On the face</span><span class="readout-v">${n.symbol?`${n.symbol} `:""}${n.label?`${n.label}: `:""}${e.resolve(n.value)??"--"}</span></div>`,{color:J.look,icon:"icon",summary:n.symbol||"None"})}`}function uf(e,n,t){let i=n.curvedText?"curved":"canvas",a=n.bezelGauge?"gauge":n.bezelText?"text":"none";return h`
    ...
    ${a==="text"&&n.bezelText?h`
      ...
      ${Po(e,n.bezelCountdown===!0,n.bezelText,r=>t(o=>{r?o.bezelCountdown=!0:delete o.bezelCountdown}))}`:f}
    ${a==="gauge"&&n.bezelGauge?pf(e,n.bezelGauge,t):f}`}function pf(e,n,t){let i=[n.colorHexes[0]??"#34C759",n.colorHexes[1]??n.colorHexes[n.colorHexes.length-1]??"#FFCC00",n.colorHexes[n.colorHexes.length-1]??"#FF3B30"],a=r=>o=>t(s=>{let l=[...i];l[r]=o??l[r],s.bezelGauge.colorHexes=l},`gstop${r}`);return h`
    ...
    ${Oe("End labels",!!(n.minLabel||n.maxLabel),r=>t(o=>{let s=o.bezelGauge;r?(s.minLabel=H(String(s.minValue)),s.maxLabel=H(String(s.maxValue))):(delete s.minLabel,delete s.maxLabel)}))}
    ${n.minLabel?se(e,n.minLabel,r=>t(o=>{o.bezelGauge.minLabel=r},"gminlab"),{label:"Min label",key:"fam-corner-gminlab"}):f}
    ${n.maxLabel?se(e,n.maxLabel,r=>t(o=>{o.bezelGauge.maxLabel=r},"gmaxlab"),{label:"Max label",key:"fam-corner-gmaxlab"}):f}`}var Mb=ne.map(e=>[e,ee(e)]),zo={setColor:"Set colour",setOpacity:"Set opacity",setText:"Set text",setIcon:"Set icon",setFontSize:"Set size",setFontWeight:"Set weight",setRotation:"Set rotation",hide:"Hide",show:"Show",setGaugeValue:"Set gauge value",setGaugeMin:"Set gauge min",setGaugeMax:"Set gauge max",setBorderColor:"Set border colour",setBorderWidth:"Set border width",setBackgroundColor:"Set background colour"},hf=Object.keys(zo),Fa=["color","text","fontSize","fontWeight","visibility"];function mf(e,n=!1){let t=ji[e].filter(i=>!n||Fa.includes(i));return hf.filter(i=>t.includes(Ee[i]))}function Fc(e,n,t,i){let a=n!==void 0&&!e.some(r=>r.id===n);return h`<label class="field"><span>Changes</span>
      <select @change=${r=>i(r.target.value,r.target)}>
        ${Ym(e,n,t).map(([r,o])=>h`<option value=${r} ?selected=${r===(n??"")}>${o}</option>`)}
      </select></label>
    ${a?h`<div class="hint warn">The part this changed has been removed, so it changes nothing. Pick another part or Whole text.</div>`:f}`}var ff={now:"the time",hour:"the hour",minute:"the minute",weekday:"the weekday",day:"the day",month:"the month",timestamp:"the timestamp"};function va(e,n){if(e.entityId==="")return"(no entity)";let t=e.displayName.trim();if(t!==""&&t!==e.entityId)return t;let i=n?.hass?.states[e.entityId]?.attributes.friendly_name;return typeof i=="string"&&i.trim()!==""?i.trim():e.entityId}function Ue(e,n){let t=e.replace(/\s+/g," ").trim();return t.length>n?`${t.slice(0,n-1)}\u2026`:t}function gf(e){if(!e||Me(e))return"";let n=[];return e.decimals!==void 0&&n.push(`${e.decimals} dp`),e.multiply!==void 0&&n.push(`\xD7${e.multiply}`),e.offset!==void 0&&n.push(`${e.offset<0?"":"+"}${e.offset}`),e.prefix&&n.push(`"${e.prefix}" first`),e.suffix&&n.push(`"${e.suffix}" after`),e.useEntityUnit&&n.push("with unit"),e.relativeTime&&n.push("as relative time"),e.duration&&n.push("as a duration"),e.textCase&&n.push(e.textCase==="capitalized"?"Capitalized":e.textCase==="upper"?"UPPER":"lower"),n.length===0?"":` (${n.join(", ")})`}function $e(e,n){return`${Na(e,n)}${gf(e.format)}`}function Na(e,n){let t=e.kind;switch(t.kind){case"literal":return t.value?`"${Ue(t.value,40)}"`:"(empty)";case"entityState":return va(t,n);case"entityAttribute":return t.attribute?`${va(t,n)} \xB7 ${t.attribute}`:va(t,n);case"entityAge":return`age of ${va(t,n)}`;case"aggregate":return yf(t.aggregate);case"time":return ff[t.timeField];case"dataAge":return"data age";case"jinja":return t.value?`template ${Ue(t.value,32)}`:"template (empty)";case"named":return t.id===""?"(no value chosen)":n?.values?.find(a=>a.id===t.id)?.name?.trim()||`named ${t.id.slice(0,8)}`;case"chartStat":{let i=(cn.find(([o])=>o===t.stat)?.[1]??t.stat).toLowerCase();if(t.layer==="")return`${i} (no chart chosen)`;let a=n?.elements?.find(o=>o.kind==="chart"&&o.payload.id===t.layer),r=a?.kind==="chart"&&a.payload.value.kind.kind!=="chartStat"?Na(a.payload.value,n):"a missing chart";return`${i} of ${r}`}}}function yf(e){let n=e.scope.kind==="entities"?`${e.scope.entities.length} entit${e.scope.entities.length===1?"y":"ies"}`:e.scope.domains.length>0?e.scope.domains.join(" + "):"matching entities";return`${e.function} of ${n}`}function wi(e,n,t){if(t<0||t>=e.length)return;let[i]=e.splice(n,1);e.splice(t,0,i)}function bf(e,n,t,i,a,r){let o=(s,l)=>e.update(d=>{let c=i(d);c&&s(c)},l?`${a}-${l}`:void 0);return h`
    ...
    ${s===void 0?f:Fc(s,n.partId,pe(e),m=>u(g=>{m?g.partId=m:delete g.partId}))}
    ...
    ${n.cases.map((m,g)=>vf(e,m,g,n,a,u,`${o}-${m.id}`,p))}
    <div class="adders"><button class="small" @click=${()=>u(m=>{m.cases.push(Or())})}>+ case</button></div>
    ${Oe("Otherwise",n.otherwise!==void 0,m=>u(g=>{m?g.otherwise=g.otherwise??[]:delete g.otherwise}))}
    ${n.otherwise?h`<div class="case-box otherwise">
          ...
          ${Rc(e,n.otherwise,a,m=>u(g=>{g.otherwise&&m(g.otherwise)}),`${o}-otherwise`,p)}
        </div>`:f}
  </div>`}function vf(e,n,t,i,a,r,o,s=!1){let l=(c,u)=>r(p=>{let m=p.cases.find(g=>g.id===n.id);m&&c(m)},u),d=e.liveBranch(i)===n.id;return h`<div class="case-box ${d?"match":""}">
    <div class="rule-head">
    ...
    ${Rc(e,n.then,a,c=>l(u=>c(u.then)),`${o}-then`,s)}
        ...
        ${s.pattern&&!kf(s.pattern)?h`<div class="hint warn">This pattern does not compile. The test fails until it does.</div>`:f}`;break;case"times":c=h`<div class="row-inline">
          ${tc(e,"From",s.value??H("22:00"),u=>o(p=>{p.comparison.value=u},"rhs"),`${r}-rhs`)}
          ${tc(e,"To",s.upper??H("06:00"),u=>o(p=>{p.comparison.upper=u},"upper"),`${r}-upper`)}
        </div>
        <div class="hint">The start is included and the end is not. An end earlier than the start wraps midnight, so 22:00 to 06:00 is the night. Equal times match nothing.</div>`;break;case"options":c=$f(n.value)?Cf(s.options??[],u=>o(p=>{p.comparison.options=ho(u)},"options")):fe("Options (comma separated)",(s.options??[]).join(", "),u=>o(p=>{p.comparison.options=u.split(",").map(m=>m.trim()).filter(Boolean)},"options"));break;case"none":break}return h`<div class="test-box">
    <div class="rule-head">
    ...
    ${ze("Comparison",s.kind,kl.map(u=>[u,Tn[u]]),u=>o(p=>{p.comparison=Dr(p.comparison,u)}))}
    ${c}
  </div>`}function kf(e){try{return new RegExp(e),!0}catch{return!1}}function tc(e,n,t,i,a){if(t.kind.kind!=="literal")return se(e,t,i,{showResolved:!0,label:n,key:a});let r=t.kind.value,o=qt(r)??"";return h`<label class="field"><span>${n}</span>
    <input type="time" .value=${o}
      @input=${he(s=>i({...t,kind:{kind:"literal",value:s}}))} />
    ${r!==""&&o===""?h`<div class="hint warn">"${r}" is not a 24-hour HH:MM time. The test stays false until it is.</div>`:f}</label>`}function $f(e){return e.kind.kind==="time"&&e.kind.timeField==="weekday"}function Cf(e,n){let t=Hd(e),i=a=>n(t.includes(a)?t.filter(r=>r!==a):[...t,a]);return h`<div class="field seg-field"><span>Days</span>
    <div class="seg wide" role="group" aria-label="Days">
      ${Ad.map((a,r)=>h`<button type="button" role="checkbox" aria-checked=${t.includes(r)?"true":"false"}
        class=${t.includes(r)?"on":""} @click=${()=>i(r)}>${a}</button>`)}
    </div></div>`}function Rc(e,n,t,i,a,r=!1){let o=mf(t,r);return h`
    ${n.length===0?h`<div class="hint keep">No changes.</div>`:f}
    ${n.map((s,l)=>Sf(e,s,l,t,(d,c)=>i(u=>{u[l]&&d(u[l])},c?`${a}-${l}-${c}`:void 0),()=>i(d=>{d.splice(l,1)}),`${a}-${l}`,r))}
    <select class="adder" @change=${s=>{let l=s.target,d=l.value;l.value="",d&&i(c=>{c.push(Jt(d))})}}>
      <option value="">+ change…</option>
      ${o.map(s=>h`<option value=${s}>${zo[s]}</option>`)}
    </select>`}var Ic=["setColor","setBorderColor","setBackgroundColor"];function Sf(e,n,t,i,a,r,o,s=!1){let l=!ji[i].includes(Ee[n.kind]),d=s&&!l&&!Fa.includes(Ee[n.kind]);return h`<div class="change-box">
    <div class="rule-head">
    ...
    ${Ac(e,n,a,o)}
  </div>`}function Ac(e,n,t,i){let a=qi(n.kind),r=f;if(a==="value"){let o=n.value??H("");if(Ic.includes(n.kind)){let s=o.kind.kind==="literal";r=h`${s?le("Colour",o.kind.kind==="literal"?o.kind.value:"",l=>t(d=>{d.value=H(l??"#FFFFFF")},"color")):se(e,o,l=>t(d=>{d.value=l},"value"),{noFormat:!0,showResolved:!0,label:"Colour from",key:`${i}-value`})}
      ...
      ${bf(e,n,t,i,a,o)}`:Tf(e,s.table,n[0],t,i,a,r,o)}function Tf(e,n,t,i,a,r,o,s){let l=(x,M)=>e.update(E=>{let P=a(E);P&&x(P)},M?`${r}-${M}`:void 0),d=n.value??nc.get(r)??o,c=n.rows.length===0,u=n.numberMode||c&&d!==void 0&&!$d(d)&&Ef(e.resolve(d)),p=ji[i],m=wa.get(r)??new Set,g=n.columns.length===0&&m.size===0?[kd[i]]:[],y=hd(n.columns,[...m,...g.filter(x=>x!==void 0)],p),v=xa.get(r),$=t?t.partId:s?.some(x=>x.id===v)?v:void 0,S=s!==void 0&&$!==void 0,k=S?p.filter(x=>Fa.includes(x)):p,C=S?y.filter(x=>!Fa.includes(x)):[],O=(x,M)=>{if(!t){x?xa.set(r,x):xa.delete(r),ke(M);return}l(E=>{let P=E[0];P&&(x?P.partId=x:delete P.partId)})},N=t?e.liveBranch(t):"none",U=t?e.forced.get(t.id)??"live":"live",te=x=>U!=="live"&&(U==="otherwise"?x==="otherwise":U.caseId===x),T=x=>{t&&e.setForced(t.id,te(x)?"live":x==="otherwise"?"otherwise":{caseId:x})},V=x=>{nc.set(r,x),n.rows.length!==0&&l(M=>xd(M,x),"lhs")},Z=()=>{xa.delete(r),l(x=>{yd(x,d??H(""),u),$!==void 0&&x[0]&&x[0].partId===void 0&&(x[0].partId=$)})},W=n.rows.map((x,M)=>ac(e,{key:`${r}-${x.caseId}`,label:wd(x.comparison,E=>$e(E,pe(e))),columns:y,changes:x.changes,live:N===x.caseId,forced:te(x.caseId),onForce:()=>T(x.caseId),when:Af(e,x.comparison,`${r}-${x.caseId}`,(E,P)=>l(B=>{let K=B[0]?.cases.find(ve=>ve.id===x.caseId)?.when.tests[0];K&&E(K.comparison)},P&&`${x.caseId}-${P}`)),updChanges:(E,P)=>l(B=>{let K=B[0]?.cases.find(ve=>ve.id===x.caseId);K&&E(K.then)},P&&`${x.caseId}-${P}`),acts:h`
    ...
    <div class="states">
      ...
      ${R===void 0?f:h`<div class="hint warn confirm-row">
        Remove the ${Ze[R]} column? Its ${ic(n,R)} value${ic(n,R)===1?"":"s"} are deleted from every state.
        <button class="danger small" @click=${x=>{ka.delete(r),wa.get(r)?.delete(R),ke(x.target),l(M=>vd(M,R))}}>Remove</button>
    ...
    </div>`}function Ef(e){let n=(e??"").trim();return n!==""&&Number.isFinite(Number(n))}var Mf=["icon","text","color","visibility","opacity","fontSize","fontWeight","rotation","gaugeValue","gaugeMin","gaugeMax","backgroundColor","borderColor","borderWidth"];function ic(e,n){let t=0;for(let i of e.rows)ua(i.changes,n)&&(t+=1);return e.otherwise&&ua(e.otherwise,n)&&(t+=1),t}function Ff(e){return!!e.target?.closest?.("input, select, textarea, button, label, [popover]")}function ac(e,n){return h`<tr class="state-row ${n.live?"live":""} ${n.forced?"forced":""}"
    title=${`${n.label}. Click to hold the previews on this state.`}
    @click=${t=>{Ff(t)||n.onForce()}}>
    <td class="when">
    ...
    ${n.columns.map(t=>h`<td>${Rf(e,t,n.changes,n.updChanges,`${n.key}-${t}`)}</td>`)}
    <td class="acts">${n.acts}</td>
  </tr>`}function Rf(e,n,t,i,a){let r=ua(t,n),o=_a(a);if(!r)return h`<button type="button" class="cell empty" title=${`Set ${Ze[n]} for this state`}
      @click=${d=>{i(c=>{c.push(Jt(pd[n]))}),uc(d.target,o)}}>unchanged</button>`;let s=(d,c)=>i(u=>{let p=u.find(m=>Ee[m.kind]===n);p&&d(p)},c&&`${n}-${c}`),l=Ze[n];return h`
    <button type="button" class="cell filled" popovertarget=${o} aria-haspopup="dialog" title=${`${l}. Click to change it.`}>${If(e,r)}</button>
    <div class="value-pop" id=${o} popover role="dialog" aria-label=${l} @toggle=${pc}>
      <div class="pop-head">
        <b>${l}</b>
        <span class="spacer"></span>
        <button type="button" class="small" popovertarget=${o} popovertargetaction="hide">Done</button>
      </div>
      ${vi.has(o)?h`${n==="visibility"?X("This state",r.kind==="hide"?"hide":"show",[["show","Shown"],["hide","Hidden"]],d=>s(c=>{c.kind=d})):Ac(e,r,s,a)}
    ...
    </div>`}function If(e,n){if(n.kind==="hide")return h`<span class="cell-word">Hidden</span>`;if(n.kind==="show")return h`<span class="cell-word">Shown</span>`;let t=qi(n.kind);if(t==="number")return h`<span class="cell-word mono">${n.number??0}</span>`;if(t==="weight")return h`<span class="cell-word">${Mn.find(([r])=>r===(n.weight??"regular"))?.[1]}</span>`;let i=n.value??H(""),a=i.kind.kind==="literal"?i.kind.value:void 0;if(Ic.includes(n.kind))return h`<span class="swatch" style=${`background:${a&&/^#[0-9a-fA-F]{6,8}$/.test(a)?a:"transparent"}`}></span>
      <span class="cell-word">${a?Re(a):$e(i,pe(e))}</span>`;if(n.kind==="setIcon"&&a){let r=e.icons.render(a,16,"#FFFFFF");return h`${r??f}<span class="cell-word">${a}</span>`}return h`<span class="cell-word">${$e(i,pe(e))}</span>`}function Re(e){return{"#FF453A":"red","#FF9F0A":"orange","#FFD60A":"amber","#34C759":"green","#30D158":"green","#0A84FF":"blue","#64D2FF":"cyan","#BF5AF2":"purple","#FFFFFF":"white","#8E8E93":"grey","#000000":"black","#FFCC00":"amber","#FF3B30":"red"}[e.toUpperCase()]??e}function Af(e,n,t,i){let a=Yt(n.kind),r=ao(n.kind),o=(s,l,d,c)=>Lf(e,s,l,`${t}-${d}`,r,c,d==="rhs"?"Compare with":"Upper bound");return h`<span class="when-cell">
    <select class="when-op" title="How this state is decided" @change=${he(s=>i(l=>{let d=Dr(l,s);l.kind=d.kind,d.value!==void 0?l.value=d.value:delete l.value,d.upper!==void 0?l.upper=d.upper:delete l.upper}))}>
      ${io.map(s=>h`<option value=${s} ?selected=${s===n.kind}>${Hf(s)}</option>`)}
    </select>
  ...
  </span>`}function Hf(e){switch(e){case"lessThan":return"below\u2026";case"lessOrEqual":return"\u2026or below";case"between":return"between\u2026";case"greaterOrEqual":return"\u2026or above";case"greaterThan":return"above\u2026";default:return Tn[e]}}function Lf(e,n,t,i,a,r,o){let s=_a(i),l={showResolved:!0,label:o,key:i};if(n.kind.kind!=="literal")return h`<span class="rhs">
    ...
    ${cc(e,s,o,n,t,l)}
  </span>`}var ki=[{kind:"toggle",title:"Toggle button",blurb:"An icon that toggles the entity when tapped and looks different while it is on.",domains:Rr,layerCount:2},{kind:"status",title:"Status text",blurb:"The entity's state as one line of text, dimmed while it is unavailable.",layerCount:1},{kind:"gauge",title:"Sensor gauge",blurb:"An arc that fills with the entity's reading and changes colour across three bands.",preferNumeric:!0,layerCount:1},{kind:"chart",title:"Forecast chart",blurb:"A bar chart of the readings already in the entity, with the highest and lowest marked. For a forecast sensor that holds a list.",layerCount:1},{kind:"history",title:"History chart",blurb:"A line of how the entity has moved over the last six hours, read from Home Assistant's recorder.",preferNumeric:!0,layerCount:1},{kind:"doorHistory",title:"Door history",blurb:"A strip of when the entity was open over the last hour, with its name above. For a door, a window or anything else with two states.",domains:["binary_sensor","cover"],layerCount:2},{kind:"camera",title:"Camera",blurb:"The camera's latest snapshot, filling the face.",domains:["camera"],layerCount:1}];function zc(e){return ki.find(n=>n.kind===e)??ki[0]}var Lc="#FF9F0A",Oa="#8E8E93",_f=["#FF453A","#FFD60A","#34C759"],Nc=["#0A84FF","#34C759","#FF9F0A"];function Pf(e){return e?.attributes?.device_class==="battery"?_f:Nc}var zf={light:{off:"lightbulb",on:"lightbulb.fill"},switch:{off:"power",on:"power"},fan:{off:"fan.fill",on:"fan.fill"},input_boolean:{off:"circle",on:"circle.fill"},cover:{off:"curtains.closed",on:"window.casement"},lock:{off:"lock.open.fill",on:"lock.fill"},media_player:{off:"speaker.slash.fill",on:"speaker.wave.2.fill"},siren:{off:"bell.slash.fill",on:"bell.fill"},humidifier:{off:"humidifier.fill",on:"humidifier.fill"},valve:{off:"spigot.fill",on:"spigot.fill"},automation:{off:"gearshape.fill",on:"gearshape.fill"},script:{off:"play.fill",on:"play.fill"},scene:{off:"sparkles",on:"sparkles"},climate:{off:"thermometer.medium",on:"flame.fill"},binary_sensor:{off:"circle",on:"circle.fill"},group:{off:"circle",on:"circle.fill"}};function Nf(e){let n=e.iconName?.trim();return n?{off:n,on:n}:zf[No(e)]??{off:"circle",on:"circle.fill"}}function Of(e){switch(No(e)){case"lock":return{kind:"equals",value:H("locked")};case"cover":case"valve":return{kind:"equals",value:H("open")};case"media_player":return{kind:"equals",value:H("playing")};default:return{kind:"isOn"}}}function No(e){return e.domain||e.entityId.split(".")[0]||""}function Ht(e){return{...e,domain:No(e)}}function Df(e){let n=e?.attributes??{},t=n.min,i=n.max;if(typeof t=="number"&&typeof i=="number"&&i>t)return{min:t,max:i};let a=typeof n.device_class=="string"?n.device_class:"",r=typeof n.unit_of_measurement=="string"?n.unit_of_measurement:"";switch(a){case"battery":case"humidity":case"moisture":return{min:0,max:100};case"temperature":return r.includes("F")?{min:0,max:100}:{min:-10,max:40};default:return r==="%"?{min:0,max:100}:{min:0,max:100}}}function Rn(e){return Math.round(e*1e4)/1e4}function an(e,n,t){return Math.min(t,Math.max(n,e))}function Oo(e,n,t){let i=Ce[e],a=an(Rn(n/i.width),0,1),r=an(Rn(t/i.height),0,1);return{x:Rn((1-a)/2),y:Rn((1-r)/2),width:a,height:r,rotationDegrees:0}}function Vf(e){let n=Ce[e],t=an(Math.round(Math.min(n.width,n.height)*.55),12,30);return{frame:Oo(e,t*1.3,t*1.3),size:t}}function Bf(e){let n=Ce[e],t=an(Math.round(Math.min(n.width,n.height)*.3),9,20);return{frame:Oo(e,n.width*.88,t*1.7),size:t}}function Uf(e){let n=Ce[e],t=Math.min(n.width,n.height)*.9;return{frame:Oo(e,t,t),size:Math.max(2.5,Math.round(t*.2)/2)}}function Oc(e){let n=e==="rectangular";return{frame:{x:.05,y:n?.34:.3,width:.9,height:n?.42:.4,rotationDegrees:0},size:2}}function Gf(e){let n=Ce[e],t=an(Math.round(n.height*.2),6,14);return{frame:{x:.06,y:.56,width:.88,height:Rn(t/n.height),rotationDegrees:0}}}function Kf(e){let n=Ce[e],t=an(Math.round(Math.min(n.width,n.height)*.26),8,15);return{frame:{x:.06,y:.2,width:.88,height:Rn(an(t*1.5/n.height,0,1)),rotationDegrees:0},size:t}}function Wf(){return{frame:{x:0,y:0,width:1,height:1,rotationDegrees:0}}}function jf(e,n){n!==void 0&&(e.kind==="text"?e.payload.fontSize=n:e.kind==="icon"?e.payload.size=n:(e.kind==="gauge"||e.kind==="chart")&&(e.payload.lineWidth=n))}function It(e,n,t,i){let a=i(t);n.payload.frame=a.frame,jf(n,a.size);let r=e.perFamily[t]??(e.perFamily[t]=dt());r.placements[n.payload.id]={frame:a.frame,isHidden:!1,...a.size!==void 0?{size:a.size}:{}}}function At(e){return We(e)}function Do(e,n){let t={kind:{kind:"entityState",...Ht(e)}},i=n?.attributes?.unit_of_measurement;return typeof i=="string"&&i.trim()!==""&&(t.format={useEntityUnit:!0}),t}function _c(e){let n=Jt("setIcon");return n.value=H(e),n}function nn(e){let n=Jt("setColor");return n.value=H(e),n}function qf(e,n){let t=si(),i=t.cases[0],a=i.when.tests[0];a.value={kind:{kind:"entityState",...Ht(e)}},a.comparison=Of(e);let r=n.on!==n.off;return i.then=r?[_c(n.on),nn(Lc)]:[nn(Lc)],t.otherwise=r?[_c(n.off),nn(Oa)]:[nn(Oa)],t}function Yf(e){let n=si(),t=n.cases[0],i=t.when.tests[0];i.value={kind:{kind:"entityState",...Ht(e)}},i.comparison={kind:"isUnavailable"};let a=Jt("setOpacity");return a.number=.35,t.then=[a],n}function Pc(e){let n=Math.abs(e)>=10?Math.round(e):Math.round(e*10)/10;return String(n)}function Jf(e,n,t=Nc){let i=n.max-n.min,a=Pc(n.min+i/3),r=Pc(n.min+i*2/3),o=[{comparison:{kind:"lessThan",value:H(a)},changes:[nn(t[0])]},{comparison:{kind:"between",value:H(a),upper:H(r)},changes:[nn(t[1])]},{comparison:{kind:"greaterThan",value:H(r)},changes:[nn(t[2])]}];return md(Do(e),o)}function Xf(e,n,t){let i=At("icon"),a=Nf(n);return i.payload.symbol=H(a.off),i.payload.colorSlot.baseColorHex=Oa,i.payload.rules=[qf(n,a)],It(e,i,t.family,Vf),e.elements.push(i),Ui(e,i.payload.id,{type:"toggleEntity",...Ht(n)}),i.payload.id}function Zf(e,n,t){let i=At("text");return i.payload.value=Do(n,t.state),i.payload.rules=[Yf(n)],It(e,i,t.family,Bf),e.elements.push(i),i.payload.id}function Qf(e,n,t){let i=At("gauge");i.payload.value=Do(n);let a=Df(t.state);return i.payload.minValue=a.min,i.payload.maxValue=a.max,i.payload.rules=[Jf(n,a,Pf(t.state))],It(e,i,t.family,Uf),e.elements.push(i),i.payload.id}function eg(e,n,t){let i=At("chart");return i.payload.value={kind:{kind:"entityState",...Ht(n)}},i.payload.historyMinutes=0,i.payload.highlight="both",i.payload.marker="pointer",It(e,i,t.family,Oc),e.elements.push(i),i.payload.id}function tg(e,n,t){let i=At("chart");return i.payload.value={kind:{kind:"entityState",...Ht(n)}},i.payload.historyMinutes=360,i.payload.historyPoints=24,i.payload.style="line",i.payload.highlight="both",i.payload.marker="pointer",It(e,i,t.family,Oc),e.elements.push(i),i.payload.id}function ng(e,n,t){let i=Ht(n),a=At("text");a.payload.value=H(i.displayName||i.entityId),a.payload.colorSlot.baseColorHex=Oa,It(e,a,t.family,Kf),e.elements.push(a);let r=t.state?.attributes?.device_class,o=At("timeline");return o.payload.value={kind:{kind:"entityState",...i}},o.payload.bands=kr(i.domain,typeof r=="string"?r:void 0),It(e,o,t.family,Gf),e.elements.push(o),o.payload.id}function ig(e,n,t){let i=At("image");return i.payload.entity=Ht(n),It(e,i,t.family,Wf),e.elements.push(i),i.payload.id}function Dc(e,n,t,i){switch(n){case"toggle":return Xf(e,t,i);case"status":return Zf(e,t,i);case"gauge":return Qf(e,t,i);case"chart":return eg(e,t,i);case"history":return tg(e,t,i);case"doorHistory":return ng(e,t,i);case"camera":return ig(e,t,i)}}var ag=/^[a-z0-9_]+\.shared_(\d+)$/;function Vc(e){return ag.test(e)}function rg(e){return e.length===0?e:e[0].toUpperCase()+e.slice(1)}function Uc(e,n){let i=(e.domain||n.split(".")[0]||"").toLowerCase().replace(/[^a-z0-9_]/g,"");return i===""?"entity":i}function Gc(e,n){let t=new Map;for(let i of zr(e,(a,r)=>n.has(r))){if(i.entityId==="")continue;let a=t.get(i.entityId);if(!a){let r=Uc(i.ref,i.entityId),o=t.size+1;a={placeholderId:`${r}.shared_${o}`,domain:r,label:`${rg(r.replace(/_/g," "))} ${o}`,originalId:i.entityId,where:[]},t.set(i.entityId,a)}a.where.includes(i.where)||a.where.push(i.where)}return[...t.values()]}function og(e,n){let t=structuredClone(e),i=new Map,a=new Map;for(let r of n)i.set(r.originalId,{entityId:r.placeholderId,displayName:r.label,domain:r.domain}),a.set(r.originalId,r.placeholderId);_r(t,r=>{let o=i.get(r.entityId);return o?{...o}:void 0}),Pr(t,r=>Lr(r,a)),delete t.openPageId,delete t.openPageName,t.tapAction.type==="openPage"&&(t.tapAction={type:"none"});for(let r of t.elements)r.kind==="tap"&&(delete r.payload.openPageId,delete r.payload.openPageName,r.payload.action.type==="openPage"&&(r.payload.action={type:"none"}));return t.dataSources=[],t}function Kc(e){let n=!1;return wl(e,t=>{let i=t.kind;if(i.kind!=="aggregate")return;let a=i.aggregate.scope;a.kind==="filter"&&a.areaIds.length+a.labelIds.length+a.floorIds.length>0&&(n=!0)}),n}function sg(e,n="  "){let t=(i,a)=>{if(i===null||typeof i!="object")return JSON.stringify(i)??"null";let r=a+n;if(Array.isArray(i))return i.length===0?"[]":`[
...
`)}
${a}}`};return t(e,"")}function Wc(e,n,t=[]){let i=n==="share"?og(e,t):e,a=bn(i);return delete a.id,delete a.slotIndex,a.dataSources=[],`${sg(a)}
`}function jc(e){let t=(e.name===""?"Complication":e.name).split(/[^\p{L}\p{N}]+/u).filter(i=>i!=="").join("-");return`${t===""?"Complication":t}.json`}var lg="There is nothing to read here. Paste a complication first.",dg="This is not valid JSON. Check for a missing brace or a stray comma.",cg="This is valid JSON but not a complication. A complication starts with { and ends with }.",ug="This does not look like a complication.",Bc="It was made by a newer panel, so update the Wrist Assistant integration before importing it.",pg="00000000-0000-4000-8000-000000000000";function hg(e){let n=/^([A-Za-z]+) is required$/.exec(e);return n?`It is missing "${n[1]}".`:e}function qc(e,n){let t=e.trim();if(t==="")return{ok:!1,error:lg};let i;try{i=JSON.parse(t)}catch{return{ok:!1,error:dg}}if(typeof i!="object"||i===null||Array.isArray(i))return{ok:!1,error:cg};let a=i,r=a.schemaVersion;if(typeof r=="number"&&r>n)return{ok:!1,error:`This complication is schema v${r}; this panel understands up to v${n}. ${Bc}`};let o={...a,id:pg,slotIndex:0},s;try{s=yn(o)}catch(d){let c=d instanceof Ne||d instanceof Error?d.message:String(d);return{ok:!1,error:`${ug}

${hg(c)}`}}let l=Vi(a);if(l.length>0){let d=l.slice(0,3).join(", "),c=l.length>3?`, and ${l.length-3} more`:"";return{ok:!1,error:`This complication uses keys this panel does not know: ${d}${c}. ${Bc}`}}return{ok:!0,config:s,raw:i}}function Vo(e,n){let t=new Set;for(let o of Object.keys(n)){let s=o.split(".")[0]??"";s!==""&&t.add(s)}let i=o=>Object.prototype.hasOwnProperty.call(n,o),a=new Map,r=zr(e,(o,s)=>Vc(o)||t.has(s));for(let o of r){if(o.entityId==="")continue;let s=Vc(o.entityId);if(!s&&i(o.entityId))continue;let l=a.get(o.entityId);l||(l={entityId:o.entityId,domain:Uc(o.ref,o.entityId),label:o.ref.displayName||o.entityId,where:[],required:s},a.set(o.entityId,l)),l.label===o.entityId&&o.ref.displayName!==""&&(l.label=o.ref.displayName),l.where.includes(o.where)||l.where.push(o.where)}return[...a.values()]}function Yc(e,n){let t=structuredClone(e);_r(t,a=>{let r=n.get(a.entityId);if(r)return{entityId:r.entityId,displayName:r.displayName,domain:r.domain||r.entityId.split(".")[0]||""}});let i=new Map;for(let[a,r]of n)i.set(a,r.entityId);return Pr(t,a=>Lr(a,i)),t}function Jc(e,n){let t=e.trim();if(t==="")return"";let i=a=>n.has(a.toLowerCase());if(!i(t))return t;for(let a=2;a<=99;a+=1){let r=`${t} ${a}`;if(!i(r))return r}return t}function mg(e){return e.length<=1?e[0]??"":`${e.slice(0,-1).join(", ")} and ${e[e.length-1]}`}function Xc(e){let n=e.elements.length,t=n===1?"1 layer":`${n} layers`,i=ia(e);return i.length===0?t:`${t}, ${mg(i)}`}function Bo(e){if(!e.parsed)return"Paste a complication first.";let n=e.name.trim();if(n==="")return"Give it a name first.";if(e.taken.has(n.toLowerCase()))return"A complication on this watch already has that name.";if(e.unchosen===1)return"One entity still needs choosing.";if(e.unchosen>1)return`${e.unchosen} entities still need choosing.`}var gg=3e4,yg=500,bg=3e4,Zc="preset-entity";function Qc(e){return`import-entity-${e}`}var xg={entityId:"",displayName:"",domain:""},vg={ArrowLeft:{dx:-1,dy:0},ArrowRight:{dx:1,dy:0},ArrowUp:{dx:0,dy:-1},ArrowDown:{dx:0,dy:1}};function Uo(e){return"id"in e?`${e.kind}:${e.id}`:e.kind}function wg(e){return e.kind==="family"?"look":"content"}function Go(e){let n=e.document?.supportedFamilies;return Array.isArray(n)?n.filter(t=>typeof t=="string"):[]}function eu(){return h`<span class="hstep" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
...
`)}var au="wrist-assistant-panel.layers.v1",mt=34,rn=200,Sg=720,Da=320,Tg=80,Eg=56,ru="wrist-assistant-panel.columns.v3",Ko=e=>Math.max(rn,Math.min(Sg,Math.round(e))),ou=e=>e.metaKey||e.ctrlKey||e.shiftKey,Mg=/^(range|checkbox|radio|color|button|submit|reset|file|image)$/,$i=typeof navigator<"u"&&/Mac|iPhone|iPad/.test(navigator.platform)?"Cmd":"Ctrl",Lt=$i==="Cmd"?"\u2318":"Ctrl+",Wo=$i==="Cmd"?"\u21E7":"Shift+";function su(e,n,t){if(e<=0)return{columns:3,left:n,right:t};let i=e-Tg;if(i>=rn*2+Da){let r=i-Da,o=n,s=t;if(o+s>r){let l=r/(o+s);o=Math.max(rn,Math.floor(o*l)),s=Math.max(rn,Math.floor(s*l));let d=o+s-r;d>0&&(o>=s?o=Math.max(rn,o-d):s=Math.max(rn,s-d))}return{columns:3,left:o,right:s}}let a=e-Eg;return a>=rn+Da?{columns:2,left:Math.min(n,a-Da),right:t}:{columns:1,left:n,right:t}}var A=class A extends yt{constructor(){super(...arguments);this.narrow=!1;this.colLeft=tu;this.colRight=nu;this.panelWidth=0;this.owners=[];this.records=[];this.maxSchemaVersion=6;this.presets=[];this.occupied=[];this.serverToken=0;this.sendStatusKnown=!1;this.polling=!1;this.sendPending=!1;this.pages=[];this.templateResults=new Map;this.historySeries=new Map;this.forced=new Map;this.showRaw=!1;this.inspect={kind:"general"};this.openSections=new Set(["content"]);this.helpSections=new Set;this.scrubStart=()=>this.draft?.beginGesture();this.scrubEnd=()=>this.draft?.endGesture();this.pickerOpen=!1;this.pickerFilter="all";this.testValues=new Map;this.thumbStep=0;this.layerDetail="compact";this.addOpen=!0;this.addDetail="expanded";this.multi=new Set;this.collapsed=new Set;this.activeFamily="rectangular";this.picking=!1;this.listHoverIds=[];this.zoomed=!1;this.helpOpen=!1;this.showTaps=!1;this.newOpen=!1;this.newName="";this.shareOpen=!1;this.shareMode="share";this.shareLabels=new Map;this.shareNote="";this.importOpen=!1;this.importText="";this.importName="";this.importMap=new Map;this.recordPreviews=new Map;this.previewCase=$n.label;this.saving=!1;this.confirmDelete=!1;this.moving=!1;this.version=0;this.historySignature="";this.icons=sd(()=>this.requestUpdate());this.imageSizes=ld(()=>this.requestUpdate());this.symbols=new sa(()=>this.requestUpdate());this.keyHandler=t=>this.onKey(t);this.heldArrows=new Set;this.keyUpHandler=t=>{this.heldArrows.delete(t.key)&&this.heldArrows.size===0&&this.draft?.endGesture()};this.fades=new ca;this.sizeObserver=new ResizeObserver(t=>{let i=t[0]?.contentRect.width??0;Math.abs(i-this.panelWidth)>=1&&(this.panelWidth=i)});this.beforeUnload=t=>{this.draft?.dirty&&t.preventDefault()};this.pickerOutside=t=>{t.composedPath().some(a=>a instanceof HTMLElement&&a.classList.contains("picker"))||this.togglePicker(!1)};this.newKeys=t=>{t.key==="Enter"&&(this.newName.trim()===""||this.newFamily===void 0||this.newNameProblem()!==void 0||(t.preventDefault(),this.createNew()))};this.importKeys={handleEvent:t=>{if(t.key!=="Enter"||t.target instanceof HTMLTextAreaElement)return;let i=this.importParse,a=i?.ok?i.config:void 0;if(!a)return;let r=Vo(a,this.hass.states);r.some(s=>Ho(Qc(s.entityId)))||Bo({parsed:!0,name:this.importName,taken:this.takenNames(),unchosen:this.unchosenCount(r)})!==void 0||(t.preventDefault(),t.stopPropagation(),this.doImport())},capture:!0};this.presetKeys={handleEvent:t=>{t.key==="Enter"&&(this.presetEntity===void 0||Ho(Zc)||(t.preventDefault(),t.stopPropagation(),this.createFromPreset()))},capture:!0}}static{this.styles=Ga`
    :host {
      ...
      --wa-text: ${be(Pe.text)};
      --wa-icon: ${be(Pe.icon)};
      --wa-gauge: ${be(Pe.gauge)};
      --wa-shape: ${be(Pe.shape)};
      --wa-image: ${be(Pe.image)};
      --wa-tap: ${be(Pe.tap)};
  ...
  `}connectedCallback(){super.connectedCallback(),this.loadColumnWidths(),this.loadListView(),this.sizeObserver.observe(this),window.addEventListener("keydown",this.keyHandler),window.addEventListener("keyup",this.keyUpHandler),window.addEventListener("beforeunload",this.beforeUnload),this.addEventListener(Ra,this.scrubStart),this.addEventListener(Ia,this.scrubEnd),this.loadOwners(),this.watchStatusTimer=window.setInterval(()=>{this.refreshWatchStatus()},bg)}loadColumnWidths(){try{let t=window.localStorage.getItem(ru);if(!t)return;let i=JSON.parse(t);typeof i.left=="number"&&(this.colLeft=Ko(i.left)),typeof i.right=="number"&&(this.colRight=Ko(i.right))}catch{}}saveColumnWidths(){try{window.localStorage.setItem(ru,JSON.stringify({left:this.colLeft,right:this.colRight}))}catch{}}loadListView(){try{let t=window.localStorage.getItem(au);if(!t)return;let i=JSON.parse(t);(i.thumbStep===0||i.thumbStep===1||i.thumbStep===2)&&(this.thumbStep=i.thumbStep),(i.detail==="compact"||i.detail==="expanded")&&(this.layerDetail=i.detail),typeof i.addOpen=="boolean"&&(this.addOpen=i.addOpen),(i.addDetail==="compact"||i.addDetail==="expanded")&&(this.addDetail=i.addDetail)}catch{}}saveListView(){try{window.localStorage.setItem(au,JSON.stringify({thumbStep:this.thumbStep,detail:this.layerDetail,addOpen:this.addOpen,addDetail:this.addDetail}))}catch{}}renderGutter(t){return h`<div class="gutter ${t}" role="separator" aria-orientation="vertical"
      ...
      ${a&&r.length>0?h`<div class="adders">
            ${r.map(o=>h`<button class="small primary"
              ...
              @click=${()=>this.mutate(s=>wc(s,o,i))}>Copy the ${ee(o)} layout</button>`)}
          </div>
    ...
    </div>`}addShape(t){this.mutate(i=>Ql(i,t)),this.activeFamily=t,this.inspect={kind:"family"}}removeShape(t){let i=this.draft?.config;if(!i||!aa(i,t))return;let a=td(i,t);a.length>0&&!window.confirm(`Remove the ${ee(t)} shape? This deletes ${a.join(", ")}. They are on this shape only, so nothing else in the complication loses anything.`)||(this.mutate(r=>ed(r,t)),this.ensureActiveFamily())}createNew(){let t=this.newFamily,i=this.newName.trim();!t||i===""||this.newNameProblem()!==void 0||(this.closeNewDialog(),this.startNew(cl(i,this.freeSlot(),[t])))}setForced(t,i){let a=new Map(this.forced);i==="live"?a.delete(t):a.set(t,i),this.forced=a}async save(t=!1){if(!(!this.draft||!this.ownerId||!this.canEdit||this.saving)&&!(!t&&!this.draft.dirty)){if(!t&&!this.slotChosen){this.saveError="The watch is full. Delete a complication first.";return}this.saving=!0,this.saveError=void 0;try{let i=this.draft;if(t){let o=this.freeSlot();if(o<0){this.saveError="The watch is full (iPhone presets count too), so there is nowhere to put a copy. Delete a complication first.";return}let s=structuredClone(i.config);s.id=Y(),s.slotIndex=o,i=new Qt(s,null)}let a=i.encoded(),r=await ys(this.hass,this.ownerId,a,i.baseRevision);if(!r.ok||!r.record){r.error==="conflict"?this.conflict={current:r.current??null,message:r.message??"Someone else saved this complication first."}:this.saveError=r.message??r.error??"Save failed";return}this.conflict=void 0,this.remoteRevision=void 0,this.selectedId=r.record.id,this.draft=Qt.fromDocument(r.record.document,r.record.revision),this.savedName=String(r.record.document?.name??""),this.recompile(),this.beginSendWait(),await this.loadRecords()}catch(i){this.saveError=ft(i)}finally{this.saving=!1}}}async deleteCurrent(){if(!(!this.draft||!this.ownerId||!this.selectedId||!this.canEdit)){if(this.draft.baseRevision===null){this.clearDraft(),this.selectedId=void 0,this.selectFirst();return}this.saving=!0;try{let t=await bs(this.hass,this.ownerId,this.selectedId,this.draft.baseRevision);if(!t.ok){t.error==="conflict"?this.conflict={current:t.current??null,message:t.message??"This complication changed on the server."}:this.saveError=t.message??t.error??"Delete failed";return}this.clearDraft(),this.selectedId=void 0,await this.loadRecords()}catch(t){this.saveError=ft(t)}finally{this.saving=!1,this.confirmDelete=!1}}}duplicate(){if(!this.draft)return;let t=structuredClone(this.draft.config);t.id=Y(),t.name=`${t.name} copy`,t.slotIndex=this.freeSlot(),this.startNew(t)}reloadFromServer(){let t=this.conflict?.current??this.records.find(i=>i.id===this.selectedId);this.conflict=void 0,t&&!t.deleted?this.openRecord(t):(this.clearDraft(),this.selectedId=void 0,this.loadRecords())}get selectedOwner(){return this.owners.find(t=>t.owner_watch_id===this.ownerId)}async moveAll(){let t=this.ownerId,i=this.moveTarget;if(!(!t||!i||this.moving)){this.moving=!0,this.moveError=void 0;try{await xs(this.hass,t,i),this.moveTarget=void 0,await this.loadOwners(),await this.selectOwner(i)}catch(a){this.moveError=ft(a)}finally{this.moving=!1}}}scheduleTemplates(t){this.debounceTimer&&window.clearTimeout(this.debounceTimer),this.debounceTimer=window.setTimeout(()=>{this.refreshTemplates()},t),this.templateTimer&&window.clearInterval(this.templateTimer),this.templateTimer=window.setInterval(()=>{this.refreshTemplates()},gg)}async refreshHistorySeries(){let t=this.draft?.config,i=t?yr(t):[],a=t?br(t):[];if(i.length===0&&a.length===0){this.historySeries.size>0&&(this.historySeries=new Map);return}let r={};for(let s of i)r[s.key]={entity_id:s.entityId,minutes:s.minutes,points:s.points,...s.mode==="states"?{mode:"states"}:{}};let o={};for(let s of a)o[s.key]={entity_id:s.entityId,minutes:s.minutes,period:s.period,type:s.type};try{let[s,l]=await Promise.all([ks(this.hass,r),$s(this.hass,o).catch(()=>({}))]),d=new Map;for(let[c,u]of Object.entries({...s,...l}))u.ok&&d.set(c,u.series);this.historySeries=d}catch{}}async refreshTemplates(){this.refreshHistorySeries();let t=this.compiled?.document;if(!t){this.templateResults=new Map,this.templateError=void 0,this.templateFetchedAt=Date.now();return}try{let a=(await ws(this.hass,{doc:t})).doc;if(!a)return;if(!a.ok){this.templateError=a.error;return}let r=Ml(a.value);if(!r){this.templateError="Template did not render to a JSON object";return}this.templateResults=r.values,this.templateError=void 0,this.templateFetchedAt=Date.now()}catch(i){this.templateError=ft(i)}}entityStateFor(t,i,a){let r=this.hass.states[t];if(!r)return;let o=r.attributes,s=t.split(".")[0]??"",l={entityId:t,state:(a?this.testValues.get(t):void 0)??r.state,unitOfMeasurement:typeof o.unit_of_measurement=="string"?o.unit_of_measurement:void 0,iconName:i,domain:s};if(s==="timer"){l.timerState=r.state,typeof o.finishes_at=="string"&&(l.finishesAt=o.finishes_at);let d=Fg(o.remaining);d!==void 0&&(l.remaining=d)}return typeof o.entity_picture=="string"&&(l.entityPicture=o.entity_picture),l}buildContext(){let t=new Map;for(let[i,a]of this.compiled?.entities??[]){let r=this.entityStateFor(i,a.iconName??"",!0);r&&t.set(i,r)}return{entityStates:t,templateResults:this.templateResults,historySeries:this.historySeries,namedValues:this.draft?.config.values??[],dataAgeSeconds:this.templateFetchedAt===void 0?void 0:(Date.now()-this.templateFetchedAt)/1e3}}renderPickButton(){let t=this.picking,i=!this.draft||this.parseError!==void 0;return h`<button class="pick ${t?"on":""}" ?disabled=${i}
      ...
      <header>
        ...
        ${this.renderPicker()}
        ${this.hass.user?.is_admin?h`<span class="hor" aria-hidden="true">or</span>${eu()}`:f}
      ...
      <div class="gate-card">
        ...
        <p class="gate-lead">${ad(t?.app_version)}</p>
        <ol class="gate-steps">
          <li>
    ...
    </div>`}setShareLabel(t,i){let a=new Map(this.shareLabels);a.set(t,i),this.shareLabels=a}openShareDialog(){this.draft&&(this.shareOpen=!0,this.shareMode="share",this.shareLabels=new Map,this.shareNote="",this.updateComplete.then(()=>{let t=this.renderRoot.querySelector("dialog.share-dialog");t&&!t.open&&t.showModal()}))}closeShareDialog(){let t=this.renderRoot.querySelector("dialog.share-dialog");t?.open?t.close():this.shareOpen=!1}async copyShareText(t){try{if(navigator.clipboard?.writeText){await navigator.clipboard.writeText(t),this.shareNote="Copied.";return}}catch{}let i=this.renderRoot.querySelector("dialog.share-dialog textarea");i?.focus(),i?.select();let a=!1;try{a=document.execCommand("copy")}catch{a=!1}this.shareNote=a?"Copied.":"Press Cmd+C or Ctrl+C to copy."}downloadShareText(t){let i=this.draft?.config;if(!i)return;let a=jc(i),r=URL.createObjectURL(new Blob([t],{type:"application/json"})),o=document.createElement("a");o.href=r,o.download=a,o.click(),window.setTimeout(()=>URL.revokeObjectURL(r),0),this.shareNote=`Saved as ${a}.`}renderImportDialog(){let t=this.importParse,i=t?.ok?t.config:void 0,a=i?Vo(i,this.hass.states):[],r=Bo({parsed:i!==void 0,name:this.importName,taken:this.takenNames(),unchosen:this.unchosenCount(a)});return h`<dialog class="import-dialog" @keydown=${this.importKeys} @close=${()=>{this.importOpen=!1}}>
      <div class="new-head">
      ...
      ${r?h`<div class="hint err">A complication on this watch already has that name.</div>`:h`<div class="hint">${Xc(t)}. It opens in the editor and reaches the watch at the first Save.</div>`}
      ...
      ${Kc(t)?h`<div class="hint warn">This design filters by areas, labels or floors from the sender's home. Check its aggregate layers after import.</div>`:f}`}renderImportRow(t){let i=this.importMap.get(t.entityId);return h`<div class="xfer-ent">
      ${et({hass:this.hass},t.label,i??xg,a=>this.setImportEntity(t.entityId,a),Qc(t.entityId),{compact:!0,domain:t.domain,needed:t.required&&i===void 0})}
      ...
      ${a?h`
          <div class="add-grid ${r?"":"lean"}">
            ${Qr.map(s=>h`<button class="add" style=${`--k:${Pe[s]}`} ?disabled=${i} title=${`Add a blank ${Sn[s].toLowerCase()} layer`}
              @click=${()=>{let l=We(s);this.addHere(d=>{d.elements.push(l)}),this.inspect={kind:"layer",id:l.payload.id}}}
              >${r?h`<span class="well">${Sd(s)}</span>`:f}<span class="add-name">${r?D(s):h`<span class="k"></span>`}<span>${Sn[s]}</span></span></button>`)}
          </div>
          <div class="presets">
      ...
      ${this.renderPresetDialog()}
        ...
        <span class="name">
          <b>${Ae(T,d)}</b>
          <small><span class="kind">${Sn[T.kind]}</span> · ${Ag(T,c,this.historySeries,R.size)}</small>
          ${C?h`<span class="facts">${Rg(this.host(),a,T,R).map(B=>h`<span class="fact"><b>${B.label}</b> ${B.value}</span>`)}</span>`:f}
        </span>
        <span class="right">
          <span class="badges">
          ...
          ${i?h`<span class="acts">
            ...
            <button class="icon" title=${`${R.isHidden?"Show":"Hide"} (${Wo}${Lt}H)`} aria-label=${R.isHidden?"Show this layer":"Hide this layer"} @click=${B=>{B.stopPropagation(),this.mutate(K=>Se(K,a,W,{isHidden:!R.isHidden}))}}>${D(R.isHidden?"hide":"show")}</button>
      ...
      </div>`},N=(T,V)=>{let Z=this.inspect.kind==="group"&&this.inspect.id===T.id,W=!this.collapsed.has(T.id),ye=this.rowDrag(T.id,i),R=V[0],b=V[V.length-1],x=E=>{let P=E.currentTarget,B=P.getBoundingClientRect(),K=B.top+(P.classList.contains("drop-before")?mt:0),ve=B.bottom-(P.classList.contains("drop-after")?mt:0),_t=(E.clientY-K)/Math.max(1,ve-K);return _t<.25?"drop-before":!W&&_t>.75?"drop-after":"drop-into"},M=V.map(E=>E.payload.id);return h`<div class="layer group ${Z?"hl":""} ${C?"rich":""}" style=${`--k:${J.group}`} tabindex="0" draggable=${ye.draggable}
        ...
        <span class="right">
          ${i?h`<span class="acts">
            <button class="icon" title=${`Ungroup: keep the layers, drop the folder (${Wo}${Lt}G)`} aria-label="Ungroup" @click=${E=>{E.stopPropagation(),this.mutate(P=>oi(P,T.id)),Z&&(this.inspect={kind:"general"})}}>${D("ungroup")}</button>
          </span>`:f}
      ...
      </div>`},U=[],te=new Set;for(let T=0;T<l.length;T++){let V=l[T],Z=V.payload.groupId,W=Z===void 0?void 0:t.groups?.find(b=>b.id===Z);if(!W){U.push(O(V,!1));continue}if(te.has(W.id))continue;te.add(W.id);let ye=l.filter(b=>b.payload.groupId===W.id);U.push(N(W,ye));let R=this.inspect.kind==="group"&&this.inspect.id===W.id;this.collapsed.has(W.id)||U.push(h`<div class="group-kids">${ye.map(b=>O(b,!0,R))}</div>`)}return h`<div class="card layers-card s${this.thumbStep}" style=${`--thumb-w:${$}px;--thumb-h:${S}px`}>
    ...
    </div>`}renderPresetDialog(){let t=this.presetKind?zc(this.presetKind):void 0,i=this.presetEntity;return h`<dialog class="preset-dialog" @keydown=${this.presetKeys}
        @close=${()=>{this.presetKind=void 0,this.presetEntity=void 0}}>
      ${t===void 0?f:h`
        <h2>${t.title}</h2>
        <div class="hint">${t.blurb}</div>
        ${et(this.host(),"Entity",i??{entityId:"",displayName:"",domain:""},a=>{this.presetEntity=a.entityId===""?void 0:a},Zc,{compact:!0,...t.domains?{domain:t.domains}:{},...t.preferNumeric?{preferNumeric:!0}:{}})}
        <div class="adders">
    ...
    </dialog>`}openPreset(t){this.canEdit&&(this.presetKind=t,this.presetEntity=void 0,this.updateComplete.then(()=>{let i=this.renderRoot.querySelector("dialog.preset-dialog");i&&(i.open||i.showModal(),i.querySelector(".entity-field input")?.focus())}))}closePresetDialog(){let t=this.renderRoot.querySelector("dialog.preset-dialog");t?.open?t.close():(this.presetKind=void 0,this.presetEntity=void 0)}createFromPreset(){let t=this.presetKind,i=this.presetEntity;if(!t||!i)return;let a={family:this.canvasFamily},r=this.hass.states[i.entityId];r&&(a.state=r);let o;this.addHere(s=>{o=Dc(s,t,i,a)}),this.closePresetDialog(),o&&(this.inspect={kind:"layer",id:o})}renderCanvas(){if(this.parseError)return h`<div class="card error">This document cannot be read: ${this.parseError}</div>`;let t=this.draft?.config;if(!t)return h`<div class="card"><div class="empty">Choose a complication in the picker above, or make a new one.</div></div>`;let i=Ji(t,this.buildContext(),this.forced);this.syncCountdownTicker(i);let a=this.currentCase(),r=this.activeFamily;return h`
      ...
      <h2 class="panel-title"><span class="swatch">${D("content")}</span>Shared values
        ...
        <span class="spacer"></span>
        ${this.canEdit?h`<button class="small" @click=${()=>{let o=vc();this.mutate(s=>{s.values.push(o)}),this.inspect={kind:"data",id:o.id}}}>Add</button>`:f}
      </h2>
    ...
    </div>`}commitTestValue(t,i){this.editingValue=void 0;let a=i.trim(),r=new Map(this.testValues),o=this.hass.states[t]?.state;a===""||a===o?r.delete(t):r.set(t,a),this.testValues=r}currentCase(){return ui.find(t=>t.label===this.previewCase)??$n}previewSlot(t){return this.currentCase().slots[t]}crumbs(t,i){let a=this.inspect,r=t.name.trim()||"Complication",o=this.activeFamily==="inline"?"Inline":ee(this.activeFamily),s=a.kind==="family"&&i===void 0?h`<span class="here" style=${`--k:${J.place}`}>${o} shape</span>`:h`<button @click=${()=>{this.inspect={kind:"family"}}} title="Edit the shape">${o}</button>`,l=f,d=f;if(i!==void 0)l=h`<span class="here" style="--k:var(--wa-accent)"><span class="kchip">Picked</span><span class="nm">${i} layers</span></span>`;else if(a.kind==="layer"){let c=t.elements.find(u=>u.payload.id===a.id);if(c){l=h`<span class="here" style=${`--k:${Pe[c.kind]}`}><span class="kchip">${Sn[c.kind]}</span><span class="nm" title=${Ae(c,pe(this.host()))}>${Ae(c,pe(this.host()))}</span></span>`;let u=st(t,c.payload.id);u&&(d=h`<span class="sep">›</span><button @click=${()=>{this.inspect={kind:"group",id:u.id}}} title="Edit the group">${u.name}</button>`)}}else if(a.kind==="group"){let c=t.groups?.find(u=>u.id===a.id);c&&(l=h`<span class="here" style=${`--k:${J.group}`}><span class="kchip">Group</span><span class="nm" title=${c.name}>${c.name}</span></span>`)}else if(a.kind==="data"){let c=t.values.find(u=>u.id===a.id);c&&(l=h`<span class="here" style=${`--k:${J.complication}`}><span class="kchip">Value</span><span class="nm">${c.name||"(unnamed)"}</span></span>`)}return h`<div class="crumbs">
      ...
      <span class="comp-acts">
        ...
        <div class="insp-body" style=${o} @change=${()=>this.draft?.endGesture()}>
          ${ge(a,"complication","Complication",yc(a),{color:J.complication,icon:"watch",alwaysOpen:!0})}
        ...
        </div>`;let s=f,l=!0;if(r.kind==="layer"){let c=t.elements.find(u=>u.payload.id===r.id);if(!c)return this.inspect={kind:"general"},f;s=Sc(a,c,this.canvasFamily,{placement:!0,tap:!0})}else if(r.kind==="group"){let c=t.groups?.find(u=>u.id===r.id);if(!c)return this.inspect={kind:"general"},f;l=!1,s=Ec(a,c)}else if(r.kind==="data"){let c=t.values.find(u=>u.id===r.id);if(!c)return this.inspect={kind:"general"},f;l=!1,s=ge(a,"shared-value","Shared value",xc(a,c),{color:J.complication,icon:"content",summary:'Read by layers whose Source is "Named value"',alwaysOpen:!0})}else s=Mc(a,this.activeFamily);let d=this.openSections.size>1;return h`
      <div class="insp-head">
        ${this.crumbs(t)}
        ${l?h`<button class="expand" @click=${()=>{this.openSections=d?new Set([wg(r)]):new Set(Ro)}}>${d?"One at a time":"Open all"}</button>`:f}
      </div>
      ...
      <input type="checkbox" .checked=${i==="all"} .indeterminate=${i==="mixed"}
        @change=${r=>a(r.target.checked)} /></label>`}multiEditor(t,i){let a=this.canvasFamily,r=this.host(),o=pe(r),s=new ut(this.buildContext(),this.draft?.config),l=kc(t,a,i),d=i.length,c=[...i].reverse(),u=m=>this.mutate(g=>{for(let y of i)Se(g,a,y.payload.id,{isHidden:m})}),p=m=>this.mutate(g=>{for(let y of i){let v=g.elements.find($=>$.payload.id===y.payload.id);v&&v.kind!=="image"&&v.kind!=="tap"&&v.kind!=="timeline"&&(v.payload.colorSlot.baseColorHex=m)}},"multi-colour");return h`
      ...
      ${ge(r,"picked-common",`All ${d} at once`,h`
          ...
          <div class="hint">Size, content and states belong to one layer at a time. Click a layer on its own to reach them.</div>`,{color:J.place,icon:"place",summary:"The settings every picked layer has",alwaysOpen:!0})}`}renderFooter(){let t=this.draft;if(!t)return f;let i=this.records.find(r=>r.id===this.selectedId),a=dd({revision:i?.revision??null,dirty:t.dirty,...this.saveError!==void 0?{error:this.saveError}:{},...this.templateError!==void 0?{templateError:this.templateError}:{}});return h`<details class="foot">
      <summary>
