...
import { addFamily, canRemoveFamily, familiesFor, familyContentSummary, firstDrawable, isDrawable, removeFamily, supportedFamilies } from "./layouts.js";
...
export class WristAssistantPanel extends LitElement {
  ...
  private renderImportDetails(cfg: CustomComplicationConfig, rows: readonly UnresolvedEntity[]) {
    ...
    return html`
      ...
      ${taken
        ? html`<div class="hint err">A complication on this ${this.deviceWord} already has that name.</div>`
        : html`<div class="hint">${importSummary(cfg)}. ${deviceKindOf(this.selectedOwner) === "iphone"
            ? "It opens in the editor, and reaches your iPhone at its next sync after the first Save."
            : "It opens in the editor and reaches the watch at the first Save."}</div>`}
      ${rows.length === 0
  ...
  private renderShapeTabs(cfg: CustomComplicationConfig, layouts: ResolvedAll) {
    ...
    // A shape this device cannot draw gets no tab, even when the document
    // carries a layout for it: a corner layout moved onto a phone stays in
    // the file untouched, there is simply nowhere here to edit it.
    return this.ownerFamilies.map((f) => {
      if (!have.includes(f)) {
  ...
  private renderInspector() {
    ...
    if (ins.kind === "layer") {
      ...
      body = html`<div class="empty-insp">${uiIcon("layers")}<b>Nothing selected</b>
        <span>Click a layer ${deviceKindOf(this.selectedOwner) === "iphone" ? "on the preview" : "on the watch"} or in the list to edit it.<br />The shape's own background and border are the bottom row of the list.</span></div>`;
    }
