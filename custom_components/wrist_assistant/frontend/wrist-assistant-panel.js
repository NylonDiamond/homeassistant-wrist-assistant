...
`)}function Or(e){let n;try{n=JSON.parse(e)}catch{return}if(typeof n!="object"||n===null||Array.isArray(n))return;let t=new Map,i=new Set;for(let[a,r]of Object.entries(n))r===null?i.add(a):t.set(a,Cl(r));return{values:t,nullKeys:i}}function Cl(e){return typeof e=="string"?e:typeof e=="boolean"?e?"true":"false":typeof e=="number"?(Number.isInteger(e)&&Math.abs(e)<1e15,String(e)):JSON.stringify(e)}function Pi(e){let n=_i(e),t=[...n.entities.entries()].sort(([i],[a])=>i<a?-1:i>a?1:0).map(([,i])=>({kind:"entity",entityId:i.entityId,displayName:i.displayName,domain:i.domain,...i.iconName!==void 0?{iconName:i.iconName}:{}}));return n.document&&t.push({kind:"template",value:n.document}),t}function Sl(e,n){if(e.values.length!==0)switch(n){case"latest":return e.values[e.values.length-1];case"highest":return Math.max(...e.values);case"lowest":return Math.min(...e.values);case"average":return e.values.reduce((t,i)=>t+i,0)/e.values.length;case"top":return e.domainMax;case"bottom":return e.domainMin}}function Tn(e){let n=e;if(/^[+-]?(\d+\.?\d*|\.\d+)([eE][+-]?\d+)?$/.test(n))return Number(n);let t=n.toLowerCase();if(t==="inf"||t==="+inf"||t==="infinity"||t==="+infinity")return 1/0;if(t==="-inf"||t==="-infinity")return-1/0;if(t==="nan"||t==="+nan"||t==="-nan")return NaN}function Ct(e){let n=e.trim(),t=Tn(n);if(t!==void 0)return t;let i="";for(let r of n)if(/\p{N}/u.test(r)||r==="."||r==="-"||r==="+")i+=r;else if(i.length>0)break;return i.length===0?void 0:Tn(i)}function El(e){return Number.isInteger(e)&&Math.abs(e)<1e16?e.toFixed(1):String(e)}function Tl(e){let n=Math.max(0,e);return n<60?`${Math.trunc(n)}s`:n<3600?`${Math.trunc(n/60)}m`:n<86400?`${Math.trunc(n/3600)}h`:`${Math.trunc(n/86400)}d`}function Fl(e){return e.replace(/\S+/g,n=>n.charAt(0).toUpperCase()+n.slice(1).toLowerCase())}function Rl(e,n,t){if(Ie(n))return e;let i=n,a=e,r=Tn(e.trim());if(i.relativeTime&&r!==void 0)a=Tl(r);else{let o=Ct(e);if(o!==void 0){let l=o*(i.multiply??1)+(i.offset??0);i.decimals!==void 0?a=l.toFixed(Math.max(0,i.decimals)):l!==o&&(a=Number.isInteger(l)?String(l):El(l))}}switch(i.useEntityUnit&&t&&(a+=t.startsWith("\xB0")||t.startsWith("%")?t:` ${t}`),i.prefix&&(a=i.prefix+a),i.suffix&&(a=a+i.suffix),i.textCase){case"upper":a=a.toUpperCase();break;case"lower":a=a.toLowerCase();break;case"capitalized":a=Fl(a);break}return a}function St(e){let n=Math.trunc(Math.max(0,e)),t=Math.trunc(n/3600),i=Math.trunc(n%3600/60),a=n%60,r=o=>String(o).padStart(2,"0");return t>0?`${t}:${r(i)}:${r(a)}`:`${i}:${r(a)}`}function jt(e,n=240){let t=[],i="",a=!1,r=()=>{if(i!==""){let o=Number(i);Number.isFinite(o)&&t.push(o)}i=""};for(let o of e){if(t.length>=n)break;if(o>="0"&&o<="9")i+=o,a=!0;else if(o===".")i.includes(".")&&r(),i+=".",a=!0;else if(o==="-"||o==="+"){let l=!a;r(),l&&(i+=o),a=!1}else r(),a=!1}return t.length<n&&r(),t}function Ml(e,n){let t,i;return n.scale==="fixed"?(t=Math.min(n.minValue,n.maxValue),i=Math.max(n.minValue,n.maxValue)):(t=e.length>0?Math.min(...e):0,i=e.length>0?Math.max(...e):1),n.baseline==="zero"&&(t=Math.min(t,0),i=Math.max(i,0)),i>t||(i=t+1),{min:t,max:i}}function Il(e,n,t){if(e===void 0)return 0;let i=Ct(e);if(i===void 0||Number.isNaN(i))return 0;let a=t-n;return a===0?0:Math.min(1,Math.max(0,(i-n)/a))}var Ve=class{constructor(n,t){this.ctx=n;this.charts=new Map;this.named=new Map(n.namedValues.map(i=>[i.id.toUpperCase(),i.value])),t&&this.settleCharts(t)}chartReadings(n){let t=rt(n),i=t!==void 0?this.ctx.historySeries?.get(t)??"":this.resolve(n.value)??"",a=jt(i);n.limit>0&&a.length>n.limit&&(a=n.takeFromEnd?a.slice(a.length-n.limit):a.slice(0,n.limit));let r=Ml(a,n),o={values:a,domainMin:r.min,domainMax:r.max},l=this.dereference(n.value);return l&&"entityId"in l.kind&&(o.entity={entityId:l.kind.entityId,displayName:l.kind.displayName,domain:l.kind.domain}),o}settleCharts(n){for(let t of n.elements)t.kind==="chart"&&this.charts.set(t.payload.id,this.chartReadings(t.payload))}dereference(n){let t=n,i=new Set,a=n.format;for(;t.kind.kind==="named";){let o=t.kind.id.toUpperCase();if(i.has(o))return;i.add(o);let l=this.named.get(o);if(!l)return;a=a&&!Ie(a)?a:l.format,t=l}let r={kind:t.kind};return a&&(r.format=a),r}directEntityUnit(n){let t=n.kind;if(t.kind==="entityState"||t.kind==="entityAttribute"||t.kind==="entityAge")return this.ctx.entityStates.get(t.entityId)?.unitOfMeasurement;if(t.kind==="chartStat"){let i=this.charts.get(t.layer.toUpperCase())?.entity;return i?this.ctx.entityStates.get(i.entityId)?.unitOfMeasurement:void 0}}resolve(n){if(!n)return;let t=this.dereference(n);if(!t)return;let i;switch(t.kind.kind){case"literal":i=t.kind.value;break;case"entityState":i=this.ctx.entityStates.get(t.kind.entityId)?.state;break;case"dataAge":i=this.ctx.dataAgeSeconds===void 0?void 0:String(Math.trunc(this.ctx.dataAgeSeconds));break;case"chartStat":{let a=this.charts.get(t.kind.layer.toUpperCase()),r=a?Sl(a,t.kind.stat):void 0;i=a&&r!==void 0?dr(r,a.domainMax-a.domainMin):void 0;break}default:{let a=zi(n,this.named);i=a===void 0?void 0:this.ctx.templateResults.get(a)}}if(i!==void 0)return Rl(i,t.format,this.directEntityUnit(t))}nowMs(){return this.ctx.nowMs??Date.now()}countdownEnd(n){if(!n)return;let t=this.dereference(n);if(!t)return;let i=t.kind;if(i.kind==="entityState"){let o=this.ctx.entityStates.get(i.entityId);if(o?.timerState!==void 0){if(o.timerState!=="active"||!o.finishesAt)return;let l=Date.parse(o.finishesAt);return Number.isFinite(l)&&l>this.nowMs()?l:void 0}}let a=this.resolve(n)?.trim();if(!a)return;let r=Date.parse(a);if(!Number.isFinite(r)){let o=Tn(a);r=o===void 0?NaN:o*1e3}return Number.isFinite(r)&&r>this.nowMs()?r:void 0}countdownFallbackText(n){if(!n)return;let t=this.dereference(n);if(!t||t.kind.kind!=="entityState")return;let i=this.ctx.entityStates.get(t.kind.entityId);if(i?.timerState!==void 0)return i.timerState==="paused"?i.remaining!==void 0&&i.remaining>0?St(i.remaining):"Paused":"Idle"}entityIcon(n){let t=this.dereference(n);return!t||t.kind.kind!=="entityState"?void 0:this.ctx.entityStates.get(t.kind.entityId)?.iconName??t.kind.iconName}isStale(){return(this.ctx.dataAgeSeconds??1/0)>(this.ctx.stalenessThresholdSeconds??1200)}evaluateTest(n){let t=n.comparison;if(t.kind==="isStale")return this.isStale();let i=this.resolve(n.value);if(i===void 0)return t.kind==="isUnavailable";let a=Ct(i),r=()=>this.resolve(t.value),o=()=>{let s=r();return s===void 0?void 0:Ct(s)},l=s=>{let d=o();return a===void 0||d===void 0?!1:s(a,d)};switch(t.kind){case"equals":{let s=r();return s!==void 0&&i===s}case"notEquals":{let s=r();return s!==void 0&&i!==s}case"isOn":return i.toLowerCase()==="on";case"isOff":return i.toLowerCase()==="off";case"isUnavailable":{let s=i.toLowerCase();return s==="unavailable"||s==="unknown"}case"isEmpty":return i.trim().length===0;case"greaterThan":return l((s,d)=>s>d);case"greaterOrEqual":return l((s,d)=>s>=d);case"lessThan":return l((s,d)=>s<d);case"lessOrEqual":return l((s,d)=>s<=d);case"between":{let s=o(),d=this.resolve(t.upper),p=d===void 0?void 0:Ct(d);if(a===void 0||s===void 0||p===void 0)return!1;let[c,h]=s<=p?[s,p]:[p,s];return a>=c&&a<=h}case"contains":{let s=r();return!!s&&i.toLowerCase().includes(s.toLowerCase())}case"startsWith":{let s=r();return!!s&&i.toLowerCase().startsWith(s.toLowerCase())}case"endsWith":{let s=r();return!!s&&i.toLowerCase().endsWith(s.toLowerCase())}case"matchesRegex":{if(!t.pattern)return!1;try{return new RegExp(t.pattern).test(i)}catch{return!1}}case"isOneOf":return(t.options??[]).some(s=>s.toLowerCase()===i.toLowerCase());default:return!1}}evaluateCondition(n){return n.tests.length===0?!0:n.join==="any"?n.tests.some(t=>this.evaluateTest(t)):n.tests.every(t=>this.evaluateTest(t))}applyRules(n,t){let i=new Map;for(let a of n){let r=t?.get(a.id),o=[];if(r&&r!=="live")r==="otherwise"?o=a.otherwise??[]:o=a.cases.find(l=>l.id===r.caseId)?.then??[];else{let l=a.cases.find(s=>this.evaluateCondition(s.when));o=l?l.then:a.otherwise??[]}for(let l of o)i.set(xe[l.kind],l)}return i}liveBranches(n){let t=new Map;for(let i of n){let a=i.cases.find(r=>this.evaluateCondition(r.when));t.set(i.id,a?a.id:i.otherwise?"otherwise":"none")}return t}styleColor(n,t){let i=n.get(t);if(!i)return;let a=this.resolve(i.value);return a||void 0}styleText(n,t){let i=n.get(t);return i?this.resolve(i.value):void 0}styleNumber(n,t){return n.get(t)?.number}resolveElement(n,t){let i=n.payload,a=this.applyRules(i.rules,t),r=a.get("visibility"),o=r?r.kind==="hide":i.isHidden,l=this.styleNumber(a,"rotation"),s=l===void 0?i.frame:{...i.frame,rotationDegrees:l},d=this.styleNumber(a,"opacity")??1,p={id:i.id,isHidden:o,frame:s,opacity:d};switch(n.kind){case"text":{let c=n.payload.countdown?this.countdownEnd(n.payload.value):void 0,h=n.payload.countdown?this.countdownFallbackText(n.payload.value):void 0,g={kind:"text",...p,text:this.styleText(a,"text")??h??this.resolve(n.payload.value)??"--",fontSize:this.styleNumber(a,"fontSize")??n.payload.fontSize,fontWeight:a.get("fontWeight")?.weight??n.payload.fontWeight,colorHex:this.styleColor(a,"color")??n.payload.colorSlot.baseColorHex};return c!==void 0&&(g.countdownEnd=c),g}case"icon":{let c=this.entityIcon(n.payload.symbol)??this.resolve(n.payload.symbol)??"questionmark.circle";return{kind:"icon",...p,symbol:this.styleText(a,"icon")??c,size:this.styleNumber(a,"fontSize")??n.payload.size,colorHex:this.styleColor(a,"color")??n.payload.colorSlot.baseColorHex}}case"gauge":{let c=this.styleText(a,"gaugeValue")??this.resolve(n.payload.value),h=this.styleNumber(a,"gaugeMin")??n.payload.minValue,g=this.styleNumber(a,"gaugeMax")??n.payload.maxValue;return{kind:"gauge",...p,fraction:Il(c,h,g),style:n.payload.style,lineWidth:n.payload.lineWidth,colorHex:this.styleColor(a,"color")??n.payload.colorSlot.baseColorHex,trackColorHex:n.payload.trackColorHex}}case"chart":{let c=n.payload,h=this.charts.get(c.id)??this.chartReadings(c),g=h.values,y={min:h.domainMin,max:h.domainMax},x=this.styleColor(a,"color")??c.colorSlot.baseColorHex,k=xt(c),S=sr(c)?g.map(f=>lr(f,k,c.bandAboveColorHex)):[],b={kind:"chart",...p,values:g,style:c.style,domainMin:y.min,domainMax:y.max,baseline:c.baseline,barGap:c.barGap,lineWidth:c.lineWidth,colorHex:x,highColorHex:c.highColorHex,lowColorHex:c.lowColorHex,marker:c.marker,pointColorHexes:S,fillBands:c.fillBands};if(g.length>0){let f=c.highlight==="highest"||c.highlight==="both",w=c.highlight==="lowest"||c.highlight==="both",R=f?g.indexOf(Math.max(...g)):-1,_=w?g.indexOf(Math.min(...g)):-1;R>=0&&(b.highIndex=R),_>=0&&_!==R&&(b.lowIndex=_)}return b}case"shape":{let c={kind:"shape",...p,shapeKind:n.payload.kind,cornerRadius:n.payload.cornerRadius,fillColorHex:this.styleColor(a,"color")??n.payload.colorSlot.baseColorHex,borderWidth:this.styleNumber(a,"borderWidth")??n.payload.borderWidth},h=this.styleColor(a,"borderColor")??n.payload.borderColorHex;return h!==void 0&&(c.borderColorHex=h),c}case"image":{let c={kind:"image",...p,entityId:n.payload.entity.entityId,showTimestamp:n.payload.timestamp===!0,contentMode:n.payload.contentMode,zoom:n.payload.zoom,panX:n.payload.panX,panY:n.payload.panY,cornerRadius:n.payload.cornerRadius,timestampCorner:n.payload.timestampCorner,timestampSize:n.payload.timestampSize};_e(n.payload)&&(c.timestampX=n.payload.timestampX,c.timestampY=n.payload.timestampY);let h=this.ctx.entityStates.get(n.payload.entity.entityId)?.entityPicture;return h!==void 0&&(c.url=h),c}case"tap":{let c={kind:"tap",...p,frame:n.payload.frame,opacity:1,action:n.payload.action};return n.payload.openPageId!==void 0&&(c.openPageId=n.payload.openPageId),n.payload.attachedTo!==void 0&&(c.attachedTo=n.payload.attachedTo),c}}}resolveLayout(n,t,i){let a=n.perFamily[t];this.settleCharts(n);let r=Sr(n,t).map(x=>this.resolveElement(x,i)),o=a?this.applyRules(a.rules,i):new Map,l={family:t,elements:r,cornerBodyShape:a?.cornerBodyShape??"wedge",borderWidth:this.styleNumber(o,"borderWidth")??a?.borderWidth??2},s=this.styleText(o,"text"),d=a?.bezelCountdown&&s===void 0?this.countdownEnd(a.bezelText):void 0,p=a?.bezelCountdown?this.countdownFallbackText(a.bezelText):void 0,c=s??p??this.resolve(a?.bezelText);c!==void 0&&(l.bezelText=c),d!==void 0&&(l.bezelCountdownEnd=d);let h=this.resolve(a?.curvedText);if(h!==void 0&&(l.curvedText=h),a?.curvedColorHex!==void 0&&(l.curvedColorHex=a.curvedColorHex),a?.bezelGauge){let x=a.bezelGauge,k=this.resolve(x.value),S=k===void 0?void 0:Ct(k);if(S!==void 0){let b=Math.min(x.minValue,x.maxValue),f=Math.max(x.minValue,x.maxValue),w={value:Math.min(f,Math.max(b,S)),minValue:b,maxValue:f===b?b+1:f,colorHexes:x.colorHexes},R=this.resolve(x.minLabel);R!==void 0&&(w.minLabel=R);let _=this.resolve(x.maxLabel);_!==void 0&&(w.maxLabel=_),l.bezelGauge=w}}let g=this.styleColor(o,"backgroundColor")??a?.backgroundColorHex;g!==void 0&&(l.backgroundColorHex=g);let y=this.styleColor(o,"borderColor")??a?.borderColorHex;return y!==void 0&&(l.borderColorHex=y),l}};function Al(e,n,t){let i=new Ve(n,t),a=e.countdown?i.countdownEnd(e.value):void 0,o={text:(e.countdown?i.countdownFallbackText(e.value):void 0)??i.resolve(e.value)??"--"};return e.label&&(o.label=e.label),e.symbol&&(o.symbol=e.symbol),a!==void 0&&(o.countdownEnd=a),o}function Oi(e,n,t){let i=new Ve(n),a={};for(let r of["rectangular","circular","corner"])e.supportedFamilies.includes(r)&&(a[r]=i.resolveLayout(e,r,t));return e.supportedFamilies.includes("inline")&&e.inline&&(a.inline=Al(e.inline,n,e)),a}var be=de,qt=[{label:"40 mm",screen:{width:162,height:197},slots:{rectangular:{width:141,height:51},circular:{width:40,height:40},corner:{width:26,height:26}},measured:!1},{label:"41 mm",screen:{width:176,height:215},slots:{rectangular:{width:153,height:55.5},circular:{width:43,height:43},corner:{width:29,height:29}},measured:!1},{label:"42 mm",screen:{width:187,height:223},slots:{rectangular:{width:163,height:59},circular:{width:46,height:46},corner:{width:31,height:31}},measured:!1},{label:"44 mm",screen:{width:184,height:224},slots:{rectangular:{width:160,height:58},circular:{width:45,height:45},corner:{width:30,height:30}},measured:!1},{label:"45 mm",screen:{width:198,height:242},slots:{rectangular:{width:172,height:62.5},circular:{width:48.5,height:48.5},corner:{width:32,height:32}},measured:!1},{label:"46 mm",screen:{width:208,height:248},slots:be,measured:!0},{label:"49 mm",screen:{width:205,height:251},slots:{rectangular:{width:178.5,height:64.5},circular:{width:50,height:50},corner:{width:33.5,height:33.5}},measured:!1}],Yt=qt.find(e=>e.measured);function jr(e){if(!e)return;let n=/^(\d+)x(\d+)$/.exec(e.trim());if(!n)return;let t=Number(n[1]),i=Number(n[2]);return qt.find(a=>a.screen.width===t&&a.screen.height===i)}function Rn(e,n){let t=be[n];if(e.width<=0||e.height<=0)return{scale:0,x:0,y:0,width:0,height:0};let i=Math.min(e.width/t.width,e.height/t.height),a=t.width*i,r=t.height*i;return{scale:i,x:(e.width-a)/2,y:(e.height-r)/2,width:a,height:r}}var Hl={regular:400,medium:500,semibold:600,bold:700};function Ae(e){if(!e)return;let n=e.startsWith("#")?e.slice(1):e;if(!/^[0-9a-fA-F]{6}([0-9a-fA-F]{2})?$/.test(n))return;let t=n.length===8?parseInt(n.slice(6,8),16)/255:1;return{color:`#${n.slice(0,6)}`,opacity:t}}function Be(e,n,t="#FFFFFF"){let i=Ae(e)??{color:t,opacity:1};return{[n]:i.color,[`${n}-opacity`]:i.opacity}}function qr(e,n){let t=Math.max(0,e.frame.width*n.width),i=Math.max(0,e.frame.height*n.height),a=(e.frame.x+e.frame.width/2)*n.width,r=(e.frame.y+e.frame.height/2)*n.height;return{x:a-t/2,y:r-i/2,w:t,h:i,cx:a,cy:r}}function Ll(e,n){let t=Be(e.colorHex,"fill");e.countdownEnd!==void 0&&e.countdownEnd>Date.now()&&(e={...e,text:St((e.countdownEnd-Date.now())/1e3)});let i=s=>s*.55,a=e.text.length*i(e.fontSize),r=a>n.w&&n.w>0?Math.max(.5,n.w/a):1,o=e.fontSize*r,l=e.text;if(n.w>0&&l.length*i(o)>n.w){let s=n.w-.8*o,d=Math.max(1,Math.floor(s/i(o)));l=`${l.slice(0,d).replace(/\s+$/,"")}\u2026`}return v`<text x=${n.cx} y=${n.cy} text-anchor="middle" dominant-baseline="central"
    ...
    ${o!==void 0?v`<text x=${n.cx} y=${n.cy} text-anchor="middle" dominant-baseline="central"
        ...
        stroke="#0A84FF" stroke-width="1" vector-effect="non-scaling-stroke" pointer-events="none" />`:m,b=v`<rect x=${s.x} y=${s.y} width=${s.w} height=${s.h} fill="transparent" stroke="none" />`,f=3,w=g&&x?[["nw",s.x,s.y],["ne",s.x+s.w,s.y],["sw",s.x,s.y+s.h],["se",s.x+s.w,s.y+s.h]].map(([R,_,U])=>v`<rect data-handle=${R} x=${_-f/2} y=${U-f/2} width=${f} height=${f}
    ...
    ${e.maxLabel!==void 0?k(`${t}-gmax`,d+x,d+x+Math.max(l,3),e.maxLabel):m}`}function Wi(e,n){let t=e.family in be?e.family:"rectangular",i=n.slot??be[t],a=be[t],r=Rn(i,t),o=`clip-${t}-${Math.random().toString(36).slice(2,8)}`,l=Ae(e.backgroundColorHex),s=Ae(e.borderColorHex),d=e.borderWidth*r.scale;if(t==="corner"){let y=r.scale,x=!!e.bezelText||!!e.bezelGauge,k=e.curvedText??"",S=k!=="",b=An(y,x),f=Ki(y,x),w=f/(a.width*y),R=b.tile.cx-f/2,_=b.tile.cy-f/2,U=`M 0 0 H ${b.quad.width-b.cornerRadius} A ${b.cornerRadius} ${b.cornerRadius} 0 0 1 ${b.quad.width} ${b.cornerRadius} V ${b.quad.height} H 0 Z`,ye=m;if(e.bezelGauge)ye=Jl(e.bezelGauge,y,o);else if(e.bezelText){let O=Wl(y,`${o}-bezel`),$=e.bezelCountdownEnd!==void 0&&e.bezelCountdownEnd>Date.now()?St((e.bezelCountdownEnd-Date.now())/1e3):e.bezelText;ye=v`<defs><path id=${O.id} d=${O.d} /></defs>
    ...
    <span class="sym-name">${n}</span>
      ...
      <div class="sym-controls">
        <input type="search" placeholder="Search symbols" .value=${o} @input=${he(f=>a.setQuery(i,f))} />
        <select @change=${he(f=>a.setCategory(i,f))}>
    ...
    <label class="field"><span>Symbol</span>
      <input type="text" class="mono" .value=${n} placeholder="lightbulb.fill"
        @input=${he(t)} @change=${he(y=>{(d.size===0||d.has(y.trim()))&&a.noteUsed(y)})} /></label>
    ...
    <div class="grid2">
      ...
      ${Q("Case",t.textCase??"",[["","As is"],["upper","UPPER"],["lower","lower"],["capitalized","Capitalized"]],a=>i({textCase:a||void 0}))}
    ...
    ${Q("Over",o.kind,[["filter","Entities matching a filter"],["entities","A fixed list"]],l=>t({...n,scope:l==="entities"?{kind:"entities",entities:[]}:{kind:"filter",domains:[],areaIds:[],labelIds:[],floorIds:[]}}))}
    ${o.kind==="filter"?u`<div class="grid2">
          ${ue("Domains",a(o.domains),l=>t({...n,scope:{...o,domains:r(l)}}),{placeholder:"light, switch"})}
          ${ue("Area ids",a(o.areaIds),l=>t({...n,scope:{...o,areaIds:r(l)}}))}
          ${ue("Label ids",a(o.labelIds),l=>t({...n,scope:{...o,labelIds:r(l)}}))}
          ${ue("Floor ids",a(o.floorIds),l=>t({...n,scope:{...o,floorIds:r(l)}}))}
    ...
    ${n.stateFilter&&"value"in n.stateFilter?ue("State",n.stateFilter.value,l=>t({...n,stateFilter:{kind:n.stateFilter.kind,value:l}})):m}
    ${n.function==="count"?m:ue("Attribute (blank = state)",n.attribute??"",l=>{let s={...n};l?s.attribute=l:delete s.attribute,t(s)})}`}var ns=bi,jd=ns.filter(([e])=>e!=="none");function qd(e,n){return e!==void 0&&n.trim()!==""&&n.trim()!==e.trim()}function is(e){let n=e.config,t=n.tapAction,i=s=>["toggleEntity","runScene","runScript","addTodo","runHTTPAction"].includes(s),a=qd(e.savedName,n.name),r=n.refreshMinutes??0,o=No.map(s=>[String(s),Do(s)]);No.includes(r)||o.push([String(r),Do(r)]);let l=n.showSuccessFlash??!0;return u`
    <div class="gen-row">
      ${ue("Name",n.name,s=>e.update(d=>{d.name=s},"name"))}
      ...
      <div class="field flash-cell"><span>Flash when a tap works</span>
        <div class="flash-row">
          ...
          ${l?u`<input type="color" class="flash-color" title="Flash colour. Click to change it." .value=${(n.successFlashColorHex??Yd).slice(0,7)}
                @input=${he(s=>e.update(d=>{d.successFlashColorHex=s.toUpperCase()},"flash"))} />`:u`<span class="muted">Off</span>`}
        </div>
      </div>
    </div>
  ...
  ${a?m:u`<div class="hint">Without a page the tap falls back to the complication list.</div>`}`}function rs(e,n){let t=e.config.values.findIndex(a=>a.id===n.id),i=`nv-${n.id}`;return u`
    ...
    ${t?u`
      ...
      ${i?m:Q("Corner",e.timestampCorner,[["topLeading","Top left"],["topTrailing","Top right"],["bottomLeading","Bottom left"],["bottomTrailing","Bottom right"]],r=>n(o=>{o.timestampCorner=r}))}
      ${Z("Text size (pt)",e.timestampSize,r=>n(o=>{o.timestampSize=Math.min(40,Math.max(4,r??Bt))},"tssize"),{step:1,min:4,max:40})}
      ...
      <div class="hint">The time the snapshot was fetched, not the time now. A frame that stops updating keeps its old time.</div>`:m}`}function fe(e,n,t,i,a={}){let r=e.openSections.has(n),o=()=>e.toggleSection(n);return u`<section class="sec" data-open=${r?"true":"false"} style=${a.color?`--c:${a.color}`:""}>
    ...
    ${r?u`<div class="sec-b">${i}</div>`:m}
        ...
        ${n.payload.countdown?u`<div class="hint">Ticks down to the value's target: an active timer's finish, or any future timestamp. A paused timer shows its remaining time.</div>`:m}`,c=u`<div class="grid2">
          ...
          ${Q("Weight",n.payload.fontWeight,pa,f=>o(w=>{w.payload.fontWeight=f}))}
        </div>`;break}case"icon":p=u`
        ${ae(e,n.payload.symbol,b=>o(f=>{f.payload.symbol=b},"symbol"),{noFormat:!0,showResolved:!0,symbol:!0,label:"Symbol",key:`${r}-symbol`})}
        ...
        ${ae(e,n.payload.value,b=>o(f=>{f.payload.value=b},"value"),{showResolved:!0,label:"Reading",key:`${r}-value`})}
        <div class="grid2">
          ${Z("Min",n.payload.minValue,b=>o(f=>{f.payload.minValue=b??0},"min"))}
          ${Z("Max",n.payload.maxValue,b=>o(f=>{f.payload.maxValue=b??100},"max"))}
        </div>`,c=u`
        <div class="grid2">
          ${Q("Style",n.payload.style,[["arc","Arc"],["ring","Ring"],["bar","Bar"]],b=>o(f=>{f.payload.style=b}),{titles:{arc:"A 270\xB0 arc, open at the bottom",ring:"A full circle",bar:"A straight bar"}})}
          ${Bn(e,n,t,"Line width",{step:.5,min:.5})}
        </div>
        ${pe("Track colour",n.payload.trackColorHex,b=>o(f=>{f.payload.trackColorHex=b??"#FFFFFF40"},"track"))}`;break;case"chart":{let b=n.payload,f=(T,C)=>o(B=>T(B.payload),C),w=rt(b),R=b.historyMinutes>0,_=b.value.kind.kind==="entityState",U=w===void 0?void 0:e.historySeries(w),ye=R&&_?U??"":e.resolve(b.value)??"",E=b.historyPoints<1,O=gn.some(T=>T.minutes===b.historyMinutes),$=oa.has(i)||!O,A=Math.floor(b.historyMinutes/1440),J=Math.floor(b.historyMinutes%1440/60),P=b.historyMinutes%60,ee=(T,C,B)=>f(D=>{D.historyMinutes=Math.min(cr,Math.max(1,Math.round(T)*1440+Math.round(C)*60+Math.round(B)))},"span"),Y=jt(ye),te=b.limit>0&&Y.length>b.limit?b.takeFromEnd?Y.slice(Y.length-b.limit):Y.slice(0,b.limit):Y,Xe=!R&&_&&Y.length===1;p=u`
        ${ae(e,b.value,T=>f(C=>{C.value=T},"value"),{label:"Readings",key:`${r}-value`})}
        ${Q("Draw",R?"history":"value",[["history","Recorded history"],["value","The value itself"]],T=>f(C=>{C.historyMinutes=T==="history"?C.historyMinutes||yn:0}),{titles:{history:"Read the entity's past from the recorder and plot it",value:"Plot the numbers the value holds right now, such as a forecast list"}})}
        ${R?u`
            ...
            <div class="grid2">
              <label class="field"><span>Span</span>
                <select @change=${T=>{let C=T.target.value;C==="custom"?(oa.add(i),Se(T.target)):(oa.delete(i),f(B=>{B.historyMinutes=Number(C)||yn}))}}>
              ...
              <div class="field readings-field"><span>Readings</span>
                <div class="readings-row">
                  ...
                  ${E?m:u`<input type="number" class="short" aria-label="How many readings" .value=${String(b.historyPoints)}
                    ...
                    @input=${he(T=>{let C=Number(T);T.trim()!==""&&Number.isFinite(C)&&C>=1&&f(B=>{B.historyPoints=Math.round(C)},"hpoints")})} />`}
                </div>
              </div>
            </div>
            ${$?u`<div class="grid3 span-parts">
                ${Z("Days",A,T=>ee(T??0,J,P),{step:1,min:0,max:7})}
                ${Z("Hours",J,T=>ee(A,T??0,P),{step:1,min:0,max:23})}
                ${Z("Minutes",P,T=>ee(A,J,T??0),{step:1,min:0,max:59})}
              </div>
            ...
            ${_&&U===void 0?u`<div class="hint">Reading the history…</div>`:m}
            ${_&&U===""?u`<div class="hint warn">Nothing recorded for this entity in that span.
        ...
        ${Y.length===0&&!(R&&(!_||U===void 0||U===""))?u`<div class="hint warn">No numbers in this value yet, so the chart draws nothing.</div>`:m}
        ${Y.length>0?u`<div class="hint">Reads <span class="nums">${ac(te)}</span>${Y.length===te.length?u` · ${te.length} ${te.length===1?"value":"values"}`:u` · ${te.length} of ${Y.length}`}</div>`:m}
        ...
        <div class="grid2">
          ${Z("Use",b.limit,T=>f(C=>{C.limit=Math.max(0,Math.round(T??0))},"limit"),{step:1,min:0})}
          ${Q("From",b.takeFromEnd?"end":"start",[["start","The first"],["end","The last"]],T=>f(C=>{C.takeFromEnd=T==="end"}))}
        </div>
        ...
        <div class="grid2">
          ${Q("Style",b.style,Ad,T=>f(C=>{C.style=T}))}
          ${b.style==="bars"?Z("Bar gap (pt)",b.barGap,T=>f(C=>{C.barGap=Math.max(0,T??0)},"gap"),{step:.5,min:0}):Bn(e,n,t,"Line width",{step:.5,min:.5})}
        </div>
        <div class="grid2">
          ${Q("Scale",b.scale,Hd,T=>f(C=>{C.scale=T}),{titles:{auto:"The plot stretches to fit the readings it has",fixed:"The plot always runs from Min to Max"}})}
          ${Q("Baseline",b.baseline,Ld,T=>f(C=>{C.baseline=T}))}
        </div>
        ${b.scale==="fixed"?u`<div class="grid2">
              ${Z("Min",b.minValue,T=>f(C=>{C.minValue=T??0},"cmin"))}
              ${Z("Max",b.maxValue,T=>f(C=>{C.maxValue=T??100},"cmax"))}
            </div>`:m}
        ...
        ${Q("Colour",b.coloring,_d,T=>f(C=>{C.coloring=T,T==="bands"&&C.bands.length===0&&(C.bands=Pd(te))}))}
        ${b.coloring==="bands"?u`
          ...
          ${b.bands.map((T,C)=>u`
            <div class="row-inline">
              ${Z("Up to",T.upTo,B=>f(D=>{let oe=D.bands[C];oe&&(oe.upTo=B??0)},`bup${T.id}`))}
              ${pe("Colour",T.colorHex,B=>f(D=>{let oe=D.bands[C];oe&&(oe.colorHex=B??"#FFFFFF")},`bcol${T.id}`))}
              <button class="icon" title="Remove this band" aria-label="Remove this band"
                @click=${()=>f(B=>{B.bands=B.bands.filter((D,oe)=>oe!==C)})}>${z("close")}</button>
            </div>`)}
          <button class="small" @click=${()=>f(T=>{T.bands=[...T.bands,Od(T)]})}>Add band</button>
          ${pe("And the rest",b.bandAboveColorHex,T=>f(C=>{C.bandAboveColorHex=T??fn},"babove"))}
        ...
        <div class="grid2">
          ${Q("Highlight",b.highlight,Zo,T=>f(C=>{C.highlight=T}))}
          ${b.highlight==="none"?m:Q("Marker",b.marker,zd,T=>f(C=>{C.marker=T}))}
        </div>
        ${b.highlight==="none"?m:u`
          <div class="grid2">
            ${b.highlight==="lowest"?m:pe("Highest colour",b.highColorHex,T=>f(C=>{C.highColorHex=T??hn},"hicol"))}
            ${b.highlight==="highest"?m:pe("Lowest colour",b.lowColorHex,T=>f(C=>{C.lowColorHex=T??mn},"locol"))}
          </div>
          ...
          ${Q("Shape",n.payload.kind,[["roundedRectangle","Rounded"],["rectangle","Rectangle"],["capsule","Capsule"],["circle","Circle"]],b=>o(f=>{f.payload.kind=b}),{titles:{roundedRectangle:"Rounded rectangle"}})}
        ...
        ${pe("Border colour",n.payload.borderColorHex,b=>o(f=>{b===void 0?delete f.payload.borderColorHex:f.payload.borderColorHex=b},"border"),!0)}
        ${n.payload.borderColorHex!==void 0?Z("Border width (pt)",n.payload.borderWidth,b=>o(f=>{f.payload.borderWidth=b??1},"bw"),{step:.5,min:0}):m}`;break;case"image":{let b=n.payload,f=(w,R)=>o(_=>w(_.payload),R);p=u`
        ...
        ${Q("Picture",b.contentMode,[["fill","Fill the frame"],["fit","Fit inside"]],w=>f(R=>{R.contentMode=w}),{titles:{fill:"Cover the frame, cropping what does not fit",fit:"Show the whole picture, with space around it"}})}
        ...
        ${Z("Corner radius (pt)",b.cornerRadius,w=>f(R=>{R.cornerRadius=Math.max(0,w??Vt)},"imgradius"),{step:1,min:0})}`;break}case"tap":{p=u`
        ...
        <div class="hint">To make one layer tappable, tick Tappable on that layer instead.</div>`;break}}let h=n.kind==="image"||n.kind==="tap"?void 0:pe(n.kind==="shape"?"Fill colour":"Colour",n.payload.colorSlot.baseColorHex,b=>o(f=>{f.kind!=="image"&&f.kind!=="tap"&&(f.payload.colorSlot.baseColorHex=b??"#FFFFFF")},"color")),g=Si(e.config,n),y=g?{kind:{kind:"entityState",...g}}:void 0,x=ie[n.kind],k=n.kind==="tap"?void 0:$e(e.config,i)[0],S=n.kind==="image"?n.payload.timestamp===!0:!1;return u`
    ${fe(e,"content","Content",u`${n.kind==="tap"?m:Zd(e,n,r)}${p}`,{color:x,icon:"content",summary:ua(e,n)})}
    ${c===void 0&&h===void 0?m:fe(e,"look",n.kind==="image"?"Picture":"Look",u`${c??m}${h??m}`,{color:x,icon:n.kind==="image"?"image":"look",...Wn(n)?{summary:Wn(n)}:{}})}
    ${n.kind==="chart"?fe(e,"numbers","Numbers",lc(e,n),{color:ie.text,icon:"text",summary:sc(e,n)}):m}
    ${n.kind==="image"?fe(e,"timestamp","Timestamp",ic(n.payload,(b,f)=>o(w=>b(w.payload),f)),{color:x,icon:"clock",summary:S?`Shown \xB7 ${n.payload.timestampSize} pt`:"Hidden"}):m}
    ${n.kind==="tap"?m:fe(e,"tappable","Tap",dc(e,n,r),{color:q.tap,icon:"tap",summary:k?Pe(k.payload.action):"Not tappable"})}
    ${fe(e,"states","States",vs(e,n.payload.rules,n.kind,b=>b.elements.find(f=>f.payload.id===i)?.payload.rules,`rules-${i}`,y),{color:q.states,icon:"states",summary:Jt(n.payload.rules).replace(/\.$/,"")})}
    ${fe(e,"placement","Place",u`
      ...
      <div class="hint">Everything about where this layer sits, how big it is drawn and whether it
        ...
        the same layer differently there.</div>`,{color:q.place,icon:"place",summary:`${Math.round(s.width*100)}% wide \xB7 ${N(t)}${l.fromPlacement?"":" \xB7 shared frame"}`})}`}function us(e,n,t,i){let a=n.action,r=o=>["toggleEntity","runScene","runScript","addTodo","runHTTPAction"].includes(o);return u`
    ...
    ${a.type==="openPage"?as(e,n.openPageId,n.openPageName,(o,l)=>t(s=>{if(o===void 0){delete s.openPageId,delete s.openPageName;return}s.openPageId=o,l?s.openPageName=l:delete s.openPageName},"tap-page")):m}`}var rc=24;function oc(e,n){let t=[],i=1/0;for(let r of X){if(r==="inline"||!e.config.supportedFamilies.includes(r))continue;let o=Fr(e.config,n,r);o&&(t.push(`${N(r)} ${Math.round(o.width)} x ${Math.round(o.height)} pt`),i=Math.min(i,o.width,o.height))}if(t.length===0)return m;let a=i<rc;return u`<div class=${a?"hint warn":"hint"}>${t.join(" \xB7 ")}${a?u`<br />That is small for a wrist. Show the tap area and drag its corners out.`:m}</div>`}function sc(e,n){let t=Gt(e.config,n.payload.id);return t.length===0?"None yet":t.map(i=>{let a=i.payload.value.kind;return a.kind==="chartStat"?(wt.find(([r])=>r===a.stat)?.[1]??"number").toLowerCase():"number"}).join(" \xB7 ")}function lc(e,n){let t=me(e),i=Gt(e.config,n.payload.id),a=o=>e.update(l=>{xr(l,n.payload.id,o)}),r=new Set(i.map(o=>o.payload.value.kind.kind==="chartStat"?o.payload.value.kind.stat:""));return u`
    ...
    </div>`,{color:q.group,icon:"folder",summary:`${t.length} layers \xB7 ${n.locked?"moves as one":"unlocked"}`})}function ms(e,n){if(n==="inline")return u`${cc(e)}${sa(e,n)}`;let t=e.config.perFamily[n];if(!t)return u`<div class="hint">No settings stored for ${N(n)} yet.</div>
    ...
    ${fe(e,"look",`${N(n)} shape`,u`
      ${pe("Background (blank = transparent)",t.backgroundColorHex,l=>i(s=>{l===void 0?delete s.backgroundColorHex:s.backgroundColorHex=l},"bg"),!0)}
      ${pe("Border colour",t.borderColorHex,l=>i(s=>{l===void 0?delete s.borderColorHex:s.borderColorHex=l},"border"),!0)}
      ${Z("Border width (pt)",t.borderWidth,l=>i(s=>{s.borderWidth=l??2},"bw"),{step:.5,min:0})}`,{color:q.place,icon:"shape",summary:`${r} \xB7 ${o}`})}
    ${n==="corner"?fe(e,"corner","Corner content",pc(e,t,i),{color:q.place,icon:"content",summary:t.curvedText?"Big curved text":"Layer canvas"}):m}
    ${fe(e,"states","Shape states",vs(e,t.rules,"layout",l=>l.perFamily[n]?.rules,`rules-${n}`),{color:q.states,icon:"states",summary:Jt(t.rules).replace(/\.$/,"")})}
    ${fe(e,"placements","Placements",u`
    ...
    ${fe(e,"content","Inline text",u`
      ...
      ${n.countdown?u`<div class="hint">Ticks down to the value's target: an active timer's finish, or any future timestamp. A paused timer shows its remaining time.</div>`:m}`,{color:ie.text,icon:"text",summary:ht(`${n.label?`${n.label}: `:""}${ge(n.value,i)}`,48)})}
    ${fe(e,"symbol","Symbol",u`
      ...
      <div class="hint">On the face: ${n.symbol?`${n.symbol} `:""}${n.label?`${n.label}: `:""}${e.resolve(n.value)??"--"}</div>`,{color:ie.icon,icon:"icon",summary:n.symbol||"None"})}`}function pc(e,n,t){let i=n.curvedText?"curved":"canvas",a=n.bezelGauge?"gauge":n.bezelText?"text":"none";return u`
    ${Q("Main content",i,[["canvas","Layer canvas"],["curved","Big curved text"]],r=>t(o=>{r==="curved"?o.curvedText||(o.curvedText=M("Text")):(delete o.curvedText,delete o.curvedColorHex)}))}
    ...
    `:m}
    ${Q("Bezel",a,[["none","None"],["text","Text label"],["gauge","Gauge arc"]],r=>t(o=>{r==="text"?(delete o.bezelGauge,o.bezelText||(o.bezelText=M("Label"))):r==="gauge"?(delete o.bezelText,o.bezelGauge||(o.bezelGauge={value:M("50"),minValue:0,maxValue:100,colorHexes:["#34C759","#FFCC00","#FF3B30"]})):(delete o.bezelText,delete o.bezelGauge)}))}
    ...
    <div class="row-inline">
      ${Q("When",n.when.join,[["all","All of these are true"],["any","Any of these is true"]],d=>l(p=>{p.when.join=d}))}
    </div>
  ...
  </div>`}function xc(e,n,t,i,a,r){let o=(c,h)=>i(c,h?`${r}-${h}`:void 0),l=n.comparison,s=ot(l.kind),d=e.evaluateTest(n),p=m;switch(s){case"value":p=ae(e,l.value??M(""),c=>o(h=>{h.comparison.value=c},"rhs"),{showResolved:!0,label:"Compare with",key:`${r}-rhs`});break;case"between":p=u`${ae(e,l.value??M(""),c=>o(h=>{h.comparison.value=c},"rhs"),{showResolved:!0,label:"Lower bound",key:`${r}-rhs`})}
        ${ae(e,l.upper??M(""),c=>o(h=>{h.comparison.upper=c},"upper"),{showResolved:!0,label:"Upper bound",key:`${r}-upper`})}`;break;case"pattern":p=u`${ue("Pattern",l.pattern??"",c=>o(h=>{h.comparison.pattern=c},"pattern"),{mono:!0,placeholder:"^on$"})}
        ${l.pattern&&!kc(l.pattern)?u`<div class="hint warn">This pattern does not compile. The test fails until it does.</div>`:m}`;break;case"options":p=ue("Options (comma separated)",(l.options??[]).join(", "),c=>o(h=>{h.comparison.options=c.split(",").map(g=>g.trim()).filter(Boolean)},"options"));break;case"none":break}return u`<div class="test-box">
    <div class="rule-head">
    ...
    ${l.kind==="isStale"?u`<div class="hint">True when the watch's cached values are older than the staleness limit. The value below is not read.</div>`:ae(e,n.value,c=>o(h=>{h.value=c},"lhs"),{showResolved:!0,label:"Value",key:`${r}-lhs`})}
    ...
    ${bs(e,n,a,o)}
  </div>`}function bs(e,n,t,i){let a=En(n.kind),r=m;if(a==="value"){let o=n.value??M("");if(ys.includes(n.kind)){let l=o.kind.kind==="literal";r=u`${l?pe("Colour",o.kind.kind==="literal"?o.kind.value:"",s=>t(d=>{d.value=M(s??"#FFFFFF")},"color")):ae(e,o,s=>t(d=>{d.value=s},"value"),{noFormat:!0,showResolved:!0,label:"Colour from",key:`${i}-value`})}
        ...
        ${l?m:u`<div class="hint">The value must resolve to a hex colour such as <code>#FF9F0A</code>. Empty or invalid results leave the colour unchanged.</div>`}`}else r=ae(e,o,l=>t(s=>{s.value=l},"value"),{noFormat:n.kind==="setIcon",symbol:n.kind==="setIcon",showResolved:!0,label:n.kind==="setIcon"?"Symbol":"To",key:`${i}-value`})}else if(a==="number"){let o=n.kind==="setOpacity"?{step:.05,min:0,max:1}:n.kind==="setRotation"?{step:1}:{step:.5,min:0};r=Z(n.kind==="setOpacity"?"Opacity (0 to 1)":n.kind==="setRotation"?"Degrees":n.kind==="setFontSize"?"Points":"Value",n.number??0,l=>t(s=>{s.number=l??0},"number"),o)}else a==="weight"&&(r=Q("Weight",n.weight??"regular",pa,o=>t(l=>{l.weight=o})));return r}var la=new Set,Un=new Map,Kn=new Map,Uo=new Map;function vs(e,n,t,i,a,r){let o=Zi(n);return!o.ok||la.has(a)?u`
      ...
      ${bc(e,n,t,i,a)}`:Cc(e,o.table,n[0],t,i,a,r)}function Cc(e,n,t,i,a,r,o){let l=(E,O)=>e.update($=>{let A=a($);A&&E(A)},O?`${r}-${O}`:void 0),s=n.value??Uo.get(r)??o,d=n.rows.length===0,p=n.numberMode||d&&s!==void 0&&!So(s)&&Sc(e.resolve(s)),c=Sn[i],h=Un.get(r)??new Set,g=n.columns.length===0&&h.size===0?[Co[i]]:[],y=fo(n.columns,[...h,...g.filter(E=>E!==void 0)],c),x=t?e.liveBranch(t):"none",k=t?e.forced.get(t.id)??"live":"live",S=E=>k!=="live"&&(k==="otherwise"?E==="otherwise":k.caseId===E),b=E=>{t&&e.setForced(t.id,S(E)?"live":E==="otherwise"?"otherwise":{caseId:E})},f=E=>{Uo.set(r,E),n.rows.length!==0&&l(O=>xo(O,E),"lhs")},w=()=>l(E=>vo(E,s??M(""),p)),R=n.rows.map((E,O)=>Wo(e,{key:`${r}-${E.caseId}`,label:$o(E.comparison,$=>ge($,me(e))),columns:y,changes:E.changes,live:x===E.caseId,forced:S(E.caseId),onForce:()=>b(E.caseId),when:Mc(e,E.comparison,`${r}-${E.caseId}`,($,A)=>l(J=>{let P=J[0]?.cases.find(ee=>ee.id===E.caseId)?.when.tests[0];P&&$(P.comparison)},A&&`${E.caseId}-${A}`)),updChanges:($,A)=>l(J=>{let P=J[0]?.cases.find(ee=>ee.id===E.caseId);P&&$(P.then)},A&&`${E.caseId}-${A}`),acts:u`
      ...
      <button class="icon danger" title="Delete this state" @click=${()=>l($=>wo($,E.caseId))}>${z("delete")}</button>`})),_=n.otherwise===void 0?m:Wo(e,{key:`${r}-otherwise`,label:"Otherwise",columns:y,changes:n.otherwise,live:x==="otherwise",forced:S("otherwise"),onForce:()=>b("otherwise"),when:u`<span class="when-otherwise">Otherwise</span>`,updChanges:(E,O)=>l($=>{let A=$[0]?.otherwise;A&&E(A)},O),acts:u`<button class="icon" title="Remove the Otherwise row" @click=${()=>l(E=>ea(E,!1))}>${z("close")}</button>`}),U=Kn.get(r),ye=Ec.filter(E=>c.includes(E)&&!y.includes(E));return u`
    <div class="states">
      ${ae(e,s??M(""),f,{label:"Testing",showResolved:!0,key:`${r}-lhs`})}
      ...
      ${U===void 0?m:u`<div class="hint warn confirm-row">
        Remove the ${Ge[U]} column? Its ${Ko(n,U)} value${Ko(n,U)===1?"":"s"} are deleted from every state.
        <button class="danger small" @click=${E=>{Kn.delete(r),Un.get(r)?.delete(U),Se(E.target),l(O=>ko(O,U))}}>Remove</button>
      ...
      <div class="states-foot">
        ...
        ${ye.length===0?m:u`<select class="chip-add" title="Add a column" @change=${E=>{let O=E.target,$=O.value;if(O.value="",!$)return;let A=Un.get(r)??new Set;A.add($),Un.set(r,A),Se(O)}}>
          <option value="" selected>+ column…</option>
          ${ye.map(E=>u`<option value=${E}>${Ge[E]}</option>`)}
        </select>`}
      </div>
    ...
    <div class="value-pop" id=${o} popover role="dialog" aria-label=${s} @toggle=${es}>
      ...
      ${Qt.has(o)?u`${n==="visibility"?Q("This state",r.kind==="hide"?"hide":"show",[["show","Shown"],["hide","Hidden"]],d=>l(p=>{p.kind=d})):bs(e,r,l,a)}
      ...
      <span class="cell-word">${a?Te(a):ge(i,me(e))}</span>`;if(n.kind==="setIcon"&&a){let r=e.icons.render(a,16,"#FFFFFF");return u`${r??m}<span class="cell-word">${a}</span>`}return u`<span class="cell-word">${ge(i,me(e))}</span>`}function Te(e){return{"#FF453A":"red","#FF9F0A":"orange","#FFD60A":"amber","#34C759":"green","#30D158":"green","#0A84FF":"blue","#64D2FF":"cyan","#BF5AF2":"purple","#FFFFFF":"white","#8E8E93":"grey","#000000":"black","#FFCC00":"amber","#FF3B30":"red"}[e.toUpperCase()]??e}function Mc(e,n,t,i){let a=ot(n.kind),r=Xi(n.kind),o=(l,s,d,p)=>Ac(e,l,s,`${t}-${d}`,r,p,d==="rhs"?"Compare with":"Upper bound");return u`<span class="when-cell">
    <select class="when-op" title="How this state is decided" @change=${he(l=>i(s=>{let d=Hi(s,l);s.kind=d.kind,d.value!==void 0?s.value=d.value:delete s.value,d.upper!==void 0?s.upper=d.upper:delete s.upper}))}>
    ...
    :host {
      ...
      --wa-text: ${se(ie.text)};
      --wa-icon: ${se(ie.icon)};
      --wa-gauge: ${se(ie.gauge)};
      --wa-shape: ${se(ie.shape)};
      --wa-image: ${se(ie.image)};
      --wa-tap: ${se(ie.tap)};
    ...
    * { box-sizing: border-box; }
    ...
    .layer.held {
      border-color: color-mix(in srgb, ${se(q.group)} 70%, var(--wa-line));
      background: color-mix(in srgb, ${se(q.group)} 10%, var(--wa-panel));
    }
    .layer.held .thumb { border-color: color-mix(in srgb, ${se(q.group)} 60%, var(--wa-line)); }
    ...
    .layer .lockbtn.on { opacity: 1; color: ${se(q.locked)}; filter: drop-shadow(0 0 4px ${se(q.locked)}); }
  ...
  `}connectedCallback(){super.connectedCallback(),this.loadColumnWidths(),this.loadListView(),this.sizeObserver.observe(this),window.addEventListener("keydown",this.keyHandler),window.addEventListener("keyup",this.keyUpHandler),window.addEventListener("beforeunload",this.beforeUnload),this.loadOwners()}loadColumnWidths(){try{let t=window.localStorage.getItem(Ls);if(!t)return;let i=JSON.parse(t);typeof i.left=="number"&&(this.colLeft=va(i.left)),typeof i.right=="number"&&(this.colRight=va(i.right))}catch{}}saveColumnWidths(){try{window.localStorage.setItem(Ls,JSON.stringify({left:this.colLeft,right:this.colRight}))}catch{}}loadListView(){try{let t=window.localStorage.getItem(Hs);if(!t)return;let i=JSON.parse(t);(i.thumbStep===0||i.thumbStep===1||i.thumbStep===2)&&(this.thumbStep=i.thumbStep),(i.detail==="compact"||i.detail==="expanded")&&(this.layerDetail=i.detail),typeof i.addOpen=="boolean"&&(this.addOpen=i.addOpen),(i.addDetail==="compact"||i.addDetail==="expanded")&&(this.addDetail=i.addDetail)}catch{}}saveListView(){try{window.localStorage.setItem(Hs,JSON.stringify({thumbStep:this.thumbStep,detail:this.layerDetail,addOpen:this.addOpen,addDetail:this.addDetail}))}catch{}}renderGutter(t){return u`<div class="gutter ${t}" role="separator" aria-orientation="vertical"
      ...
      ${a?u`
          <div class="add-grid ${r?"":"lean"}">
            ${ji.map(l=>u`<button class="add" style=${`--k:${ie[l]}`} ?disabled=${i} title=${`Add a blank ${dt[l].toLowerCase()} layer`}
      ...
      ${this.renderPresetDialog()}
        ...
        <span class="name">
          <b>${ve($,c)}</b>
          <small><span class="kind">${dt[$.kind]}</span> · ${up($,h,this.historySeries,Y.size)}</small>
          ${_?u`<span class="facts">${pp(this.host(),a,$,Y).map(D=>u`<span class="fact"><b>${D.label}</b> ${D.value}</span>`)}</span>`:m}
        </span>
        <span class="right">
          <span class="badges">
            ${Xe?u`<span class="badge tap" title=${`Tappable \xB7 ${ve(Xe,c)}`}>tap</span>`:m}
            ${$.payload.rules.length===0?m:u`<span class="badge states" title=${T}>${T.replace(/\.$/,"").toLowerCase()}</span>`}
            ${te?u`<span class="badge">hidden</span>`:m}
          </span>
          ${i?u`<span class="acts">
            <button class="icon" title=${`Bring forward (${He}])`} aria-label="Bring forward" @click=${D=>{D.stopPropagation(),r(P,1)}}>${z("up")}</button>
            <button class="icon" title=${`Send back (${He}[)`} aria-label="Send back" @click=${D=>{D.stopPropagation(),r(P,-1)}}>${z("down")}</button>
            <button class="icon" title=${`${Y.isHidden?"Show in":"Hide in"} ${N(a)} (${wa}${He}H)`} aria-label=${Y.isHidden?"Show this layer":"Hide this layer"} @click=${D=>{D.stopPropagation(),this.mutate(oe=>ke(oe,a,P,{isHidden:!Y.isHidden}))}}>${z(Y.isHidden?"hide":"show")}</button>
            <button class="icon" title=${`Duplicate (${He}D)`} aria-label="Duplicate" @click=${D=>{D.stopPropagation(),o(P)}}>${z("duplicate")}</button>
            <button class="icon danger" title="Delete (Delete)" aria-label="Delete" @click=${D=>{D.stopPropagation(),l(P)}}>${z("delete")}</button>
          </span>`:m}
        </span>
      </div>`},ye=($,A)=>{let J=this.inspect.kind==="group"&&this.inspect.id===$.id,P=!this.collapsed.has($.id),ee=this.rowDrag($.id,i),Y=A[0],te=A[A.length-1],Xe=C=>{let B=C.currentTarget,D=B.getBoundingClientRect(),oe=D.top+(B.classList.contains("drop-before")?Ue:0),Ps=D.bottom-(B.classList.contains("drop-after")?Ue:0),$a=(C.clientY-oe)/Math.max(1,Ps-oe);return $a<.25?"drop-before":!P&&$a>.75?"drop-after":"drop-into"},T=A.map(C=>C.payload.id);return u`<div class="layer group ${J?"hl":""} ${_?"rich":""}" style=${`--k:${q.group}`} tabindex="0" draggable=${ee.draggable}
        ...
        @drop=${C=>{C.preventDefault();let B=Xe(C);this.clearDragMarks();let D=this.dragId;if(this.dragId=void 0,!(!D||!Y||!te)){if(B==="drop-before"){this.reorderLayer(D,Y.payload.id,!0,!0);return}if(B==="drop-after"){this.reorderLayer(D,te.payload.id,!1,!0);return}this.isGroupId(D)||(this.reorderLayer(D,Y.payload.id,!0),this.mutate(oe=>xi(oe,D,$.id)))}}}>
        <button class="chev" aria-expanded=${P?"true":"false"} title=${P?"Fold the group":"Unfold the group"}
          @click=${C=>{C.stopPropagation();let B=new Set(this.collapsed);P?B.add($.id):B.delete($.id),this.collapsed=B}}>${z("chevron")}</button>
        <span class="bar"></span>
        ${R(A.map(C=>C.payload.id))}
        <span class="name">
          <b>${$.name}</b>
          <small><span class="kind">Group</span> · ${A.length} layer${A.length===1?"":"s"} · ${$.locked?"moves as one":"unlocked"}</small>
          ${_?u`<span class="facts"><span class="fact"><b>Holds</b> ${A.map(C=>ve(C,c)).join(", ")}</span></span>`:m}
        </span>
        <span class="right">
          ${i?u`<span class="acts">
            <button class="icon" title=${`Ungroup: keep the layers, drop the folder (${wa}${He}G)`} aria-label="Ungroup" @click=${C=>{C.stopPropagation(),this.mutate(B=>Ut(B,$.id)),J&&(this.inspect={kind:"general"})}}>${z("ungroup")}</button>
          </span>`:m}
          <button class="icon lockbtn ${$.locked?"on":""}" ?disabled=${!i}
            ...
            @click=${C=>{C.stopPropagation(),this.mutate(B=>{let D=B.groups?.find(oe=>oe.id===$.id);D&&(D.locked=!D.locked)})}}>${z($.locked?"lock":"unlock")}</button>
        </span>
      </div>`},E=[],O=new Set;for(let $=0;$<d.length;$++){let A=d[$],J=A.payload.groupId,P=J===void 0?void 0:t.groups?.find(te=>te.id===J);if(!P){E.push(U(A,!1));continue}if(O.has(P.id))continue;O.add(P.id);let ee=d.filter(te=>te.payload.groupId===P.id);E.push(ye(P,ee));let Y=this.inspect.kind==="group"&&this.inspect.id===P.id;this.collapsed.has(P.id)||E.push(u`<div class="group-kids">${ee.map(te=>U(te,!0,Y))}</div>`)}return u`<div class="card">
      ...
      ${E}
      <div class="layer pinned ${y?"hl":""}" style=${`--k:${q.place}`} tabindex="0" title="The shape is always the bottom layer"
        ...
        @drop=${$=>{$.preventDefault(),this.clearDragMarks();let A=this.dragId,J=[...d].reverse().find(P=>P.payload.id!==A&&P.payload.groupId!==A);A&&J&&this.reorderLayer(A,J.payload.id,!1,!0),this.dragId=void 0}}>
      ...
      <div class="off-rows">
        ${t.map(l=>u`<div class="off-row" style=${`--k:${ie[l.kind]}`} tabindex="0"
          title=${`${ve(l,r)} is on the complication but not on this shape`}
      ...
      <div class="values-list">
        ...
        ${l.map(p=>{let c=s.resolve({kind:{kind:"named",id:p.id}}),h=this.inspect.kind==="data"&&this.inspect.id===p.id;return u`<div class="datum ${h?"hl":""}" @click=${()=>{this.inspect={kind:"data",id:p.id}}}>
            ...
            <span class="meta ${c===void 0?"none":""}" title=${ge(p.value,d)}>${c??"unresolved"}</span>
      ...
      <div class="tiles">
        ${Et.map(r=>{if(!a.includes(r))return u`<button class="tile off ${r}" ?disabled=${!this.canEdit} title=${`Add the ${N(r)} shape`} @click=${()=>this.addShape(r)}>
            ...
            </button>`;let l=r===this.activeFamily,s;if(r==="inline")s=this.renderInlinePreview(i.inline,!0);else{let c=i[r];s=c?Wi(c,{icons:this.icons,imageSizes:this.imageSizes,slot:Yt.slots[r]}):m}let d=r!=="inline"&&t.elements.every(c=>re(t,r,c).isHidden||c.payload.isHidden)&&t.elements.length>0,p=this.canEdit&&Tt(t,r);return u`<div class="tile-wrap">
    ...
    </div>`}commitTestValue(t,i){this.editingValue=void 0;let a=i.trim(),r=new Map(this.testValues),o=this.hass.states[t]?.state;a===""||a===o?r.delete(t):r.set(t,a),this.testValues=r}currentCase(){return qt.find(t=>t.label===this.previewCase)??Yt}previewSlot(t){return this.currentCase().slots[t]}crumbs(t,i){let a=this.inspect,r=t.name.trim()||"Complication",o=this.activeFamily==="inline"?"Inline":N(this.activeFamily),l=a.kind==="family"&&i===void 0?u`<span class="here" style=${`--k:${q.place}`}>${o} shape</span>`:u`<button @click=${()=>{this.inspect={kind:"family"}}} title="Edit the shape">${o}</button>`,s=m,d=m;if(i!==void 0)s=u`<span class="here" style="--k:var(--wa-accent)"><span class="kchip">Picked</span>${i} layers</span>`;else if(a.kind==="layer"){let p=t.elements.find(c=>c.payload.id===a.id);if(p){s=u`<span class="here" style=${`--k:${ie[p.kind]}`}><span class="kchip">${dt[p.kind]}</span>${ve(p,me(this.host()))}</span>`;let c=Oe(t,p.payload.id);c&&(d=u`<span class="sep">›</span><button @click=${()=>{this.inspect={kind:"group",id:c.id}}} title="Edit the group">${c.name}</button>`)}}else if(a.kind==="group"){let p=t.groups?.find(c=>c.id===a.id);p&&(s=u`<span class="here" style=${`--k:${q.group}`}><span class="kchip">Group</span>${p.name}</span>`)}else if(a.kind==="data"){let p=t.values.find(c=>c.id===a.id);p&&(s=u`<span class="here" style=${`--k:${q.complication}`}><span class="kchip">Value</span>${p.name||"(unnamed)"}</span>`)}else a.kind==="general"&&(s=u`<span class="mini">nothing selected</span>`);return u`<div class="crumbs">
      ...
      ${s===m?m:u`<span class="sep">›</span>${s}`}
        ...
        <div class="insp-body" style=${this.canEdit?"":"pointer-events:none;opacity:.6"}
          @change=${()=>this.draft?.endGesture()}>${this.multiEditor(t,i)}</div>`;let a=this.host(),r=this.inspect,o=m,l=!0;if(r.kind==="layer"){let d=t.elements.find(p=>p.payload.id===r.id);if(!d)return this.inspect={kind:"general"},m;o=ps(a,d,this.canvasFamily)}else if(r.kind==="group"){let d=t.groups?.find(p=>p.id===r.id);if(!d)return this.inspect={kind:"general"},m;l=!1,o=hs(a,d)}else if(r.kind==="data"){let d=t.values.find(p=>p.id===r.id);if(!d)return this.inspect={kind:"general"},m;l=!1,o=u`<div class="sec" data-open="true" style=${`--c:${q.complication}`}>
          ...
          <div class="picked">
            ${d.map(h=>u`<div class="row" style=${`--k:${ie[h.kind]}`}>
              ...
              <span class="name">
                ...
                <b>${ve(h,r)}</b><span class="kind">${dt[h.kind]}</span>
              </span>
            </div>`)}
          </div>
      ...
      <div class="sec" data-open="true" style=${`--c:${q.place}`}>
        ...
        <div class="sec-b">
          ...
          ${l.colourable?u`${pe("Colour",l.colour,h=>{h!==void 0&&c(h)})}
