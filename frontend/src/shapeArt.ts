 ...
 * screen's top left quarter, bezel text and all, the way the editor's stage
 * draws it. Inline, which that face does not carry, sits as a line over the
 * clock.
 *
...
function watchBody(families: readonly FamilyKind[], live: LiveShapes, shelved: boolean): unknown {
  ...
  // The corner goes under the clock: its quarter's black reaches the
  // clock's first digit.
  return svg`${shell}
    ${corner}
...
/**
 * The corner on the watch: the screen's top left quarter, which is what the
 ...
 * is fitted to half the screen's height, pushed to the left edge, and masked
...
/**
 * Where a shape's slot sits on the watch drawing, in the drawing's own units,
 * the way the Modular face lays them out. One table rather than four literals
 * inside the drawing, because the card that draws a shape without its watch
 * has to put it exactly where the watch would have.
 */
function watchSlot(family: FamilyKind): Slot | undefined {
  switch (family) {
    case "rectangular":
      return { x: 14, y: 56, width: 58, height: 21 };
    case "circular":
      return { x: 14, y: 65, width: 14, height: 14 };
    case "corner":
      return { x: 14, y: 17, width: 13, height: 13 };
    case "inline":
      return { x: 14, y: 13, width: 58, height: 8 };
    default:
      return undefined;
  }
}
 ...
 *
 * The gallery's copy (`homepage/gallery/device-art.js`) uses these windows
 * too, grown away from the band to one 2:1 well, since its grid mixes shapes
 * and every card there shares that well. Change a window in both.
 */
...
function bareRx(family: FamilyKind, phone: boolean): number {
  if (!phone) return 3;
  return isHomeFamily(family) ? 3 : LOCK_RECT_RX;
}

/** The lit fill a shape drawn on its own falls back to: the slot's own shape
 * at the slot's own size, so a document this panel cannot draw still says
 * which shape it is and still measures the same as the rest of the grid. */
