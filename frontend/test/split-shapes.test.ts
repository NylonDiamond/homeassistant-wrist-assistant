...
describe("documentParts", () => {
  ...
  it("calls a one-shape document with a linkId an unlink", () => {
    ...
    cfg.linkId = "AAAAAAAA-0000-4000-8000-00000000000A";
    expect(planFor(cfg)).toBe("unlink");
  });
});
...
describe("splitDocument", () => {
  ...
  it("drops the linkId from every child", () => {
    const parent = fixture("linked_complication.json");
    expect(parent.linkId).toBeDefined();
    for (const child of splitDocument(parent, counter())) expect(child.linkId).toBeUndefined();
  });
  ...
  it("hands back a one-shape document as itself, minus the link", () => {
    const parent = fixture("control_only.json");
    parent.linkId = "AAAAAAAA-0000-4000-8000-00000000000A";
    const children = splitDocument(parent, counter());
    expect(children).toHaveLength(1);
    expect(children[0]!.id).toBe(parent.id);
    expect(children[0]!.linkId).toBeUndefined();
