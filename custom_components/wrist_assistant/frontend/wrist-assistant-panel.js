...
`)}function Or(e){let n;try{n=JSON.parse(e)}catch{return}if(typeof n!="object"||n===null||Array.isArray(n))return;let t=new Map,i=new Set;for(let[a,r]of Object.entries(n))r===null?i.add(a):t.set(a,Cl(r));return{values:t,nullKeys:i}}function Cl(e){return typeof e=="string"?e:typeof e=="boolean"?e?"true":"false":typeof e=="number"?(Number.isInteger(e)&&Math.abs(e)<1e15,String(e)):JSON.stringify(e)}function Pi(e){let n=_i(e),t=[...n.entities.entries()].sort(([i],[a])=>i<a?-1:i>a?1:0).map(([,i])=>({kind:"entity",entityId:i.entityId,displayName:i.displayName,domain:i.domain,...i.iconName!==void 0?{iconName:i.iconName}:{}}));return n.document&&t.push({kind:"template",value:n.document}),t}function Sl(e,n){if(e.values.length!==0)switch(n){case"latest":return e.values[e.values.length-1];case"highest":return Math.max(...e.values);case"lowest":return Math.min(...e.values);case"average":return e.values.reduce((t,i)=>t+i,0)/e.values.length;case"top":return e.domainMax;case"bottom":return e.domainMin}}function Tn(e){let n=e;if(/^[+-]?(\d+\.?\d*|\.\d+)([eE][+-]?\d+)?$/.test(n))return Number(n);let t=n.toLowerCase();if(t==="inf"||t==="+inf"||t==="infinity"||t==="+infinity")return 1/0;if(t==="-inf"||t==="-infinity")return-1/0;if(t==="nan"||t==="+nan"||t==="-nan")return NaN}function Ct(e){let n=e.trim(),t=Tn(n);if(t!==void 0)return t;let i="";for(let r of n)if(/\p{N}/u.test(r)||r==="."||r==="-"||r==="+")i+=r;else if(i.length>0)break;return i.length===0?void 0:Tn(i)}function El(e){return Number.isInteger(e)&&Math.abs(e)<1e16?e.toFixed(1):String(e)}function Tl(e){let n=Math.max(0,e);return n<60?`${Math.trunc(n)}s`:n<3600?`${Math.trunc(n/60)}m`:n<86400?`${Math.trunc(n/3600)}h`:`${Math.trunc(n/86400)}d`}function Fl(e){return e.replace(/\S+/g,n=>n.charAt(0).toUpperCase()+n.slice(1).toLowerCase())}function Rl(e,n,t){if(Ie(n))return e;let i=n,a=e,r=Tn(e.trim());if(i.relativeTime&&r!==void 0)a=Tl(r);else{let o=Ct(e);if(o!==void 0){let l=o*(i.multiply??1)+(i.offset??0);i.decimals!==void 0?a=l.toFixed(Math.max(0,i.decimals)):l!==o&&(a=Number.isInteger(l)?String(l):El(l))}}switch(i.useEntityUnit&&t&&(a+=t.startsWith("\xB0")||t.startsWith("%")?t:` ${t}`),i.prefix&&(a=i.prefix+a),i.suffix&&(a=a+i.suffix),i.textCase){case"upper":a=a.toUpperCase();break;case"lower":a=a.toLowerCase();break;case"capitalized":a=Fl(a);break}return a}function St(e){let n=Math.trunc(Math.max(0,e)),t=Math.trunc(n/3600),i=Math.trunc(n%3600/60),a=n%60,r=o=>String(o).padStart(2,"0");return t>0?`${t}:${r(i)}:${r(a)}`:`${i}:${r(a)}`}function jt(e,n=240){let t=[],i="",a=!1,r=()=>{if(i!==""){let o=Number(i);Number.isFinite(o)&&t.push(o)}i=""};for(let o of e){if(t.length>=n)break;if(o>="0"&&o<="9")i+=o,a=!0;else if(o===".")i.includes(".")&&r(),i+=".",a=!0;else if(o==="-"||o==="+"){let l=!a;r(),l&&(i+=o),a=!1}else r(),a=!1}return t.length<n&&r(),t}function Ml(e,n){let t,i;return n.scale==="fixed"?(t=Math.min(n.minValue,n.maxValue),i=Math.max(n.minValue,n.maxValue)):(t=e.length>0?Math.min(...e):0,i=e.length>0?Math.max(...e):1),n.baseline==="zero"&&(t=Math.min(t,0),i=Math.max(i,0)),i>t||(i=t+1),{min:t,max:i}}function Il(e,n,t){if(e===void 0)return 0;let i=Ct(e);if(i===void 0||Number.isNaN(i))return 0;let a=t-n;return a===0?0:Math.min(1,Math.max(0,(i-n)/a))}var Ve=class{constructor(n,t){this.ctx=n;this.charts=new Map;this.named=new Map(n.namedValues.map(i=>[i.id.toUpperCase(),i.value])),t&&this.settleCharts(t)}chartReadings(n){let t=rt(n),i=t!==void 0?this.ctx.historySeries?.get(t)??"":this.resolve(n.value)??"",a=jt(i);n.limit>0&&a.length>n.limit&&(a=n.takeFromEnd?a.slice(a.length-n.limit):a.slice(0,n.limit));let r=Ml(a,n),o={values:a,domainMin:r.min,domainMax:r.max},l=this.dereference(n.value);return l&&"entityId"in l.kind&&(o.entity={entityId:l.kind.entityId,displayName:l.kind.displayName,domain:l.kind.domain}),o}settleCharts(n){for(let t of n.elements)t.kind==="chart"&&this.charts.set(t.payload.id,this.chartReadings(t.payload))}dereference(n){let t=n,i=new Set,a=n.format;for(;t.kind.kind==="named";){let o=t.kind.id.toUpperCase();if(i.has(o))return;i.add(o);let l=this.named.get(o);if(!l)return;a=a&&!Ie(a)?a:l.format,t=l}let r={kind:t.kind};return a&&(r.format=a),r}directEntityUnit(n){let t=n.kind;if(t.kind==="entityState"||t.kind==="entityAttribute"||t.kind==="entityAge")return this.ctx.entityStates.get(t.entityId)?.unitOfMeasurement;if(t.kind==="chartStat"){let i=this.charts.get(t.layer.toUpperCase())?.entity;return i?this.ctx.entityStates.get(i.entityId)?.unitOfMeasurement:void 0}}resolve(n){if(!n)return;let t=this.dereference(n);if(!t)return;let i;switch(t.kind.kind){case"literal":i=t.kind.value;break;case"entityState":i=this.ctx.entityStates.get(t.kind.entityId)?.state;break;case"dataAge":i=this.ctx.dataAgeSeconds===void 0?void 0:String(Math.trunc(this.ctx.dataAgeSeconds));break;case"chartStat":{let a=this.charts.get(t.kind.layer.toUpperCase()),r=a?Sl(a,t.kind.stat):void 0;i=a&&r!==void 0?dr(r,a.domainMax-a.domainMin):void 0;break}default:{let a=zi(n,this.named);i=a===void 0?void 0:this.ctx.templateResults.get(a)}}if(i!==void 0)return Rl(i,t.format,this.directEntityUnit(t))}nowMs(){return this.ctx.nowMs??Date.now()}countdownEnd(n){if(!n)return;let t=this.dereference(n);if(!t)return;let i=t.kind;if(i.kind==="entityState"){let o=this.ctx.entityStates.get(i.entityId);if(o?.timerState!==void 0){if(o.timerState!=="active"||!o.finishesAt)return;let l=Date.parse(o.finishesAt);return Number.isFinite(l)&&l>this.nowMs()?l:void 0}}let a=this.resolve(n)?.trim();if(!a)return;let r=Date.parse(a);if(!Number.isFinite(r)){let o=Tn(a);r=o===void 0?NaN:o*1e3}return Number.isFinite(r)&&r>this.nowMs()?r:void 0}countdownFallbackText(n){if(!n)return;let t=this.dereference(n);if(!t||t.kind.kind!=="entityState")return;let i=this.ctx.entityStates.get(t.kind.entityId);if(i?.timerState!==void 0)return i.timerState==="paused"?i.remaining!==void 0&&i.remaining>0?St(i.remaining):"Paused":"Idle"}entityIcon(n){let t=this.dereference(n);return!t||t.kind.kind!=="entityState"?void 0:this.ctx.entityStates.get(t.kind.entityId)?.iconName??t.kind.iconName}isStale(){return(this.ctx.dataAgeSeconds??1/0)>(this.ctx.stalenessThresholdSeconds??1200)}evaluateTest(n){let t=n.comparison;if(t.kind==="isStale")return this.isStale();let i=this.resolve(n.value);if(i===void 0)return t.kind==="isUnavailable";let a=Ct(i),r=()=>this.resolve(t.value),o=()=>{let s=r();return s===void 0?void 0:Ct(s)},l=s=>{let d=o();return a===void 0||d===void 0?!1:s(a,d)};switch(t.kind){case"equals":{let s=r();return s!==void 0&&i===s}case"notEquals":{let s=r();return s!==void 0&&i!==s}case"isOn":return i.toLowerCase()==="on";case"isOff":return i.toLowerCase()==="off";case"isUnavailable":{let s=i.toLowerCase();return s==="unavailable"||s==="unknown"}case"isEmpty":return i.trim().length===0;case"greaterThan":return l((s,d)=>s>d);case"greaterOrEqual":return l((s,d)=>s>=d);case"lessThan":return l((s,d)=>s<d);case"lessOrEqual":return l((s,d)=>s<=d);case"between":{let s=o(),d=this.resolve(t.upper),p=d===void 0?void 0:Ct(d);if(a===void 0||s===void 0||p===void 0)return!1;let[c,h]=s<=p?[s,p]:[p,s];return a>=c&&a<=h}case"contains":{let s=r();return!!s&&i.toLowerCase().includes(s.toLowerCase())}case"startsWith":{let s=r();return!!s&&i.toLowerCase().startsWith(s.toLowerCase())}case"endsWith":{let s=r();return!!s&&i.toLowerCase().endsWith(s.toLowerCase())}case"matchesRegex":{if(!t.pattern)return!1;try{return new RegExp(t.pattern).test(i)}catch{return!1}}case"isOneOf":return(t.options??[]).some(s=>s.toLowerCase()===i.toLowerCase());default:return!1}}evaluateCondition(n){return n.tests.length===0?!0:n.join==="any"?n.tests.some(t=>this.evaluateTest(t)):n.tests.every(t=>this.evaluateTest(t))}applyRules(n,t){let i=new Map;for(let a of n){let r=t?.get(a.id),o=[];if(r&&r!=="live")r==="otherwise"?o=a.otherwise??[]:o=a.cases.find(l=>l.id===r.caseId)?.then??[];else{let l=a.cases.find(s=>this.evaluateCondition(s.when));o=l?l.then:a.otherwise??[]}for(let l of o)i.set(we[l.kind],l)}return i}liveBranches(n){let t=new Map;for(let i of n){let a=i.cases.find(r=>this.evaluateCondition(r.when));t.set(i.id,a?a.id:i.otherwise?"otherwise":"none")}return t}styleColor(n,t){let i=n.get(t);if(!i)return;let a=this.resolve(i.value);return a||void 0}styleText(n,t){let i=n.get(t);return i?this.resolve(i.value):void 0}styleNumber(n,t){return n.get(t)?.number}resolveElement(n,t){let i=n.payload,a=this.applyRules(i.rules,t),r=a.get("visibility"),o=r?r.kind==="hide":i.isHidden,l=this.styleNumber(a,"rotation"),s=l===void 0?i.frame:{...i.frame,rotationDegrees:l},d=this.styleNumber(a,"opacity")??1,p={id:i.id,isHidden:o,frame:s,opacity:d};switch(n.kind){case"text":{let c=n.payload.countdown?this.countdownEnd(n.payload.value):void 0,h=n.payload.countdown?this.countdownFallbackText(n.payload.value):void 0,g={kind:"text",...p,text:this.styleText(a,"text")??h??this.resolve(n.payload.value)??"--",fontSize:this.styleNumber(a,"fontSize")??n.payload.fontSize,fontWeight:a.get("fontWeight")?.weight??n.payload.fontWeight,colorHex:this.styleColor(a,"color")??n.payload.colorSlot.baseColorHex};return c!==void 0&&(g.countdownEnd=c),g}case"icon":{let c=this.entityIcon(n.payload.symbol)??this.resolve(n.payload.symbol)??"questionmark.circle";return{kind:"icon",...p,symbol:this.styleText(a,"icon")??c,size:this.styleNumber(a,"fontSize")??n.payload.size,colorHex:this.styleColor(a,"color")??n.payload.colorSlot.baseColorHex}}case"gauge":{let c=this.styleText(a,"gaugeValue")??this.resolve(n.payload.value),h=this.styleNumber(a,"gaugeMin")??n.payload.minValue,g=this.styleNumber(a,"gaugeMax")??n.payload.maxValue;return{kind:"gauge",...p,fraction:Il(c,h,g),style:n.payload.style,lineWidth:n.payload.lineWidth,colorHex:this.styleColor(a,"color")??n.payload.colorSlot.baseColorHex,trackColorHex:n.payload.trackColorHex}}case"chart":{let c=n.payload,h=this.charts.get(c.id)??this.chartReadings(c),g=h.values,y={min:h.domainMin,max:h.domainMax},k=this.styleColor(a,"color")??c.colorSlot.baseColorHex,x=xt(c),R=sr(c)?g.map(f=>lr(f,x,c.bandAboveColorHex)):[],b={kind:"chart",...p,values:g,style:c.style,domainMin:y.min,domainMax:y.max,baseline:c.baseline,barGap:c.barGap,lineWidth:c.lineWidth,colorHex:k,highColorHex:c.highColorHex,lowColorHex:c.lowColorHex,marker:c.marker,pointColorHexes:R,fillBands:c.fillBands};if(g.length>0){let f=c.highlight==="highest"||c.highlight==="both",w=c.highlight==="lowest"||c.highlight==="both",E=f?g.indexOf(Math.max(...g)):-1,_=w?g.indexOf(Math.min(...g)):-1;E>=0&&(b.highIndex=E),_>=0&&_!==E&&(b.lowIndex=_)}return b}case"shape":{let c={kind:"shape",...p,shapeKind:n.payload.kind,cornerRadius:n.payload.cornerRadius,fillColorHex:this.styleColor(a,"color")??n.payload.colorSlot.baseColorHex,borderWidth:this.styleNumber(a,"borderWidth")??n.payload.borderWidth},h=this.styleColor(a,"borderColor")??n.payload.borderColorHex;return h!==void 0&&(c.borderColorHex=h),c}case"image":{let c={kind:"image",...p,entityId:n.payload.entity.entityId,showTimestamp:n.payload.timestamp===!0,contentMode:n.payload.contentMode,zoom:n.payload.zoom,panX:n.payload.panX,panY:n.payload.panY,cornerRadius:n.payload.cornerRadius,timestampCorner:n.payload.timestampCorner,timestampSize:n.payload.timestampSize};_e(n.payload)&&(c.timestampX=n.payload.timestampX,c.timestampY=n.payload.timestampY);let h=this.ctx.entityStates.get(n.payload.entity.entityId)?.entityPicture;return h!==void 0&&(c.url=h),c}case"tap":{let c={kind:"tap",...p,frame:n.payload.frame,opacity:1,action:n.payload.action};return n.payload.openPageId!==void 0&&(c.openPageId=n.payload.openPageId),n.payload.attachedTo!==void 0&&(c.attachedTo=n.payload.attachedTo),c}}}resolveLayout(n,t,i){let a=n.perFamily[t];this.settleCharts(n);let r=Sr(n,t).map(k=>this.resolveElement(k,i)),o=a?this.applyRules(a.rules,i):new Map,l={family:t,elements:r,cornerBodyShape:a?.cornerBodyShape??"wedge",borderWidth:this.styleNumber(o,"borderWidth")??a?.borderWidth??2},s=this.styleText(o,"text"),d=a?.bezelCountdown&&s===void 0?this.countdownEnd(a.bezelText):void 0,p=a?.bezelCountdown?this.countdownFallbackText(a.bezelText):void 0,c=s??p??this.resolve(a?.bezelText);c!==void 0&&(l.bezelText=c),d!==void 0&&(l.bezelCountdownEnd=d);let h=this.resolve(a?.curvedText);if(h!==void 0&&(l.curvedText=h),a?.curvedColorHex!==void 0&&(l.curvedColorHex=a.curvedColorHex),a?.bezelGauge){let k=a.bezelGauge,x=this.resolve(k.value),R=x===void 0?void 0:Ct(x);if(R!==void 0){let b=Math.min(k.minValue,k.maxValue),f=Math.max(k.minValue,k.maxValue),w={value:Math.min(f,Math.max(b,R)),minValue:b,maxValue:f===b?b+1:f,colorHexes:k.colorHexes},E=this.resolve(k.minLabel);E!==void 0&&(w.minLabel=E);let _=this.resolve(k.maxLabel);_!==void 0&&(w.maxLabel=_),l.bezelGauge=w}}let g=this.styleColor(o,"backgroundColor")??a?.backgroundColorHex;g!==void 0&&(l.backgroundColorHex=g);let y=this.styleColor(o,"borderColor")??a?.borderColorHex;return y!==void 0&&(l.borderColorHex=y),l}};function Al(e,n,t){let i=new Ve(n,t),a=e.countdown?i.countdownEnd(e.value):void 0,o={text:(e.countdown?i.countdownFallbackText(e.value):void 0)??i.resolve(e.value)??"--"};return e.label&&(o.label=e.label),e.symbol&&(o.symbol=e.symbol),a!==void 0&&(o.countdownEnd=a),o}function Oi(e,n,t){let i=new Ve(n),a={};for(let r of["rectangular","circular","corner"])e.supportedFamilies.includes(r)&&(a[r]=i.resolveLayout(e,r,t));return e.supportedFamilies.includes("inline")&&e.inline&&(a.inline=Al(e.inline,n,e)),a}var ge=oe,qt=[{label:"40 mm",screen:{width:162,height:197},slots:{rectangular:{width:141,height:51},circular:{width:40,height:40},corner:{width:26,height:26}},measured:!1},{label:"41 mm",screen:{width:176,height:215},slots:{rectangular:{width:153,height:55.5},circular:{width:43,height:43},corner:{width:29,height:29}},measured:!1},{label:"42 mm",screen:{width:187,height:223},slots:{rectangular:{width:163,height:59},circular:{width:46,height:46},corner:{width:31,height:31}},measured:!1},{label:"44 mm",screen:{width:184,height:224},slots:{rectangular:{width:160,height:58},circular:{width:45,height:45},corner:{width:30,height:30}},measured:!1},{label:"45 mm",screen:{width:198,height:242},slots:{rectangular:{width:172,height:62.5},circular:{width:48.5,height:48.5},corner:{width:32,height:32}},measured:!1},{label:"46 mm",screen:{width:208,height:248},slots:ge,measured:!0},{label:"49 mm",screen:{width:205,height:251},slots:{rectangular:{width:178.5,height:64.5},circular:{width:50,height:50},corner:{width:33.5,height:33.5}},measured:!1}],Yt=qt.find(e=>e.measured);function jr(e){if(!e)return;let n=/^(\d+)x(\d+)$/.exec(e.trim());if(!n)return;let t=Number(n[1]),i=Number(n[2]);return qt.find(a=>a.screen.width===t&&a.screen.height===i)}function Rn(e,n){let t=ge[n];if(e.width<=0||e.height<=0)return{scale:0,x:0,y:0,width:0,height:0};let i=Math.min(e.width/t.width,e.height/t.height),a=t.width*i,r=t.height*i;return{scale:i,x:(e.width-a)/2,y:(e.height-r)/2,width:a,height:r}}var Hl={regular:400,medium:500,semibold:600,bold:700};function Ae(e){if(!e)return;let n=e.startsWith("#")?e.slice(1):e;if(!/^[0-9a-fA-F]{6}([0-9a-fA-F]{2})?$/.test(n))return;let t=n.length===8?parseInt(n.slice(6,8),16)/255:1;return{color:`#${n.slice(0,6)}`,opacity:t}}function Be(e,n,t="#FFFFFF"){let i=Ae(e)??{color:t,opacity:1};return{[n]:i.color,[`${n}-opacity`]:i.opacity}}function qr(e,n){let t=Math.max(0,e.frame.width*n.width),i=Math.max(0,e.frame.height*n.height),a=(e.frame.x+e.frame.width/2)*n.width,r=(e.frame.y+e.frame.height/2)*n.height;return{x:a-t/2,y:r-i/2,w:t,h:i,cx:a,cy:r}}function Ll(e,n){let t=Be(e.colorHex,"fill");e.countdownEnd!==void 0&&e.countdownEnd>Date.now()&&(e={...e,text:St((e.countdownEnd-Date.now())/1e3)});let i=s=>s*.55,a=e.text.length*i(e.fontSize),r=a>n.w&&n.w>0?Math.max(.5,n.w/a):1,o=e.fontSize*r,l=e.text;if(n.w>0&&l.length*i(o)>n.w){let s=n.w-.8*o,d=Math.max(1,Math.floor(s/i(o)));l=`${l.slice(0,d).replace(/\s+$/,"")}\u2026`}return v`<text x=${n.cx} y=${n.cy} text-anchor="middle" dominant-baseline="central"
    ...
    </g>`}var _l=5;function Pl(e,n){let t=e.values,i=Math.max(t.length,1),a=e.highIndex!==void 0||e.lowIndex!==void 0,r=e.marker==="none"||!a?0:_l,o=e.style==="bars"?0:e.lineWidth/2,l=n.x,s=Math.max(n.w,0),d=n.y+r+o,p=Math.max(n.h-r-o*2,1),c=d+p,h=Math.max(e.domainMax-e.domainMin,Number.EPSILON),g=e.baseline==="lowest",y=g?p*.12:0,k=Math.min(Math.max(e.barGap,0),s/(i*2)),x=Math.max((s-k*(i-1))/i,.5),R=f=>Math.min(1,Math.max(0,(f-e.domainMin)/h)),b=f=>c-R(f)*p;return{count:t.length,barWidth:x,plotTop:d,plotBottom:c,baselineY:g?c:b(0),barRect(f){let w=l+f*(x+k),E=t[f],_,G;if(g){let ue=y+R(E)*(p-y);_=c-ue,G=c}else _=b(E),G=g?c:b(0),_>G&&([_,G]=[G,_]);return{x:w,y:_,w:x,h:Math.max(G-_,.5)}},point(f){let w=Math.max(s-o*2,0);return{x:t.length>1?l+o+w*f/(t.length-1):l+s/2,y:b(t[f])}},markerCenter(f,w){let E=w?this.barRect(f):void 0;return{x:E?E.x+E.w/2:this.point(f).x,y:n.y+r/2}}}}function Ol(e,n){if(e.values.length===0)return m;let t=Pl(e,n),i=Be(e.colorHex,"fill"),a=Be(e.highColorHex,"fill",e.colorHex),r=Be(e.lowColorHex,"fill",e.colorHex),o=(p,c)=>v`<circle cx=${p.x} cy=${p.y} r="1.7" fill=${c.fill} fill-opacity=${c["fill-opacity"]} />`,l=[],s=e.pointColorHexes.length===t.count,d=p=>s?Be(e.pointColorHexes[p],"fill",e.colorHex):i;if(e.style==="bars")for(let p=0;p<t.count;p++){let c=t.barRect(p),h=p===e.highIndex?a:p===e.lowIndex?r:d(p),g=Math.min(1.2,c.w/2,c.h/2);l.push(v`<rect x=${c.x} y=${c.y} width=${c.w} height=${c.h} rx=${g}
        fill=${h.fill} fill-opacity=${h["fill-opacity"]} />`)}else{let p=Array.from({length:t.count},(h,g)=>t.point(g)),c=p.map((h,g)=>`${g===0?"M":"L"}${h.x} ${h.y}`).join(" ");if(e.style==="area")if(e.fillBands&&s&&t.count>1)for(let h=0;h<t.count-1;h++){let g=p[h],y=p[h+1],k=d(h+1),x=`M${g.x} ${g.y} L${y.x} ${y.y} L${y.x} ${t.baselineY} L${g.x} ${t.baselineY} Z`;l.push(v`<path d=${x} fill=${k.fill}
  ...
  </button>`}function Xo(e,n,t,i){let a=e.symbols,r=a.isOpen(i),o=a.query(i),l=e.icons.names(),s=l??[],d=new Set(s),p=n.trim(),c=p!==""&&d.size>0&&!d.has(p),h=y=>{t(y),a.noteUsed(y)},g=m;if(r){let y=a.category(i),k=Td(y,o,s,d),x=co(k.names,o),R=k.fromPack?x.slice(0,Ed):x,b=d.size===0?a.recent:a.recent.filter(f=>d.has(f));g=u`<div class="sym-browse">
    ...
    ${Qt.has(n)?ts(e,i,a,r):m}
        ...
        <div class="hint">Rendered by Home Assistant. The result should be one value, not a whole document.</div>`;break;case"named":s=e.config.values.length===0?u`<div class="hint warn">There are no named values yet. Add one in the Data card first.</div>`:Ce("Value",a.id,[["","(choose)"],...e.config.values.map(p=>[p.id,p.name||p.id.slice(0,8)])],p=>r({...a,id:p}));break;case"chartStat":{let p=pe(e),c=e.config.elements.filter(h=>h.kind==="chart");s=c.length===0?u`<div class="hint warn">There is no chart layer yet. Add one first, then this can print one of its numbers.</div>`:u`
          ${Ce("Chart",a.layer,[["","(choose)"],...c.map(h=>[h.payload.id,ye(h,p)])],h=>r({...a,layer:h}))}
  ...
  ${a?m:u`<div class="hint">Without a page the tap falls back to the complication list.</div>`}`}function rs(e,n){let t=e.config.values.findIndex(a=>a.id===n.id),i=`nv-${n.id}`;return u`
    ...
    ${t?u`
      ...
      <div class="hint">The time the snapshot was fetched, not the time now. A frame that stops updating keeps its old time.</div>`:m}`}function he(e,n,t,i,a={}){let r=e.openSections.has(n),o=()=>e.toggleSection(n);return u`<section class="sec" data-open=${r?"true":"false"} style=${a.color?`--c:${a.color}`:""}>
    ...
    ${r?u`<div class="sec-b">${i}</div>`:m}
        ...
        ${le("Track colour",n.payload.trackColorHex,b=>o(f=>{f.payload.trackColorHex=b??"#FFFFFF40"},"track"))}`;break;case"chart":{let b=n.payload,f=(T,C)=>o(P=>T(P.payload),C),w=rt(b),E=b.historyMinutes>0,_=b.value.kind.kind==="entityState",G=w===void 0?void 0:e.historySeries(w),ue=E&&_?G??"":e.resolve(b.value)??"",S=b.historyPoints<1,N=gn.some(T=>T.minutes===b.historyMinutes),$=oa.has(i)||!N,A=Math.floor(b.historyMinutes/1440),D=Math.floor(b.historyMinutes%1440/60),W=b.historyMinutes%60,q=(T,C,P)=>f(Z=>{Z.historyMinutes=Math.min(cr,Math.max(1,Math.round(T)*1440+Math.round(C)*60+Math.round(P)))},"span"),Y=jt(ue),ke=b.limit>0&&Y.length>b.limit?b.takeFromEnd?Y.slice(Y.length-b.limit):Y.slice(0,b.limit):Y,Xe=!E&&_&&Y.length===1;p=u`
        ${ie(e,b.value,T=>f(C=>{C.value=T},"value"),{label:"Readings",key:`${r}-value`})}
        ${ee("Draw",E?"history":"value",[["history","Recorded history"],["value","The value itself"]],T=>f(C=>{C.historyMinutes=T==="history"?C.historyMinutes||yn:0}),{titles:{history:"Read the entity's past from the recorder and plot it",value:"Plot the numbers the value holds right now, such as a forecast list"}})}
        ${E?u`
              ...
              <div class="field readings-field"><span>Readings</span>
                <div class="readings-row">
                  <div class="seg wide" role="radiogroup" aria-label="Readings">
                    <button type="button" role="radio" aria-checked=${S?"false":"true"} class=${S?"":"on"}
                      title="Average the recorded states into this many equal time slots"
                      @click=${()=>{S&&f(T=>{T.historyPoints=24})}}>Average</button>
                    <button type="button" role="radio" aria-checked=${S?"true":"false"} class=${S?"on":""}
                      title="Plot every recorded state change, no averaging"
                      @click=${()=>{S||f(T=>{T.historyPoints=mi})}}>Every one</button>
                  </div>
                  ${S?m:u`<input type="number" class="short" aria-label="How many readings" .value=${String(b.historyPoints)}
            ...
            ${$?u`<div class="grid3 span-parts">
                ${Q("Days",A,T=>q(T??0,D,W),{step:1,min:0,max:7})}
                ${Q("Hours",D,T=>q(A,T??0,W),{step:1,min:0,max:23})}
                ${Q("Minutes",W,T=>q(A,D,T??0),{step:1,min:0,max:59})}
              </div>
            ...
            <div class="hint">${S?u`Every state the recorder holds in that span, oldest first, one reading per change,
        ...
        ${Y.length===0&&!(E&&(!_||G===void 0||G===""))?u`<div class="hint warn">No numbers in this value yet, so the chart draws nothing.</div>`:m}
        ${Y.length>0?u`<div class="hint">Reads <span class="nums">${ac(ke)}</span>${Y.length===ke.length?u` · ${ke.length} ${ke.length===1?"value":"values"}`:u` · ${ke.length} of ${Y.length}`}</div>`:m}
        ...
        <div class="hint">${E?"Trims the series after it arrives, so 0 draws every reading fetched above.":"A forecast sensor often carries 24 or 48 entries. 0 draws all of them."}</div>`,c=u`
        <div class="grid2">
        ...
        ${ee("Colour",b.coloring,_d,T=>f(C=>{C.coloring=T,T==="bands"&&C.bands.length===0&&(C.bands=Pd(ke))}))}
        ${b.coloring==="bands"?u`
          ...
          ${b.bands.map((T,C)=>u`
            <div class="row-inline">
              ${Q("Up to",T.upTo,P=>f(Z=>{let fe=Z.bands[C];fe&&(fe.upTo=P??0)},`bup${T.id}`))}
              ${le("Colour",T.colorHex,P=>f(Z=>{let fe=Z.bands[C];fe&&(fe.colorHex=P??"#FFFFFF")},`bcol${T.id}`))}
              <button class="icon" title="Remove this band" aria-label="Remove this band"
                @click=${()=>f(P=>{P.bands=P.bands.filter((Z,fe)=>fe!==C)})}>${z("close")}</button>
            </div>`)}
        ...
        ${n.payload.borderColorHex!==void 0?Q("Border width (pt)",n.payload.borderWidth,b=>o(f=>{f.payload.borderWidth=b??1},"bw"),{step:.5,min:0}):m}`;break;case"image":{let b=n.payload,f=(w,E)=>o(_=>w(_.payload),E);p=u`
        ...
        ${ee("Picture",b.contentMode,[["fill","Fill the frame"],["fit","Fit inside"]],w=>f(E=>{E.contentMode=w}),{titles:{fill:"Cover the frame, cropping what does not fit",fit:"Show the whole picture, with space around it"}})}
        ${Dn("Zoom",b.zoom,w=>f(E=>{E.zoom=w},"zoom"),{min:Ui,max:4,step:.05,def:1,format:w=>`${w.toFixed(2)}x`})}
        ${Dn("Pan left/right",b.panX,w=>f(E=>{E.panX=w},"panx"),{min:-1,max:1,step:.02,def:0})}
        ${Dn("Pan up/down",b.panY,w=>f(E=>{E.panY=w},"pany"),{min:-1,max:1,step:.02,def:0})}
        <div class="hint">${nc(b)}</div>
        ${Q("Corner radius (pt)",b.cornerRadius,w=>f(E=>{E.cornerRadius=Math.max(0,w??Vt)},"imgradius"),{step:1,min:0})}`;break}case"tap":{p=u`
        ...
        <div class="hint">To make one layer tappable, tick Tappable on that layer instead.</div>`;break}}let h=n.kind==="image"||n.kind==="tap"?void 0:le(n.kind==="shape"?"Fill colour":"Colour",n.payload.colorSlot.baseColorHex,b=>o(f=>{f.kind!=="image"&&f.kind!=="tap"&&(f.payload.colorSlot.baseColorHex=b??"#FFFFFF")},"color")),g=Si(e.config,n),y=g?{kind:{kind:"entityState",...g}}:void 0,k=ne[n.kind],x=n.kind==="tap"?void 0:$e(e.config,i)[0],R=n.kind==="image"?n.payload.timestamp===!0:!1;return u`
    ${he(e,"content","Content",u`${n.kind==="tap"?m:Zd(e,n,r)}${p}`,{color:k,icon:"content",summary:ua(e,n)})}
    ${c===void 0&&h===void 0?m:he(e,"look",n.kind==="image"?"Picture":"Look",u`${c??m}${h??m}`,{color:k,icon:n.kind==="image"?"image":"look",...Wn(n)?{summary:Wn(n)}:{}})}
    ${n.kind==="chart"?he(e,"numbers","Numbers",lc(e,n),{color:ne.text,icon:"text",summary:sc(e,n)}):m}
    ${n.kind==="image"?he(e,"timestamp","Timestamp",ic(n.payload,(b,f)=>o(w=>b(w.payload),f)),{color:k,icon:"clock",summary:R?`Shown \xB7 ${n.payload.timestampSize} pt`:"Hidden"}):m}
    ${n.kind==="tap"?m:he(e,"tappable","Tap",dc(e,n,r),{color:J.tap,icon:"tap",summary:x?Pe(x.payload.action):"Not tappable"})}
    ${he(e,"states","States",vs(e,n.payload.rules,n.kind,b=>b.elements.find(f=>f.payload.id===i)?.payload.rules,`rules-${i}`,y),{color:J.states,icon:"states",summary:Jt(n.payload.rules).replace(/\.$/,"")})}
    ${he(e,"placement","Place",u`
      ${Dn("Rotation",s.rotationDegrees,b=>d({rotationDegrees:b},"rot"),{min:-180,max:180,step:1,def:0,format:b=>`${Math.round(b)}\xB0`})}
      <div class="hint">Drag the layer on the ${O(t)} preview to move it, or pull a
      ...
      <div class="hint">Everything about where this layer sits, how big it is drawn and whether it
        shows belongs to the ${O(t)} shape alone. Pick another shape above to place
        the same layer differently there.</div>`,{color:J.place,icon:"place",summary:`${Math.round(s.width*100)}% wide \xB7 ${O(t)}${l.fromPlacement?"":" \xB7 shared frame"}`})}`}function us(e,n,t,i){let a=n.action,r=o=>["toggleEntity","runScene","runScript","addTodo","runHTTPAction"].includes(o);return u`
    ...
    ${a.type==="openPage"?as(e,n.openPageId,n.openPageName,(o,l)=>t(s=>{if(o===void 0){delete s.openPageId,delete s.openPageName;return}s.openPageId=o,l?s.openPageName=l:delete s.openPageName},"tap-page")):m}`}var rc=24;function oc(e,n){let t=[],i=1/0;for(let r of X){if(r==="inline"||!e.config.supportedFamilies.includes(r))continue;let o=Fr(e.config,n,r);o&&(t.push(`${O(r)} ${Math.round(o.width)} x ${Math.round(o.height)} pt`),i=Math.min(i,o.width,o.height))}if(t.length===0)return m;let a=i<rc;return u`<div class=${a?"hint warn":"hint"}>${t.join(" \xB7 ")}${a?u`<br />That is small for a wrist. Show the tap area and drag its corners out.`:m}</div>`}function sc(e,n){let t=Gt(e.config,n.payload.id);return t.length===0?"None yet":t.map(i=>{let a=i.payload.value.kind;return a.kind==="chartStat"?(wt.find(([r])=>r===a.stat)?.[1]??"number").toLowerCase():"number"}).join(" \xB7 ")}function lc(e,n){let t=pe(e),i=Gt(e.config,n.payload.id),a=o=>{let l;e.update(s=>{l=xr(s,n.payload.id,o)}),l&&e.selectLayer(l)},r=new Set(i.map(o=>o.payload.value.kind.kind==="chartStat"?o.payload.value.kind.stat:""));return u`
        ...
        <div class="chart-numbers">
          ${i.map(o=>u`
            <button class="small" title="Edit this number" @click=${()=>e.selectLayer(o.payload.id)}>
              <b>${e.resolve(o.payload.value)??"--"}</b> · <span class="ent-tok">${ye(o,t)}</span>
            </button>`)}
        </div>
    ...
    <div class="hint">${t.length} layer${t.length===1?"":"s"}: ${t.map(r=>ye(r,i)).join(", ")}. Click one in the list to edit it.</div>
    <div class="adders">
      <button class="small" title="Keep the layers where they are and drop the folder" @click=${()=>e.update(r=>Ut(r,n.id))}>Ungroup</button>
    </div>`,{color:J.group,icon:"folder",summary:`${t.length} layers \xB7 ${n.locked?"moves as one":"unlocked"}`})}function ms(e,n){if(n==="inline")return u`${cc(e)}${sa(e,n)}`;let t=e.config.perFamily[n];if(!t)return u`<div class="hint">No settings stored for ${O(n)} yet.</div>
      <button class="small" @click=${()=>e.update(l=>{l.perFamily[n]={placements:{},cornerBodyShape:"circle",borderWidth:2,rules:[]}})}>Add ${O(n)} settings</button>
    ...
    ${n==="corner"?he(e,"corner","Corner content",pc(e,t,i),{color:J.place,icon:"content",summary:t.curvedText?"Big curved text":"Layer canvas"}):m}
    ${he(e,"states","Shape states",vs(e,t.rules,"layout",l=>l.perFamily[n]?.rules,`rules-${n}`),{color:J.states,icon:"states",summary:Jt(t.rules).replace(/\.$/,"")})}
    ${he(e,"placements","Placements",u`
      <div class="hint">${a===0?`Nothing is on the ${O(n)} shape. The Layers card offers to copy another shape's whole arrangement onto it.`:`${a} layer${a===1?" is":"s are"} on the ${O(n)} shape, each with its own frame and size here.`}</div>
      <div class="adders">
        <button class="small" title=${`Put every layer on the ${O(n)} shape at the frame the layer itself carries`}
    ...
    ${sa(e,n)}`}function sa(e,n){let t=!Tt(e.config,n),i=t?"A complication keeps at least one shape.":`Drop the ${O(n)} shape. The watch stops listing this complication for ${O(n)} slots.`;return he(e,"shape","Remove this shape",u`
    <div class="adders">
      <button class="danger small" ?disabled=${t} title=${i} @click=${()=>e.removeFamily(n)}>Remove the ${O(n)} shape</button>
    </div>
    ${t?u`<div class="hint">This is the only shape. Add another before removing it.</div>`:u`<div class="hint">The watch stops listing this complication for ${O(n)} slots.</div>`}`,{color:J.place,icon:"delete",summary:t?"The only shape":"Drops its layout"})}function cc(e){let n=e.config.inline;if(!n)return u`<div class="hint">This complication lists Inline but has no Inline text yet (it was saved by an older integration). The watch shows "No inline layout" until one is added.</div>
    ...
    ${he(e,"content","Inline text",u`
      ...
      ${n.countdown?u`<div class="hint">Ticks down to the value's target: an active timer's finish, or any future timestamp. A paused timer shows its remaining time.</div>`:m}`,{color:ne.text,icon:"text",summary:ht(`${n.label?`${n.label}: `:""}${me(n.value,i)}`,48)})}
    ${he(e,"symbol","Symbol",u`
    ...
    </select>`}var ys=["setColor","setBorderColor","setBackgroundColor"];function $c(e,n,t,i,a,r,o){let l=!Sn[i].includes(we[n.kind]);return u`<div class="change-box">
    <div class="rule-head">
    ...
    ${bs(e,n,a,o)}
      ...
      ${bc(e,n,t,i,a)}`:Cc(e,o.table,n[0],t,i,a,r)}function Cc(e,n,t,i,a,r,o){let l=(S,N)=>e.update($=>{let A=a($);A&&S(A)},N?`${r}-${N}`:void 0),s=n.value??Uo.get(r)??o,d=n.rows.length===0,p=n.numberMode||d&&s!==void 0&&!So(s)&&Sc(e.resolve(s)),c=Sn[i],h=Un.get(r)??new Set,g=n.columns.length===0&&h.size===0?[Co[i]]:[],y=fo(n.columns,[...h,...g.filter(S=>S!==void 0)],c),k=t?e.liveBranch(t):"none",x=t?e.forced.get(t.id)??"live":"live",R=S=>x!=="live"&&(x==="otherwise"?S==="otherwise":x.caseId===S),b=S=>{t&&e.setForced(t.id,R(S)?"live":S==="otherwise"?"otherwise":{caseId:S})},f=S=>{Uo.set(r,S),n.rows.length!==0&&l(N=>xo(N,S),"lhs")},w=()=>l(S=>vo(S,s??M(""),p)),E=n.rows.map((S,N)=>Wo(e,{key:`${r}-${S.caseId}`,label:$o(S.comparison,$=>me($,pe(e))),columns:y,changes:S.changes,live:k===S.caseId,forced:R(S.caseId),onForce:()=>b(S.caseId),when:Mc(e,S.comparison,`${r}-${S.caseId}`,($,A)=>l(D=>{let W=D[0]?.cases.find(q=>q.id===S.caseId)?.when.tests[0];W&&$(W.comparison)},A&&`${S.caseId}-${A}`)),updChanges:($,A)=>l(D=>{let W=D[0]?.cases.find(q=>q.id===S.caseId);W&&$(W.then)},A&&`${S.caseId}-${A}`),acts:u`
      <button class="icon" title="Move up" ?disabled=${N===0} @click=${()=>l($=>Qi($,N,N-1))}>${z("up")}</button>
      <button class="icon" title="Move down" ?disabled=${N===n.rows.length-1} @click=${()=>l($=>Qi($,N,N+1))}>${z("down")}</button>
      <button class="icon danger" title="Delete this state" @click=${()=>l($=>wo($,S.caseId))}>${z("delete")}</button>`})),_=n.otherwise===void 0?m:Wo(e,{key:`${r}-otherwise`,label:"Otherwise",columns:y,changes:n.otherwise,live:k==="otherwise",forced:R("otherwise"),onForce:()=>b("otherwise"),when:u`<span class="when-otherwise">Otherwise</span>`,updChanges:(S,N)=>l($=>{let A=$[0]?.otherwise;A&&S(A)},N),acts:u`<button class="icon" title="Remove the Otherwise row" @click=${()=>l(S=>ea(S,!1))}>${z("close")}</button>`}),G=Kn.get(r),ue=Ec.filter(S=>c.includes(S)&&!y.includes(S));return u`
    <div class="states">
      ...
      <table class="states-table">
        <thead>
          <tr>
            ...
            ${y.map(S=>u`<th>
              <span>${Ge[S]}</span>
              <button class="icon" title=${`Remove the ${Ge[S]} column`}
                @click=${N=>{Kn.set(r,S),Se(N.target)}}>${z("close")}</button>
            </th>`)}
            <th class="acts"></th>
          </tr>
        </thead>
      ...
      ${G===void 0?m:u`<div class="hint warn confirm-row">
        ...
        <button class="danger small" @click=${S=>{Kn.delete(r),Un.get(r)?.delete(G),Se(S.target),l(N=>ko(N,G))}}>Remove</button>
        <button class="small" @click=${S=>{Kn.delete(r),Se(S.target)}}>Cancel</button>
      </div>`}
      <div class="states-foot">
        <button class="small" @click=${w}>+ state</button>
        ${n.otherwise===void 0?u`<button class="small" title="What this layer looks like when no state above matches" @click=${()=>l(S=>ea(S,!0))}>+ otherwise</button>`:m}
        <span class="spacer"></span>
        ${x==="live"?m:u`<button class="small" @click=${()=>t&&e.setForced(t.id,"live")}>Back to live</button>`}
        ${ue.length===0?m:u`<select class="chip-add" title="Add a column" @change=${S=>{let N=S.target,$=N.value;if(N.value="",!$)return;let A=Un.get(r)??new Set;A.add($),Un.set(r,A),Se(N)}}>
          <option value="" selected>+ column…</option>
          ${ue.map(S=>u`<option value=${S}>${Ge[S]}</option>`)}
        </select>`}
      </div>
      ...
      <div class="states-switch">
        <button class="link" @click=${S=>{la.add(r),Se(S.target)}}>Advanced</button>
    ...
    <td class="acts">${n.acts}</td>
      ...
      @click=${d=>{i(p=>{p.push(st(mo[n]))}),Gd(d.target,o)}}>unchanged</button>`;let l=(d,p)=>i(c=>{let h=c.find(g=>we[g.kind]===n);h&&d(h)},p&&`${n}-${p}`),s=Ge[n];return u`
    ...
    <div class="value-pop" id=${o} popover role="dialog" aria-label=${s} @toggle=${es}>
      ...
      ${Qt.has(o)?u`${n==="visibility"?ee("This state",r.kind==="hide"?"hide":"show",[["show","Shown"],["hide","Hidden"]],d=>l(p=>{p.kind=d})):bs(e,r,l,a)}
          <button class="link" @click=${d=>{d.target.closest("[popover]")?.hidePopover(),i(p=>{let c=p.findIndex(h=>we[h.kind]===n);c>=0&&p.splice(c,1)})}}>Leave ${s.toLowerCase()} unchanged</button>`:m}
      ...
      <span class="cell-word">${a?Te(a):me(i,pe(e))}</span>`;if(n.kind==="setIcon"&&a){let r=e.icons.render(a,16,"#FFFFFF");return u`${r??m}<span class="cell-word">${a}</span>`}return u`<span class="cell-word">${me(i,pe(e))}</span>`}function Te(e){return{"#FF453A":"red","#FF9F0A":"orange","#FFD60A":"amber","#34C759":"green","#30D158":"green","#0A84FF":"blue","#64D2FF":"cyan","#BF5AF2":"purple","#FFFFFF":"white","#8E8E93":"grey","#000000":"black","#FFCC00":"amber","#FF3B30":"red"}[e.toUpperCase()]??e}function Mc(e,n,t,i){let a=ot(n.kind),r=Xi(n.kind),o=(l,s,d,p)=>Ac(e,l,s,`${t}-${d}`,r,p,d==="rhs"?"Compare with":"Upper bound");return u`<span class="when-cell">
    ...
    :host {
      ...
      --wa-text: ${be(ne.text)};
      --wa-icon: ${be(ne.icon)};
      --wa-gauge: ${be(ne.gauge)};
      --wa-shape: ${be(ne.shape)};
      --wa-image: ${be(ne.image)};
      --wa-tap: ${be(ne.tap)};
    ...
    * { box-sizing: border-box; }
    ...
    .layer .lockbtn.on { opacity: 1; color: ${be(J.locked)}; filter: drop-shadow(0 0 4px ${be(J.locked)}); }
  ...
  `}connectedCallback(){super.connectedCallback(),this.loadColumnWidths(),this.loadListView(),this.sizeObserver.observe(this),window.addEventListener("keydown",this.keyHandler),window.addEventListener("keyup",this.keyUpHandler),window.addEventListener("beforeunload",this.beforeUnload),this.loadOwners()}loadColumnWidths(){try{let t=window.localStorage.getItem(Ls);if(!t)return;let i=JSON.parse(t);typeof i.left=="number"&&(this.colLeft=va(i.left)),typeof i.right=="number"&&(this.colRight=va(i.right))}catch{}}saveColumnWidths(){try{window.localStorage.setItem(Ls,JSON.stringify({left:this.colLeft,right:this.colRight}))}catch{}}loadListView(){try{let t=window.localStorage.getItem(Hs);if(!t)return;let i=JSON.parse(t);(i.thumbStep===0||i.thumbStep===1||i.thumbStep===2)&&(this.thumbStep=i.thumbStep),(i.detail==="compact"||i.detail==="expanded")&&(this.layerDetail=i.detail),typeof i.addOpen=="boolean"&&(this.addOpen=i.addOpen),(i.addDetail==="compact"||i.addDetail==="expanded")&&(this.addDetail=i.addDetail)}catch{}}saveListView(){try{window.localStorage.setItem(Hs,JSON.stringify({thumbStep:this.thumbStep,detail:this.layerDetail,addOpen:this.addOpen,addDetail:this.addDetail}))}catch{}}renderGutter(t){return u`<div class="gutter ${t}" role="separator" aria-orientation="vertical"
      ...
      ${a&&r.length>0?u`<div class="adders">
            ${r.map(o=>u`<button class="small primary"
              title=${`Put every layer on the ${O(i)} shape where it sits on the ${O(o)} one, scaled to this canvas`}
              @click=${()=>this.mutate(l=>ls(l,o,i))}>Copy the ${O(o)} layout</button>`)}
          </div>
    ...
    </dialog>`}setShowTaps(t){this.showTaps=t,t&&this.togglePicking(!1)}togglePicking(t=!this.picking){this.picking=t,this.pickHoverId=void 0,t&&(this.showTaps=!1,this.cancelGesture?.())}hitLayerId(t){let i=this.draft?.config;if(!i)return;let r=t.target?.closest?.("[data-element-id]")?.getAttribute("data-element-id");return r?Ti(i,r):void 0}leaveRow(t){this.listHoverIds.length===t.length&&this.listHoverIds.every((a,r)=>t[r]===a)&&(this.listHoverIds=[])}onPickMove(t){this.picking&&(this.pickHoverId=this.hitLayerId(t))}pickAt(t,i){let a=this.hitLayerId(i);this.togglePicking(!1),a&&(t!==this.activeFamily&&(this.activeFamily=t),this.inspect={kind:"layer",id:a})}onPreviewPointerDown(t,i){if(this.picking){i.preventDefault(),this.pickAt(t,i);return}let a=i.target,r=a.closest("[data-handle]")?.getAttribute("data-handle"),o=a.closest("[data-element-id]")?.getAttribute("data-element-id")??void 0,l=a.closest("svg"),s=a.closest("[data-ts-corner]")?.getAttribute("data-ts-corner")??null,d=s!==null||a.closest("[data-ts-handle]")!==null;if(d||(this.timestampActiveId=void 0),this.showTaps){let x=this.focusTapId();if(x!==void 0&&o===x&&l&&this.draft&&this.canEdit){if(t!==this.activeFamily){this.activeFamily=t;return}i.preventDefault(),this.beginTapBoxGesture(t,i,l,x,r??void 0);return}let R=this.hitLayerId(i);R?this.inspect={kind:"layer",id:R}:o===void 0&&(this.inspect={kind:"general"});return}if(!this.draft||!this.canEdit)return;if(t!==this.activeFamily){this.activeFamily=t;return}let p=zs(i);if(!p&&this.multi.size>0&&(this.multi=new Set),!o||!l)return;let c=Ti(this.draft.config,o),h=this.draft.config.elements.find(x=>x.payload.id===c);if(!c||!h)return;if(p){i.preventDefault(),this.togglePick(c);return}let g=Oe(this.draft.config,c);if(g?.locked&&!r&&!d){this.beginGroupGesture(t,i,l,g);return}if((this.inspect.kind!=="layer"||this.inspect.id!==c)&&(this.inspect={kind:"layer",id:c},r))return;i.preventDefault();let y=ae(this.draft.config,t,h).frame,k=this.gestureCanvas(t);if(d&&h.kind==="image"&&h.payload.timestamp===!0){this.timestampActiveId=c;let x=h.payload,R=oe[t],b=y.width*R.width,f=y.height*R.height,w={x:0,y:0,w:b,h:f,cx:b/2,cy:f/2},E=In(x,w,Mn(new Date));if(this.cancelGesture?.(),s){let S=k.width/R.width,N=x.timestampSize;this.cancelGesture=Ho(l,i,s,{w:E.w*S,h:E.h*S},($,A)=>{let D=Math.min(40,Math.max(4,Math.round(N*$)));this.mutate(W=>{let q=W.elements.find(Y=>Y.payload.id===c);q?.kind==="image"&&(q.payload.timestampSize=D)},`ts-size-${c}`),A&&(this.draft?.endGesture(),this.cancelGesture=void 0)});return}let _={x:0,y:0,w:y.width*k.width,h:y.height*k.height},G=_e(x)?{x:x.timestampX,y:x.timestampY}:{x:(E.x+E.w/2)/w.w,y:(E.y+E.h/2)/w.h},ue=!1;this.cancelGesture=Ao(l,_,i,G,(S,N,$)=>{$||(ue=!0),ue&&this.mutate(A=>{let D=A.elements.find(W=>W.payload.id===c);D?.kind==="image"&&(D.payload.timestampX=S,D.payload.timestampY=N)},`ts-${c}`),$&&(this.draft?.endGesture(),this.cancelGesture=void 0)});return}this.cancelGesture?.(),this.cancelGesture=Nn(l,k,i,{elementId:c,frame:y,handle:r??void 0},{onFrame:(x,R,b)=>{this.mutate(f=>xe(f,t,x,{frame:R}),`drag-${x}-${t}`),b&&(this.draft?.endGesture(),this.cancelGesture=void 0)}})}beginGroupGesture(t,i,a,r){let o=this.draft?.config;if(!o)return;let l=Ne(o,r.id);if(l.length===0)return;(this.inspect.kind!=="group"||this.inspect.id!==r.id)&&(this.inspect={kind:"group",id:r.id}),i.preventDefault();let s=new Map(l.map(x=>[x.payload.id,ae(o,t,x).frame])),d=[...s.values()],p=Math.min(...d.map(x=>x.x)),c=Math.min(...d.map(x=>x.y)),h=Math.max(...d.map(x=>x.x+x.width)),g=Math.max(...d.map(x=>x.y+x.height)),y={x:p,y:c,width:h-p,height:g-c,rotationDegrees:0},k=x=>Math.round(x*1e3)/1e3;this.cancelGesture?.(),this.cancelGesture=Nn(a,this.gestureCanvas(t),i,{elementId:r.id,frame:y},{onFrame:(x,R,b)=>{let f=R.x-y.x,w=R.y-y.y;this.mutate(E=>{for(let[_,G]of s)xe(E,t,_,{frame:{...G,x:k(G.x+f),y:k(G.y+w)}})},`drag-group-${r.id}-${t}`),b&&(this.draft?.endGesture(),this.cancelGesture=void 0)}})}nudge(t,i,a){let r=this.draft?.config;if(!r||!this.canEdit||this.showTaps||this.picking)return!1;let o=a?Mo:1,l=t*o,s=i*o,d=this.canvasFamily,p=oe[d];if(this.timestampActiveId!==void 0&&this.nudgeTimestamp(this.timestampActiveId,d,l,s))return!0;if(this.multi.size>=2)return this.nudgeMany([...this.multi],d,p,`nudge-multi-${d}`,l,s);if(this.inspect.kind==="group"){let x=this.inspect.id;return this.nudgeMany(Ne(r,x).map(R=>R.payload.id),d,p,`nudge-group-${x}-${d}`,l,s)}if(this.inspect.kind!=="layer")return!1;let c=this.inspect.id,h=r.elements.find(x=>x.payload.id===c);if(!h)return!1;let g=Oe(r,c);if(g?.locked)return this.nudgeMany(Ne(r,g.id).map(x=>x.payload.id),d,p,`nudge-group-${g.id}-${d}`,l,s);let y=ae(r,d,h).frame,k=na(y,l,s,p);return(k.x!==y.x||k.y!==y.y)&&this.mutate(x=>xe(x,d,c,{frame:k}),`nudge-${c}-${d}`),!0}nudgeMany(t,i,a,r,o,l){let s=this.draft?.config;if(!s)return!1;let d=w=>Math.round(w*1e3)/1e3,p=new Map;for(let w of t){let E=s.elements.find(_=>_.payload.id===w);E&&p.set(w,ae(s,i,E).frame)}if(p.size===0)return!1;let c=[...p.values()],h=Math.min(...c.map(w=>w.x)),g=Math.min(...c.map(w=>w.y)),y=Math.max(...c.map(w=>w.x+w.width)),k=Math.max(...c.map(w=>w.y+w.height)),x={x:h,y:g,width:y-h,height:k-g,rotationDegrees:0},R=na(x,o,l,a),b=R.x-x.x,f=R.y-x.y;return(b!==0||f!==0)&&this.mutate(w=>{for(let[E,_]of p)xe(w,i,E,{frame:{..._,x:d(_.x+b),y:d(_.y+f)}})},r),!0}nudgeTimestamp(t,i,a,r){let o=this.draft?.config,l=o?.elements.find(x=>x.payload.id===t);if(!o||l?.kind!=="image"||l.payload.timestamp!==!0)return!1;let s=l.payload,d=oe[i],p=ae(o,i,l).frame,c=p.width*d.width,h=p.height*d.height,g=In(s,{x:0,y:0,w:c,h,cx:c/2,cy:h/2},Mn(new Date)),y=_e(s)?{x:s.timestampX,y:s.timestampY}:{x:c>0?(g.x+g.w/2)/c:.5,y:h>0?(g.y+g.h/2)/h:.5},k=Io(y,a,r,{w:c,h});return(k.x!==y.x||k.y!==y.y)&&this.mutate(x=>{let R=x.elements.find(b=>b.payload.id===t);R?.kind==="image"&&(R.payload.timestampX=k.x,R.payload.timestampY=k.y)},`nudge-ts-${t}`),!0}gestureCanvas(t){let i=Rn(this.previewSlot(t),t);if(t!=="corner")return{width:i.width,height:i.height};let a=this.draft?.config.perFamily.corner,r=!!a?.bezelText||!!a?.bezelGauge,o=Ki(i.scale,r);return{width:o,height:o}}focusTapId(){let t=this.draft?.config;if(!t||!this.showTaps||this.inspect.kind!=="layer")return;let i=this.inspect.id,a=t.elements.find(r=>r.payload.id===i);if(a)return a.kind==="tap"?a.payload.id:$e(t,i)[0]?.payload.id}beginTapBoxGesture(t,i,a,r,o){let l=this.draft?.config,s=l?.elements.find(c=>c.payload.id===r);if(!l||!s)return;let d=se(l,s),p=ae(l,t,s).frame;this.cancelGesture?.(),this.cancelGesture=Nn(a,this.gestureCanvas(t),i,{elementId:r,frame:p,handle:o},{onFrame:(c,h,g)=>{this.mutate(y=>{d?Tr(y,c,t,h):xe(y,t,c,{frame:h})},`tap-box-${c}-${t}`),g&&(this.draft?.endGesture(),this.cancelGesture=void 0)}})}render(){let t=this.draft,i=!!t?.dirty,a=this.narrow?{columns:1,left:this.colLeft,right:this.colRight}:_s(this.panelWidth,this.colLeft,this.colRight);return u`
      <header>
      ...
      ${this.watchSupported?u`<div class="layout cols-${a.columns}"
          ...
          </div>`}`}pickerRows(){return[...this.records.map(i=>({slot:Number(i.document?.slotIndex??0),kind:"record",record:i})),...this.occupied.map(i=>i.kind==="custom"?{slot:i.slot,kind:"locked",name:i.name||"Unnamed complication",badge:i.home||"Other home",title:`A complication on ${i.home?`the ${i.home} home`:"another home"}${i.families?.length?` (${i.families.map(O).join(", ")})`:""}. Edit it in that home's Wrist Assistant panel.`,families:i.families??[]}:{slot:i.slot,kind:"locked",name:i.name||"Unnamed preset",badge:"iPhone",title:"An iPhone preset complication. Edit it in the Wrist Assistant app on the iPhone.",families:[]})].sort((i,a)=>i.slot-a.slot)}shapeDots(t){return u`<span class="shape-dots">${Et.map(i=>u`<span class="shape-dot ${i} ${t.includes(i)?"on":""}" title=${O(i)}></span>`)}</span>`}renderPicker(){let t=this.draft,i=this.records.find(s=>s.id===this.selectedId),a=t?t.config.name.trim()||"Untitled":"No complication",r=t?t.config.supportedFamilies:[],o=this.pickerRows(),l=this.freeSlot();return u`<div class="picker">
      ...
      ${this.pickerOpen?u`<div class="menu" role="listbox">
        ...
        ${this.hass.user?.is_admin?u`
          ...
          ${this.newShapeChooser&&l>=0?u`<div class="new-shape">
            ...
            <div class="adders">
              ${Et.map(s=>u`<button class="small ${s==="rectangular"?"primary":""}" @click=${()=>{this.togglePicker(!1),this.createNew(s)}}>${O(s)}</button>`)}
            </div>
          </div>`:m}`:m}
      </div>`:m}
      ...
      ${this.renderPresetDialog()}
        ...
        <span class="right">
          <span class="badges">
            ${ke?u`<span class="badge tap" title=${`Tappable \xB7 ${ye(ke,c)}`}>tap</span>`:m}
          ...
          ${i?u`<span class="acts">
            ...
            <button class="icon" title=${`${q.isHidden?"Show in":"Hide in"} ${O(a)} (${wa}${He}H)`} aria-label=${q.isHidden?"Show this layer":"Hide this layer"} @click=${P=>{P.stopPropagation(),this.mutate(Z=>xe(Z,a,D,{isHidden:!q.isHidden}))}}>${z(q.isHidden?"hide":"show")}</button>
      ...
      </div>`},ue=($,A)=>{let D=this.inspect.kind==="group"&&this.inspect.id===$.id,W=!this.collapsed.has($.id),q=this.rowDrag($.id,i),Y=A[0],ke=A[A.length-1],Xe=C=>{let P=C.currentTarget,Z=P.getBoundingClientRect(),fe=Z.top+(P.classList.contains("drop-before")?Ue:0),Ps=Z.bottom-(P.classList.contains("drop-after")?Ue:0),$a=(C.clientY-fe)/Math.max(1,Ps-fe);return $a<.25?"drop-before":!W&&$a>.75?"drop-after":"drop-into"},T=A.map(C=>C.payload.id);return u`<div class="layer group ${D?"hl":""} ${_?"rich":""}" style=${`--k:${J.group}`} tabindex="0" draggable=${q.draggable}
        ...
        @drop=${C=>{C.preventDefault();let P=Xe(C);this.clearDragMarks();let Z=this.dragId;if(this.dragId=void 0,!(!Z||!Y||!ke)){if(P==="drop-before"){this.reorderLayer(Z,Y.payload.id,!0,!0);return}if(P==="drop-after"){this.reorderLayer(Z,ke.payload.id,!1,!0);return}this.isGroupId(Z)||(this.reorderLayer(Z,Y.payload.id,!0),this.mutate(fe=>xi(fe,Z,$.id)))}}}>
        <button class="chev" aria-expanded=${W?"true":"false"} title=${W?"Fold the group":"Unfold the group"}
          @click=${C=>{C.stopPropagation();let P=new Set(this.collapsed);W?P.add($.id):P.delete($.id),this.collapsed=P}}>${z("chevron")}</button>
        <span class="bar"></span>
        ${E(A.map(C=>C.payload.id))}
        <span class="name">
          <b>${$.name}</b>
          <small><span class="kind">Group</span> · ${A.length} layer${A.length===1?"":"s"} · ${$.locked?"moves as one":"unlocked"}</small>
          ${_?u`<span class="facts"><span class="fact"><b>Holds</b> ${A.map(C=>ye(C,c)).join(", ")}</span></span>`:m}
        </span>
        <span class="right">
          ${i?u`<span class="acts">
          ...
          <button class="icon lockbtn ${$.locked?"on":""}" ?disabled=${!i}
            title=${$.locked?"Locked: drags on the watch move the whole group. Click to unlock.":"Unlocked: each layer moves alone. Click to lock."}
            aria-label=${$.locked?"Unlock the group":"Lock the group"}
            @click=${C=>{C.stopPropagation(),this.mutate(P=>{let Z=P.groups?.find(fe=>fe.id===$.id);Z&&(Z.locked=!Z.locked)})}}>${z($.locked?"lock":"unlock")}</button>
        </span>
      </div>`},S=[],N=new Set;for(let $=0;$<d.length;$++){let A=d[$],D=A.payload.groupId,W=D===void 0?void 0:t.groups?.find(Y=>Y.id===D);if(!W){S.push(G(A,!1));continue}if(N.has(W.id))continue;N.add(W.id);let q=d.filter(Y=>Y.payload.groupId===W.id);S.push(ue(W,q)),this.collapsed.has(W.id)||S.push(u`<div class="group-kids">${q.map(Y=>G(Y,!0))}</div>`)}return u`<div class="card">
      ...
      <div class="layer pinned ${y?"hl":""}" style=${`--k:${J.place}`} tabindex="0" title="The shape is always the bottom layer"
        ...
        @drop=${$=>{$.preventDefault(),this.clearDragMarks();let A=this.dragId,D=[...d].reverse().find(W=>W.payload.id!==A&&W.payload.groupId!==A);A&&D&&this.reorderLayer(A,D.payload.id,!1,!0),this.dragId=void 0}}>
        <span class="grip">${z("shape")}</span>
        <span class="bar"></span>
        ${E([])}
        <span class="name">
          <b>${O(this.activeFamily)} shape</b>
          <small><span class="kind">Background</span> · ${k}</small>
        </span>
      ...
      <summary>${o} layer${o===1?"":"s"} not on the ${O(i)} shape</summary>
      <div class="off-rows">
        ${t.map(l=>u`<div class="off-row" style=${`--k:${ne[l.kind]}`} tabindex="0"
          title=${`${ye(l,r)} is on the complication but not on this shape`}
          ...
          ${a?u`<button class="icon" title=${`Put it on the ${O(i)} shape`}
            aria-label=${`Put it on the ${O(i)} shape`}
            @click=${s=>{s.stopPropagation(),this.mutate(d=>xe(d,i,l.payload.id,{isHidden:!1}))}}>${z("show")}</button>`:m}
        </div>`)}
      </div>
      ...
      ${Wi(r,g)}
    </div>`}renderUnder(t,i){let a=pe(this.host()),r=this.inspect,o=r.kind==="layer"?t.elements.find(c=>c.payload.id===r.id):void 0,l;if(this.showTaps)l=u`Every tap area is outlined. Where two overlap, the one higher in Layers wins. Anywhere else does <b>${Pe(t.tapAction)}</b>.`;else if(this.picking)l="Point at a layer and click it. Escape stops.";else if(i==="inline")l="One line of text. Edit it on the right.";else if(r.kind==="group"){let c=t.groups?.find(g=>g.id===r.id),h=c?Ne(t,c.id).length:0;l=c?u`editing group <b>${c.name}</b>. ${c.locked?`Drag to move all ${h} layers.`:"Unlocked: each layer drags alone."}`:""}else if(o){let c=Oe(t,o.payload.id);l=c?.locked?u`editing <b>${ye(o,a)}</b> in <b>${c.name}</b>. A drag moves the whole group; pull a corner to resize this layer. Arrow keys nudge the group.`:u`editing <b>${ye(o,a)}</b>. Drag it, or pull a corner. Arrow keys nudge it.`}else l="click a layer to edit it";if(i==="inline")return u`<div class="under"><b>Inline</b> · ${l}</div>`;let s=this.currentCase().slots[i],d=Rn(s,i),p=Math.round(d.scale*100);return u`<div class="under"><b>${O(i)}</b> · ${s.width} × ${s.height} pt${p!==100?` \xB7 ${p}%`:""} · ${l}</div>`}renderInlinePreview(t,i){let a;if(!t)a=u`<div class="inline-line missing">No inline text</div>`;else{let r=Date.now(),o=t.countdownEnd!==void 0&&t.countdownEnd>r?St((t.countdownEnd-r)/1e3):t.text,l=t.symbol?this.icons.render(t.symbol,i?11:15,"#FFFFFF"):void 0;a=u`<div class="inline-line">${l??m}<span>${t.label?`${t.label}: `:""}${o}</span></div>`}return i?a:u`<div class="preview inline active" @click=${()=>{this.inspect={kind:"family"}}}>${a}</div>`}renderSettingsRow(t){let i=this.host(),a=this.records.find(p=>p.id===this.selectedId),r=this.selectedOwner,o=[a?`Revision ${a.revision}`:"Not saved yet",r?xa(r):void 0].filter(Boolean).join(" \xB7 "),l=t.values,s=new Ve(this.buildContext(),this.draft?.config),d=pe(i);return u`<div class="strip-row" style=${`--c:${J.complication}`} @change=${()=>this.draft?.endGesture()}>
      ...
      <div class="values-list">
        ...
        ${l.map(p=>{let c=s.resolve({kind:{kind:"named",id:p.id}}),h=this.inspect.kind==="data"&&this.inspect.id===p.id;return u`<div class="datum ${h?"hl":""}" @click=${()=>{this.inspect={kind:"data",id:p.id}}}>
            ...
            <span class="meta ${c===void 0?"none":""}" title=${me(p.value,d)}>${c??"unresolved"}</span>
            ${this.canEdit?u`<button class="icon danger" title="Delete value" aria-label="Delete value" @click=${g=>{g.stopPropagation(),this.mutate(y=>{y.values=y.values.filter(k=>k.id!==p.id)}),h&&(this.inspect={kind:"general"})}}>${z("delete")}</button>`:m}
          </div>`})}
        </div>`}
      </div>
      ...
      <div class="tiles">
        ${Et.map(r=>{if(!a.includes(r))return u`<button class="tile off ${r}" ?disabled=${!this.canEdit} title=${`Add the ${O(r)} shape`} @click=${()=>this.addShape(r)}>
      ...
      ${i.length===0?u`<div class="hint">No entities yet. Give a layer an entity and its live value shows here.</div>`:u`<div class="chips values">
        ${i.map(r=>{let o=this.hass.states[r],l=typeof o?.attributes.friendly_name=="string"?o.attributes.friendly_name:r,s=typeof o?.attributes.unit_of_measurement=="string"?` ${o.attributes.unit_of_measurement}`:"",d=o?`${o.state}${s}`:"not in Home Assistant",p=this.testValues.get(r),h=t.elements.find(y=>Cn(t,y.payload.id).some(k=>k.ref.entityId===r))?.kind??"text",g=this.editingValue===r;return u`<button class="vchip ${p!==void 0?"testing":""}" style=${`--k:${ne[h]}`}
      ...
      <span>${t}${i==="mixed"?u` <span class="mixed">(mixed)</span>`:m}</span></label>`}multiEditor(t,i){let a=this.canvasFamily,r=pe(this.host()),o=new Ve(this.buildContext(),this.draft?.config),l=ds(t,a,i),s=i.length,d=[...i].reverse(),p=h=>this.mutate(g=>{for(let y of i)xe(g,a,y.payload.id,{isHidden:h})}),c=h=>this.mutate(g=>{for(let y of i){let k=g.elements.find(x=>x.payload.id===y.payload.id);k&&k.kind!=="image"&&k.kind!=="tap"&&(k.payload.colorSlot.baseColorHex=h)}},"multi-colour");return u`
          ...
          <div class="picked">
            ${d.map(h=>u`<div class="row" style=${`--k:${ne[h.kind]}`}>
              ...
              <span class="name">
                ...
                <b>${ye(h,r)}</b><span class="kind">${dt[h.kind]}</span>
              </span>
            </div>`)}
          </div>
      ...
      <div class="sec" data-open="true" style=${`--c:${J.place}`}>
        ...
        <div class="sec-b">
          ...
          <div class="hint">Hiding, like size and place, belongs to the ${O(a)} shape alone.</div>
